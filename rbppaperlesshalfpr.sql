...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `buscar_das` (IN `linea` VARCHAR(255), OUT `id_das` INT)   BEGIN
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `llenarDas` (OUT `idDas` INT, IN `linea` VARCHAR(10), IN `codEmp` VARCHAR(20), IN `codKeep` VARCHAR(20), IN `codIns` VARCHAR(20), IN `fecha` DATE, IN `proce` VARCHAR(20))  NO SQL BEGIN
...
INSERT INTO `das`(`linea`, `empleado_id_empleado`, `id_keeper`, `id_inspector`, `fecha`,`activaSupervisor`,`activaOperador`, `procesO`) VALUES(linea,idE,idKe,idIn,fecha,1,1,proce);
