...
--
-- Servidor: localhost
-- Tiempo de generación: 25-03-2025 a las 16:19:39
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `actualizarInspectorDAS` (IN `das_id` INT, IN `inspector_id` VARCHAR(20))   BEGIN
    UPDATE das
    SET id_inspector = (SELECT empleado.id_empleado FROM empleado WHERE empleado.codigo_alea = inspector_id)
    WHERE id_das = das_id;
END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `actualizarLoteMOG` (IN `nuevo_lote` VARCHAR(20), IN `orden_mog` VARCHAR(30))   BEGIN
	UPDATE mog
    SET lote = nuevo_lote
    WHERE mog = orden_mog;
END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `actualizarMetaPiezas` (IN `meta` INT, IN `das_id` INT)   BEGIN 
	UPDATE das
	SET piezas_meta = meta
    WHERE id_das = das_id;
END$$
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `actualizarSoporteDAS` (IN `das_id` INT, IN `soporte_id` VARCHAR(20))   BEGIN
    UPDATE das
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `buscarMOG` (IN `mogg` VARCHAR(50))   BEGIN
	SELECT mog, mog.cantidad_planeada 
    FROM mog 
    WHERE mog.mog=mogg;
END$$
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `buscar_das` (IN `linea` VARCHAR(255), OUT `id_das` INT, IN `turno` INT, OUT `estado` TINYINT)   BEGIN
    SELECT d.id_das INTO id_das
    FROM das AS d
    WHERE d.linea = linea
    AND d.turno = turno
    AND d.fecha = CASE
        WHEN CURTIME() >= '07:00:00' AND CURTIME() <= '23:59:59' THEN CURDATE()
        ELSE DATE_SUB(CURDATE(), INTERVAL 1 DAY)
    END;

    SELECT d.activaOperador INTO estado
    FROM das AS d
    WHERE d.linea = linea
    AND d.turno = turno
    AND d.fecha = CASE
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `EstaCerradaOrdenParaOperador` (IN `orden` VARCHAR(50), OUT `cerrada` BOOLEAN)   BEGIN
    DECLARE activo TINYINT(1);

    SELECT registro_rbp.activo_op INTO activo
    FROM registro_rbp
    WHERE registro_rbp.orden_manufactura = orden
    LIMIT 1;

    SET cerrada = (activo IS NOT NULL AND activo = 0);
END$$
...
SET scr=(SELECT sum(defecto1.cantidad_defecto) from defecto1 WHERE defecto1.registro_rbp_id_registro_rbp=idr);
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `get_razon_rechazo_categoria` (IN `categoria` VARCHAR(50))   BEGIN
    SELECT cr.categoria, sr.subcategoria, rr.nombre_rechazo, rr.id_razon_rechazo
    FROM razon_rechazo1 AS rr
    INNER JOIN categoria_rechazo AS cr ON rr.categoria_rechazo_id_categoria_rechazo = cr.id_categoria_rechazo
    INNER JOIN subcategoria_rechazo AS sr ON sr.id_subcategoria = rr.subcategoria_rechazo_id_subcategoria_rechazo
    WHERE cr.categoria = categoria;
END$$
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `get_scrap_por_rbp` (IN `id_rbp` INT)   begin

SELECT razon_rechazo_id_razon_rechazo, cantidad_defecto, columna FROM `defecto1` where registro_rbp_id_registro_rbp = id_rbp ORDER BY `defecto1`.`columna` ASC;
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `InsertarRegistroCausasParo` (IN `empleado_numero` VARCHAR(50), IN `causas_paro_id_causas_paro` INT, IN `tiempo` INT, IN `detalle` VARCHAR(255), IN `hora_inicio` VARCHAR(50), IN `fecha` DATE, IN `linea` VARCHAR(50), IN `das_id_das` INT, IN `codigo_mog` VARCHAR(40), IN `hora_fin` VARCHAR(50), IN `codigo_andon` INT, IN `codigo_nivel` INT)   BEGIN
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `insertar_cambio_mog` (IN `id_rbp` INT, IN `id_mog` INT)   BEGIN
	INSERT INTO cambio_mog(registro_rbp_id_registro_rbp, mog_id_mog)
    VALUES (id_rbp, id_mog);
END$$
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `insertar_das_prod_pren` (IN `inicio` VARCHAR(10), IN `fin` VARCHAR(10), IN `centro` VARCHAR(5), IN `extremo` VARCHAR(5), IN `piezas_totales` INT, IN `bm_kg` DOUBLE, IN `pcs_bm` INT, IN `pza_ok` INT, IN `ng_kg` DOUBLE, IN `pcs_ng` INT, IN `id_das` INT, IN `mog` VARCHAR(50), IN `id_rbp` INT)   BEGIN

DECLARE id_mog int;
set id_mog = (select mog.id_mog from mog where mog.mog = mog);

insert into das_prod_prensa (inicio_tp, fin_tp, centroEstampado, extremo, sello, pzasTotales, bm_kg, pcs_bm, pza_ok, ng_kg, pcs_ng, das_iddas, mog_idmog, rbp_id_rbp) values(inicio, fin, centro, extremo, '', piezas_totales, bm_kg, pcs_bm, pza_ok, ng_kg, pcs_ng, id_das, id_mog, id_rbp);

END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `insertar_piezas_procesadas` (IN `id_rbp` INT, IN `linea` VARCHAR(10), IN `pzas_bandeja_completa` INT, IN `bandejas_completas` INT, IN `pzas_bandeja_incompleta` INT)   BEGIN
    INSERT INTO piezas_procesadas_prensa (
        registro_rbp_id_registro_rbp,
        linea,
        pzas_bandeja_completa,
        numero_bandejas_completas,
        piezas_bandeja_incompleta
    ) VALUES (
        id_rbp,
        linea,
        pzas_bandeja_completa,
        bandejas_completas,
        pzas_bandeja_incompleta
    ); -- Aquí se agregó el punto y coma
END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `insertar_piezas_procesadas_maquinado` (IN `id_rbp` INT, IN `linea` VARCHAR(4), IN `pzas_procesadas` INT, IN `piezas_fila_canasta` INT, IN `filas_canasta` INT, IN `niveles_canasta` INT, IN `total_canastas` INT, IN `niveles_completos_sobrante` INT, IN `filas_completas_sobrante` INT, IN `cambio_mog` INT, IN `sobrante` INT, IN `rango_canastas_1` INT, IN `rango_canastas_2` INT, IN `id_das` INT)   BEGIN
    -- Insertar el nuevo registro
    INSERT INTO piezas_procesadas_maquinado (
        registro_rbp_id_registro_rbp,
        das_id_das,
        linea,
        rango_canasta_1,
        rango_canasta_2,
        cantidad_piezas_procesadas,
        piezasxfila,
        filas,
        niveles,
        canastas,
        ...
        niveles_canasta,
        total_canastas,
        niveles_completos_sobrante,
        filas_completas_sobrante,
        cambio_mog,
        sobrante
    );
END$$
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `insertar_scrap` (IN `cantidad` INT, IN `razon_rechazo` INT, IN `id_rbp` INT, IN `columna` INT)   BEGIN
    INSERT INTO defecto1 (
        cantidad_defecto,
        razon_rechazo_id_razon_rechazo,
        registro_rbp_id_registro_rbp,
        columna
    ) VALUES (
        cantidad,
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `insertCoil` (IN `orden` VARCHAR(20), IN `lote` VARCHAR(30), IN `mtr` DOUBLE, IN `termi` BOOLEAN, IN `item` VARCHAR(40), IN `id_das` INT)  NO SQL BEGIN
DECLARE reg int;

SET reg=(SELECT id_registro_rbp FROM registro_rbp WHERE registro_rbp.orden_manufactura=orden);

INSERT INTO `lote_coil`(`registro_rbp_id_registro_rbp`, `lote_coil`, `metros`, `f_terminado`, lote_coil.ITEM, lote_coil.das_id_das) VALUES (reg,lote,mtr,termi,item,id_das);
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `llenarDas` (OUT `idDas` INT, IN `linea` VARCHAR(10), IN `codEmp` VARCHAR(20), IN `codKeep` VARCHAR(20), IN `codIns` VARCHAR(20), IN `fecha` DATE, IN `proce` VARCHAR(20), IN `turno` INT)  NO SQL BEGIN
...
INSERT INTO `das`(`linea`, `empleado_id_empleado`, `id_keeper`, `id_inspector`, `fecha`,`activaSupervisor`,`activaOperador`, `procesO`, `turno`) VALUES(linea,idE,idKe,idIn,fecha,1,1,proce,turno);
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtenerAndonPorProceso` (IN `proceso_id` INT)   BEGIN
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtenerCambiosMOG` (IN `rbp_id` INT)   BEGIN

SELECT mog AS mog_hija, cantidad_planeada FROM mog INNER JOIN cambio_mog ON cambio_mog.mog_id_mog = mog.id_mog INNER JOIN registro_rbp ON registro_rbp.id_registro_rbp = cambio_mog.registro_rbp_id_registro_rbp WHERE cambio_mog.registro_rbp_id_registro_rbp = rbp_id;

END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtenerCategoriasParoProceso` (IN `proceso_id` INT)   BEGIN
  SELECT categoria
  FROM causas_paro
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtenerCausasPorCategoriaParoProceso` (IN `proceso_id` INT, IN `categoria_seleccionada` VARCHAR(30))   BEGIN
  SELECT idcausas_paro, descripcion
  FROM causas_paro
  WHERE procesos_idproceso = proceso_id
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtenerCoilsRBP` (IN `id_rbp` INT)   BEGIN
SELECT lote_coil, sum(metros), f_terminado FROM lote_coil WHERE registro_rbp_id_registro_rbp = id_rbp GROUP BY lote_coil;
END$$
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtenerGrupoMaquina` (IN `cod_maquina` VARCHAR(4), OUT `grupo_maquina` INT(1))   SET grupo_maquina = (SELECT catgm.grupo
FROM catalogo_grupo_maquinado catgm
INNER JOIN work_center_maquina_has_catalogo_grupo wcm_catgm ON catgm.id_grupo = wcm_catgm.catalogo_grupo_maquinado_id_grupo
INNER JOIN work_center_maquina wcm ON wcm_catgm.work_center_maquina_id_work_center_maquina = wcm.id_work_center_maquina
WHERE wcm.codigo_maquina = cod_maquina)$$
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtenerNivelMaquinado` ()   BEGIN
    SELECT id_nivel, nivel
    FROM catalogo_nivel;
END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtenerParosMaquinado` (IN `id_das` INT)   BEGIN

SELECT causas_paro.descripcion, hora_inicio, hora_fin, tiempo, catalogo_andon.descripcion AS andon, catalogo_nivel.escalacion, detalle
FROM registrocausasparo
LEFT JOIN causas_paro ON causas_paro_idcausas_paro = idcausas_paro
LEFT JOIN catalogo_andon ON catalogo_andon_id_andon = id_andon
LEFT JOIN catalogo_nivel ON catalogo_nivel_id_nivel = id_nivel
WHERE das_id_das = id_das
ORDER BY registrocausasparo.hora_inicio;

END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtenerParosPorDas` (IN `id_das` INT)   BEGIN

select fecha, hora_inicio, tiempo, detalle, descripcion as razon from registrocausasparo inner join causas_paro on causas_paro_idcausas_paro = idcausas_paro where das_id_das = id_das;
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtener_piezas_procesadas_maquinado` (IN `id_rbp` INT)   BEGIN
    SELECT rango_canasta_1, rango_canasta_2, niveles_completos, filas_completas, sobrante
    FROM piezas_procesadas_maquinado
    WHERE registro_rbp_id_registro_rbp = id_rbp
    ORDER BY id_piezas_procesadas_maquinado
    DESC LIMIT 1;
END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtener_piezas_x_hora_maq` (IN `id_das` INT)   BEGIN
    ...
    WHERE rxhmq.das_id_das = id_das
    ORDER BY fecha, hora;

END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtener_Registro_Produccion_Maquinado` (IN `das_id` INT)   BEGIN
	SELECT
        mog.mog,
        mog.modelo,
        mog.std,
        mog.lote,
        registrocausasparo.hora_inicio as hri_cambio_modelo,
        registrocausasparo.hora_fin as hrf_cambio_modelo,
        corriendoactualmente.hora_inicio as hri_proceso,
        MAX(registro_x_hora_maq.hora) AS hrf_proceso,
        (SELECT acumulado
          FROM registro_x_hora_maq
          WHERE registro_x_hora_maq.das_id_das = das.id_das
          ORDER BY hora DESC
          LIMIT 1) AS piezas_procesadas,
        (SELECT COALESCE(SUM(cantidadxhora), 0)
         FROM registro_x_hora_maq AS sub_registro
         WHERE sub_registro.das_id_das = das.id_das AND sub_registro.ok_ng = 'OK') AS piezas_buenas,
        (SELECT COALESCE(SUM(cantidadxhora), 0)
         FROM registro_x_hora_maq AS sub_registro
         WHERE sub_registro.das_id_das = das.id_das AND sub_registro.ok_ng = 'NG') AS piezas_rechazadas
    FROM
        mog
    INNER JOIN
        corriendoactualmente ON corriendoactualmente.mogActual = mog.mog
    INNER JOIN
        das ON das.fecha = corriendoactualmente.fecha
    INNER JOIN
        registro_x_hora_maq ON registro_x_hora_maq.das_id_das = das.id_das
    INNER JOIN
        registrocausasparo ON registrocausasparo.das_id_das = das.id_das
    WHERE
        das.id_das = das_id;
END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `obtener_registro_x_hora` (IN `id_das` INT)   BEGIN
    SELECT
        hora,
        cantidadxhora,
        acumulado,
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `previsualizacion_final_das` (IN `id_das` INT)   BEGIN

select lote_coil.ITEM, lote_coil.lote_coil, lote_coil.metros, das_prod_prensa.centroEstampado, das_prod_prensa.extremo, registro_rbp.orden_manufactura, mog.modelo, mog.STD, das_prod_prensa.inicio_tp, das_prod_prensa.fin_tp, das_prod_prensa.pzasTotales, das_prod_prensa.pza_ok, das_prod_prensa.bm_kg, das_prod_prensa.ng_kg, das_prod_prensa.pcs_bm, das_prod_prensa.pcs_ng from lote_coil inner join das_prod_prensa on lote_coil.das_id_das = das_prod_prensa.das_iddas inner join registro_rbp on lote_coil.registro_rbp_id_registro_rbp = registro_rbp.id_registro_rbp inner join mog on registro_rbp.mog_id_mog = mog.id_mog where das_id_das = id_das;
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `RBP_preview` (IN `id_rbp` INT)   BEGIN

SELECT
    piezas_procesadas_prensa.linea,
    das.fecha,
    TIMEDIFF(das_prod_prensa.fin_tp, das_prod_prensa.inicio_tp) AS horas_trabajadas,
    CONCAT(empleado.nombre_empleado, ' ', empleado.apellido) AS nombre_empleado,
    das_prod_prensa.pzasTotales,
    lote_coil.lote_coil,
    lote_coil.metros,
    lote_coil.f_terminado,
    das_prod_prensa.bm_kg,
    das_prod_prensa.ng_kg,
    das_prod_prensa.centroEstampado,
    das_prod_prensa.extremo
FROM
    registro_rbp 
    inner join das_prod_prensa on registro_rbp.id_registro_rbp = das_prod_prensa.rbp_id_rbp
    inner join piezas_procesadas_prensa on registro_rbp.id_registro_rbp = piezas_procesadas_prensa.registro_rbp_id_registro_rbp
    inner join empleado_has_registro_rbp on empleado_has_registro_rbp.registro_rbp_id_registro_rbp = registro_rbp.id_registro_rbp
    inner join empleado on empleado.id_empleado = empleado_has_registro_rbp.empleado_id_empleado
    inner join lote_coil on lote_coil.registro_rbp_id_registro_rbp = registro_rbp.id_registro_rbp
    inner join das on das.id_das = das_prod_prensa.das_iddas
    where registro_rbp.id_registro_rbp = id_rbp;
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `registrar_empleado_has_registro_rbp` (IN `codigo_empleado` VARCHAR(40), IN `id_rbp` INT, IN `codigo_supervisor` VARCHAR(40))   BEGIN

declare id_empleado, id_supervisor int;

select empleado.id_empleado into id_empleado from empleado where empleado.codigo_alea = codigo_empleado;

select empleado.id_empleado into id_supervisor from empleado where empleado.codigo_alea = codigo_supervisor AND empleado.tipo_usuario = "Supervisor";

INSERT INTO empleado_has_registro_rbp values(id_empleado, id_rbp, id_supervisor);

END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `registroxhora` (IN `mogg` VARCHAR(20), IN `orden` VARCHAR(20), IN `hor` VARCHAR(10), IN `cantxh` INT, IN `acum` INT, IN `comen` VARCHAR(1000), IN `fecha` DATE, IN `linea` VARCHAR(20), IN `id_das` INT)  NO SQL BEGIN
...
INSERT INTO `registro_x_hora`(`hora`, `cantidadxhora`, `acumulado`, `comentarios`, `fecha`, `mog_id_mog`, `registro_rbp_id_registro_rbp`, `linea`, `das_id_das`)VALUES(hor,cantxh,acum,comen,fecha,idm,idor, linea, id_das);
END$$

CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `registro_x_hora_maq` (IN `mog_ingresada` VARCHAR(20), IN `orden_ingresada` VARCHAR(20), IN `cod_inspector_ingresado` VARCHAR(20), IN `hora_p` VARCHAR(10), IN `cantxh_p` INT, IN `acum_p` INT, IN `calidad` VARCHAR(2), IN `fecha_p` DATE, IN `linea_p` VARCHAR(20), IN `iddas` INT)   BEGIN

DECLARE idmog int;
DECLARE idorden int;
DECLARE idinspector int;
...
SET idinspector=(SELECT empleado.id_empleado FROM empleado WHERE empleado.codigo_alea=cod_inspector_ingresado);

INSERT INTO `registro_x_hora_maq`(`hora`, `cantidadxhora`, `acumulado`, `ok_ng`, `fecha`, `linea`, `mog_id_mog`, `registro_rbp_id_registro_rbp`, `empleado_id_empleado`, `das_id_das`)
VALUES(hora_p,cantxh_p,acum_p,calidad,fecha_p,linea_p,idmog,idorden,idinspector, iddas);
END$$
...
CREATE DEFINER=`adminpaperless`@`%` PROCEDURE `validarLoteCoil` (IN `loteCoil` VARCHAR(255), OUT `resultado` BOOLEAN)   BEGIN
    DECLARE terminado INT;

    SELECT COUNT(lote_coil) INTO terminado
    FROM lote_coil
    WHERE lote_coil = loteCoil AND f_terminado = 1;
...
--
-- Estructura de tabla para la tabla `cambio_mog`
--

CREATE TABLE `cambio_mog` (
  `id_cambio_mog` int(10) UNSIGNED NOT NULL,
  `registro_rbp_id_registro_rbp` int(10) UNSIGNED NOT NULL,
  `mog_id_mog` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Volcado de datos para la tabla `cambio_mog`
--

INSERT INTO `cambio_mog` (`id_cambio_mog`, `registro_rbp_id_registro_rbp`, `mog_id_mog`) VALUES
(1, 220, 242),
(5, 220, 243),
(6, 220, 244);
...
--
-- Estructura de tabla para la tabla `catalogo_andon`
--

CREATE TABLE `catalogo_andon` (
  `id_andon` int(11) NOT NULL,
  `andon` varchar(2) NOT NULL,
  `descripcion` varchar(30) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Volcado de datos para la tabla `catalogo_andon`
--

INSERT INTO `catalogo_andon` (`id_andon`, `andon`, `descripcion`) VALUES
(1, 'SP', 'Soporte de proceso'),
(2, 'SV', 'Supervisor'),
(3, 'GA', 'Gerente del área'),
(4, 'MT', 'Mantenimiento'),
(5, 'SL', 'Lider de turno');
...
--
-- Estructura de tabla para la tabla `catalogo_andon_has_procesos`
--

CREATE TABLE `catalogo_andon_has_procesos` (
  `catalogo_andon_id_andon` int(11) NOT NULL,
  `procesos_id_proceso` int(10) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Volcado de datos para la tabla `catalogo_andon_has_procesos`
--

INSERT INTO `catalogo_andon_has_procesos` (`catalogo_andon_id_andon`, `procesos_id_proceso`) VALUES
(1, 1),
(1, 2),
(2, 1),
(2, 2),
...
--
-- Estructura de tabla para la tabla `catalogo_grupo_maquinado`
--

CREATE TABLE `catalogo_grupo_maquinado` (
  `id_grupo` int(10) UNSIGNED NOT NULL,
  `grupo` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Volcado de datos para la tabla `catalogo_grupo_maquinado`
--

INSERT INTO `catalogo_grupo_maquinado` (`id_grupo`, `grupo`) VALUES
(1, 1),
(2, 2),
(3, 3);
...
CREATE TABLE `catalogo_nivel` (
  ...
  `escalacion` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `catalogo_nivel` (`id_nivel`, `nivel`, `escalacion`) VALUES
(1, 1, 'Operador'),
(2, 2, 'Soporte de proceso'),
(3, 3, 'Supervisor/Mantenimiento/Inspección'),
(4, 4, 'Gerente/Mantenimiento SV/Inspección');
...
(694, 'TH17', 'HBL056746', 'MOG083559', '07:11:21', '2025-03-05', 1),
(695, 'TP08', 'MOG083559', 'MOG083559', '07:50:14', '2025-03-05', 1),
(696, 'TP08', 'MOG083559', 'MOG083559', '08:08:44', '2025-03-05', 1),
(697, 'TP08', 'MOG083559', 'MOG083559', '08:18:55', '2025-03-05', 1),
(698, 'TP08', 'MOG083559', 'MOG083559', '09:06:42', '2025-03-05', 1),
(699, 'TP08', 'MOG083559', 'MOG083559', '09:08:18', '2025-03-05', 1),
(700, 'TP08', 'MOG083559', 'MOG083559', '11:21:54', '2025-03-05', 1),
(701, 'TP08', 'MOG083559', 'MOG083559', '11:25:18', '2025-03-05', 1),
(702, 'TP08', 'MOG083559', 'MOG083559', '11:26:25', '2025-03-05', 1),
(703, 'TP08', 'MOG083559', 'MOG083559', '11:28:44', '2025-03-05', 1),
(704, 'TP08', 'MOG083559', 'MOG083559', '11:29:57', '2025-03-05', 1),
(705, 'TP08', 'MOG083559', 'MOG083559', '11:30:53', '2025-03-05', 1),
(706, 'TP08', 'MOG083559', 'MOG083559', '11:31:48', '2025-03-05', 1),
(707, 'TP08', 'MOG083559', 'MOG083559', '11:32:22', '2025-03-05', 1),
(708, 'TP08', 'MOG083559', 'MOG083559', '11:52:48', '2025-03-05', 1),
(709, 'TP08', 'MOG083559', 'MOG083559', '13:41:07', '2025-03-05', 1),
(710, 'TP08', 'MOG083558', 'MOG083558', '13:42:32', '2025-03-05', 1),
(711, 'TH17', 'HBL056746', 'MOG083559', '13:48:34', '2025-03-05', 1),
(712, 'TH17', 'HBL056746', 'MOG083559', '13:48:34', '2025-03-05', 1),
(713, 'TH17', 'HBL056746', 'MOG083559', '13:49:26', '2025-03-05', 1),
(714, 'TH17', 'HBL056746', 'MOG083559', '14:03:45', '2025-03-05', 1),
(715, 'TH17', 'HBL056746', 'MOG083559', '14:12:25', '2025-03-05', 1),
(716, 'TH17', 'HBL056746', 'MOG083559', '07:32:40', '2025-03-06', 1),
(717, 'TH17', 'HBL056746', 'MOG083559', '07:35:44', '2025-03-06', 1),
(718, 'TH17', 'HBL056746', 'MOG083559', '07:36:58', '2025-03-06', 1),
(719, 'TH17', 'HBL056746', 'MOG083559', '07:40:41', '2025-03-06', 1),
(720, 'TP08', 'MOG083559', 'MOG083559', '07:44:43', '2025-03-06', 1),
(721, 'TH17', 'HBL056746', 'MOG083559', '07:50:13', '2025-03-06', 1),
(722, 'TP08', 'MOG083559', 'MOG083559', '07:52:36', '2025-03-06', 1),
(723, 'TH17', 'HBL056746', 'MOG083559', '08:01:27', '2025-03-06', 1),
(724, 'TP08', 'MOG083559', 'MOG083559', '08:07:02', '2025-03-06', 1),
(725, 'TP08', 'MOG083559', 'MOG083559', '08:09:00', '2025-03-06', 1),
(726, 'TP08', 'MOG083559', 'MOG083559', '08:13:52', '2025-03-06', 1),
(727, 'TP08', 'MOG083559', 'MOG083559', '08:16:34', '2025-03-06', 1),
(728, 'TP08', 'MOG083559', 'MOG083559', '08:18:41', '2025-03-06', 1),
(729, 'TP08', 'MOG083559', 'MOG083559', '08:25:16', '2025-03-06', 1),
(730, 'TP08', 'MOG083559', 'MOG083559', '08:25:48', '2025-03-06', 1),
(731, 'TH17', 'HBL056746', 'MOG083559', '08:35:15', '2025-03-06', 1),
(732, 'TP08', 'MOG083559', 'MOG083559', '08:40:10', '2025-03-06', 1),
(733, 'TH17', 'HBL056746', 'MOG083559', '08:46:07', '2025-03-06', 1),
(734, 'TP08', 'MOG083559', 'MOG083559', '08:50:31', '2025-03-06', 1),
(735, 'TP08', 'MOG083558', 'MOG083558', '08:52:37', '2025-03-06', 1),
(736, 'TP08', 'MOG083559', 'MOG083559', '08:55:17', '2025-03-06', 1),
(737, 'TP08', 'MOG083559', 'MOG083559', '08:57:59', '2025-03-06', 1);
INSERT INTO `corriendoactualmente` (`id_corriendo`, `linea`, `ordenActual`, `mogActual`, `hora_inicio`, `fecha`, `activo`) VALUES
(738, 'TH17', 'HBL056746', 'MOG083559', '09:00:31', '2025-03-06', 1),
(739, 'TP08', 'MOG083559', 'MOG083559', '09:02:43', '2025-03-06', 1),
(740, 'TP08', 'MOG083559', 'MOG083559', '09:14:18', '2025-03-06', 1),
(741, 'TP08', 'MOG083559', 'MOG083559', '10:07:52', '2025-03-06', 1),
(742, 'TP08', 'MOG083559', 'MOG083559', '10:09:55', '2025-03-06', 1),
(743, 'TP08', 'MOG083559', 'MOG083559', '10:10:31', '2025-03-06', 1),
(744, 'TP08', 'MOG083559', 'MOG083559', '10:12:33', '2025-03-06', 1),
(745, 'TH17', 'HBL056746', 'MOG083559', '10:22:00', '2025-03-06', 1),
(746, 'TP05', 'MOG083559', 'MOG083559', '10:22:49', '2025-03-06', 1),
(747, 'TP08', 'MOG083559', 'MOG083559', '10:23:47', '2025-03-06', 1),
(748, 'TH17', 'HBL056746', 'MOG083559', '10:23:54', '2025-03-06', 1),
(749, 'TH17', 'HBL056746', 'MOG083559', '10:28:26', '2025-03-06', 1),
(750, 'TP08', 'MOG083559', 'MOG083559', '10:28:52', '2025-03-06', 1),
(751, 'TP08', 'MOG083559', 'MOG083559', '10:32:55', '2025-03-06', 1),
(752, 'TH17', 'HBL056746', 'MOG083559', '10:34:20', '2025-03-06', 1),
(753, 'TP08', 'MOG083559', 'MOG083559', '10:36:52', '2025-03-06', 1),
(754, 'TH17', 'HBL056746', 'MOG083559', '10:39:16', '2025-03-06', 1),
(755, 'TP08', 'MOG083559', 'MOG083559', '10:39:56', '2025-03-06', 1),
(756, 'TH17', 'HBL056746', 'MOG083559', '10:47:10', '2025-03-06', 1),
(757, 'TH17', 'HBL056746', 'MOG083559', '10:50:07', '2025-03-06', 1),
(758, 'TH17', 'HBL056746', 'MOG083559', '10:59:40', '2025-03-06', 1),
(759, 'TH17', 'HBL056746', 'MOG083559', '11:01:25', '2025-03-06', 1),
(760, 'TH17', 'HBL056746', 'MOG083559', '11:24:55', '2025-03-06', 1),
(761, 'TH17', 'HBL056746', 'MOG083559', '11:31:19', '2025-03-06', 1),
(762, 'TP08', 'MOG083559', 'MOG083559', '11:32:35', '2025-03-06', 1),
(763, 'TP08', 'MOG083559', 'MOG083559', '11:33:18', '2025-03-06', 1),
(764, 'TH17', 'HBL056746', 'MOG083559', '11:34:32', '2025-03-06', 1),
(765, 'TH17', 'HBL056746', 'MOG083559', '11:35:52', '2025-03-06', 1),
(766, 'TH17', 'HBL056746', 'MOG083559', '11:41:59', '2025-03-06', 1),
(767, 'TH17', 'HBL056746', 'MOG083559', '11:45:39', '2025-03-06', 1),
(768, 'TH17', 'HBL056746', 'MOG083559', '11:47:36', '2025-03-06', 1),
(769, 'TP08', 'MOG083559', 'MOG083559', '11:51:03', '2025-03-06', 1),
(770, 'TP08', 'MOG083559', 'MOG083559', '11:58:10', '2025-03-06', 1),
(771, 'TP08', 'MOG083559', 'MOG083559', '12:00:25', '2025-03-06', 1),
(772, 'TP08', 'MOG083559', 'MOG083559', '12:02:21', '2025-03-06', 1),
(773, 'TP08', 'MOG083559', 'MOG083559', '12:05:14', '2025-03-06', 1),
(774, 'TP08', 'MOG083559', 'MOG083559', '12:06:23', '2025-03-06', 1),
...
(831, 'TP08', 'MOG083559', 'MOG083559', '14:27:55', '2025-03-07', 1),
(832, 'TP08', 'MOG083559', 'MOG083559', '14:32:09', '2025-03-07', 1),
(833, 'TP08', 'MOG083559', 'MOG083559', '14:33:13', '2025-03-07', 1),
(834, 'TP08', 'MOG047460', 'MOG047460', '14:38:17', '2025-03-07', 1),
(835, 'TP08', 'MOG047460', 'MOG047460', '14:42:34', '2025-03-07', 1),
(836, 'TP08', 'MOG047460', 'MOG047460', '14:49:11', '2025-03-07', 1),
(837, 'TP08', 'MOG083559', 'MOG083559', '14:58:52', '2025-03-07', 1),
(838, 'TP08', 'MOG083559', 'MOG083559', '15:24:30', '2025-03-07', 1),
(839, 'TP08', 'MOG083559', 'MOG083559', '15:30:35', '2025-03-07', 1),
(840, 'TP08', 'MOG083559', 'MOG083559', '15:42:14', '2025-03-07', 1),
(841, 'TP08', 'MOG083559', 'MOG083559', '15:43:34', '2025-03-07', 1),
(842, 'TH17', 'HBL056746', 'MOG083559', '07:43:20', '2025-03-10', 1),
(843, 'TP08', 'MOG083559', 'MOG083559', '08:45:19', '2025-03-10', 1),
(844, 'TP08', 'MOG083559', 'MOG083559', '08:46:15', '2025-03-10', 1),
(845, 'TP08', 'MOG083559', 'MOG083559', '08:47:46', '2025-03-10', 1),
(846, 'TP08', 'MOG083559', 'MOG083559', '08:49:59', '2025-03-10', 1),
(847, 'TP08', 'MOG083560', 'MOG083560', '08:55:29', '2025-03-10', 1),
(848, 'TP05', 'MOG083560', 'MOG083560', '09:00:31', '2025-03-10', 1),
(849, 'TP02', 'MOG083560', 'MOG083560', '09:02:54', '2025-03-10', 1),
(850, 'TP02', 'MOG083560', 'MOG083560', '09:03:45', '2025-03-10', 1),
(851, 'TH17', 'HBL056746', 'MOG083559', '09:24:24', '2025-03-10', 1),
(852, 'TH17', 'HBL056746', 'MOG083559', '09:59:17', '2025-03-10', 1),
(853, 'TH17', 'HBL056746', 'MOG083559', '10:02:35', '2025-03-10', 1),
(854, 'TH17', 'HBL056746', 'MOG083559', '10:11:11', '2025-03-10', 1),
(855, 'TH17', 'HBL056746', 'MOG083559', '10:18:48', '2025-03-10', 1),
(856, 'TP08', 'MOG083559', 'MOG083559', '11:02:52', '2025-03-10', 1),
(857, 'TP08', 'MOG083558', 'MOG083558', '11:03:33', '2025-03-10', 1),
(858, 'TP08', 'MOG083559', 'MOG083559', '11:04:12', '2025-03-10', 1),
(859, 'TP08', 'MOG083559', 'MOG083559', '11:04:31', '2025-03-10', 1),
(860, 'TP08', 'MOG083559', 'MOG083559', '11:06:35', '2025-03-10', 1),
(861, 'TP08', 'MOG083911', 'MOG083911', '11:08:39', '2025-03-10', 1),
(862, 'TP08', 'MOG083559', 'MOG083559', '11:09:09', '2025-03-10', 1),
(863, 'TH17', 'HBL056746', 'MOG083559', '11:33:49', '2025-03-10', 1),
(864, 'TP08', 'MOG083559', 'MOG083559', '11:33:57', '2025-03-10', 1),
(865, 'TH17', 'HBL056746', 'MOG083559', '11:42:06', '2025-03-10', 1),
(866, 'TH17', 'HBL056748', 'MOG083561', '11:43:35', '2025-03-10', 1),
(867, 'TH17', 'HBL056746', 'MOG083559', '11:43:40', '2025-03-10', 1),
(868, 'TP07', 'MOG083559', 'MOG083559', '11:43:46', '2025-03-10', 1),
(869, 'TH17', 'HBL056746', 'MOG083559', '11:45:55', '2025-03-10', 1),
(870, 'TH17', 'HBL056746', 'MOG083559', '11:45:58', '2025-03-10', 1),
(871, 'TP08', 'MOG083559', 'MOG083559', '11:47:30', '2025-03-10', 1),
(872, 'TH17', 'HBL056746', 'MOG083559', '11:48:33', '2025-03-10', 1),
(873, 'TH17', 'HBL056746', 'MOG083559', '11:48:54', '2025-03-10', 1),
(874, 'TP08', 'MOG083559', 'MOG083559', '11:49:43', '2025-03-10', 1),
(875, 'TP08', 'MOG083559', 'MOG083559', '11:53:26', '2025-03-10', 1),
(876, 'TP10', 'MOG083559', 'MOG083559', '11:54:54', '2025-03-10', 1),
(877, 'TP07', 'MOG083559', 'MOG083559', '11:55:26', '2025-03-10', 1),
(878, 'TH17', 'HBL056746', 'MOG083559', '11:57:34', '2025-03-10', 1),
(879, 'TH17', 'HBL056746', 'MOG083559', '11:59:21', '2025-03-10', 1),
(880, 'TH17', 'HBL056746', 'MOG083559', '11:59:50', '2025-03-10', 1),
(881, 'TP08', 'MOG083559', 'MOG083559', '12:08:12', '2025-03-10', 1),
(882, 'TH17', 'HBL056746', 'MOG083559', '12:08:24', '2025-03-10', 1),
(883, 'TH17', 'HBL056746', 'MOG083559', '12:09:44', '2025-03-10', 1),
(884, 'TH17', 'HBL056746', 'MOG083559', '12:10:18', '2025-03-10', 1),
(885, 'TH17', 'HBL056746', 'MOG083559', '12:10:38', '2025-03-10', 1),
(886, 'TH17', 'HBL056746', 'MOG083559', '12:11:09', '2025-03-10', 1),
(887, 'TH17', 'HBL056746', 'MOG083559', '12:11:40', '2025-03-10', 1),
(888, 'TH17', 'HBL056746', 'MOG083559', '12:12:16', '2025-03-10', 1),
(889, 'TH17', 'HBL056746', 'MOG083559', '12:13:32', '2025-03-10', 1),
(890, 'TH17', 'HBL056746', 'MOG083559', '12:29:10', '2025-03-10', 1),
(891, 'TH17', 'HBL056746', 'MOG083559', '12:31:50', '2025-03-10', 1),
(892, 'TH17', 'HBL056746', 'MOG083559', '12:35:24', '2025-03-10', 1),
(893, 'TH17', 'HBL056746', 'MOG083559', '12:37:53', '2025-03-10', 1),
(894, 'TH17', 'HBL056746', 'MOG083559', '12:41:18', '2025-03-10', 1),
(895, 'TP08', 'MOG083559', 'MOG083559', '12:45:39', '2025-03-10', 1),
(896, 'TP08', 'MOG083559', 'MOG083559', '12:45:47', '2025-03-10', 1),
(897, 'TH17', 'HBL056746', 'MOG083559', '12:46:01', '2025-03-10', 1),
(898, 'TP08', 'MOG083559', 'MOG083559', '12:48:29', '2025-03-10', 1),
(899, 'TP08', 'MOG083559', 'MOG083559', '13:55:53', '2025-03-10', 1),
(900, 'TH17', 'HBL056746', 'MOG083559', '14:17:34', '2025-03-10', 1),
(901, 'TH17', 'HBL056746', 'MOG083559', '14:20:09', '2025-03-10', 1),
(902, 'TH17', 'HBL056746', 'MOG083559', '14:20:39', '2025-03-10', 1),
(903, 'TP08', 'MOG083559', 'MOG083559', '14:50:06', '2025-03-10', 1),
(904, 'TP08', 'MOG083559', 'MOG083559', '14:54:34', '2025-03-10', 1),
(905, 'TP08', 'MOG083559', 'MOG083559', '14:56:01', '2025-03-10', 1),
(906, 'TP08', 'MOG083559', 'MOG083559', '15:00:33', '2025-03-10', 1),
(907, 'TP08', 'MOG083559', 'MOG083559', '15:01:30', '2025-03-10', 1),
(908, 'TP08', 'MOG083559', 'MOG083559', '15:05:05', '2025-03-10', 1),
(909, 'TP08', 'MOG083559', 'MOG083559', '15:10:21', '2025-03-10', 1),
(910, 'TP08', 'MOG083559', 'MOG083559', '15:16:02', '2025-03-10', 1),
(911, 'TP08', 'MOG083559', 'MOG083559', '15:17:09', '2025-03-10', 1),
(912, 'TP08', 'MOG083559', 'MOG083559', '15:18:19', '2025-03-10', 1),
(913, 'TP08', 'MOG083559', 'MOG083559', '15:19:18', '2025-03-10', 1),
(914, 'TP08', 'MOG083559', 'MOG083559', '15:20:59', '2025-03-10', 1),
(915, 'TP08', 'MOG083559', 'MOG083559', '15:22:34', '2025-03-10', 1),
(916, 'TH17', 'HBL056746', 'MOG083559', '07:11:38', '2025-03-11', 1),
(917, 'TP08', 'MOG083559', 'MOG083559', '07:21:31', '2025-03-11', 1),
(918, 'TP08', 'MOG083559', 'MOG083559', '07:26:22', '2025-03-11', 1),
(919, 'TH17', 'HBL056746', 'MOG083559', '07:26:49', '2025-03-11', 1),
(920, 'TH17', 'HBL056746', 'MOG083559', '07:28:44', '2025-03-11', 1),
(921, 'TP08', 'MOG083559', 'MOG083559', '07:29:28', '2025-03-11', 1),
(922, 'TH17', 'HBL056746', 'MOG083559', '07:29:46', '2025-03-11', 1),
(923, 'TH17', 'HBL056746', 'MOG083559', '07:30:10', '2025-03-11', 1),
(924, 'TH17', 'HBL056746', 'MOG083559', '07:31:01', '2025-03-11', 1),
(925, 'TP08', 'MOG083559', 'MOG083559', '07:35:39', '2025-03-11', 1),
(926, 'TH17', 'HBL056746', 'MOG083559', '07:42:40', '2025-03-11', 1),
(927, 'TH17', 'HBL056746', 'MOG083559', '07:44:21', '2025-03-11', 1),
(928, 'TH17', 'HBL056746', 'MOG083559', '07:47:04', '2025-03-11', 1),
(929, 'TH17', 'HBL056746', 'MOG083559', '07:48:38', '2025-03-11', 1),
(930, 'TH17', 'HBL056746', 'MOG083559', '07:53:32', '2025-03-11', 1),
(931, 'TH17', 'HBL056746', 'MOG083559', '07:54:16', '2025-03-11', 1),
(932, 'TH17', 'HBL056746', 'MOG083559', '07:55:13', '2025-03-11', 1),
(933, 'TH17', 'HBL056746', 'MOG083559', '07:55:46', '2025-03-11', 1),
(934, 'TH17', 'HBL056746', 'MOG083559', '07:56:29', '2025-03-11', 1),
(935, 'TH17', 'HBL056746', 'MOG083559', '08:06:30', '2025-03-11', 1),
(936, 'TH17', 'HBL056746', 'MOG083559', '08:12:28', '2025-03-11', 1),
(937, 'TH17', 'HBL056746', 'MOG083559', '08:15:36', '2025-03-11', 1),
(938, 'TH17', 'HBL056746', 'MOG083559', '08:53:07', '2025-03-11', 1),
(939, 'TH17', 'HBL056746', 'MOG083559', '08:55:26', '2025-03-11', 1),
(940, 'TH17', 'HBL056746', 'MOG083559', '08:59:17', '2025-03-11', 1),
(941, 'TH17', 'HBL056746', 'MOG083559', '07:22:34', '2025-03-12', 1),
(942, 'TH17', 'HBL056746', 'MOG083559', '07:30:10', '2025-03-12', 1),
(943, 'TH17', 'HBL056746', 'MOG083559', '07:31:11', '2025-03-12', 1),
(944, 'TH17', 'HBL056746', 'MOG083559', '07:32:10', '2025-03-12', 1),
(945, 'TH17', 'HBL056746', 'MOG083559', '07:32:23', '2025-03-12', 1),
(946, 'TH17', 'HBL056746', 'MOG083559', '07:32:24', '2025-03-12', 1),
(947, 'TH17', 'HBL056746', 'MOG083559', '07:33:02', '2025-03-12', 1),
(948, 'TH17', 'HBL056746', 'MOG083559', '07:33:29', '2025-03-12', 1),
(949, 'TH17', 'HBL056746', 'MOG083559', '07:37:29', '2025-03-12', 1),
(950, 'TH17', 'HBL056746', 'MOG083559', '07:40:21', '2025-03-12', 1),
(951, 'TH17', 'HBL056746', 'MOG083559', '07:44:26', '2025-03-12', 1),
(952, 'TH17', 'HBL056746', 'MOG083559', '08:07:19', '2025-03-12', 1),
(953, 'TH17', 'HBL056746', 'MOG083559', '08:08:08', '2025-03-12', 1),
(954, 'TH17', 'HBL056746', 'MOG083559', '08:11:32', '2025-03-12', 1),
(955, 'TH17', 'HBL056742', 'MOG083550', '08:11:58', '2025-03-12', 1),
(956, 'TH17', 'HBL056746', 'MOG083559', '08:12:57', '2025-03-12', 1),
(957, 'TH17', 'HBL056746', 'MOG083559', '08:23:34', '2025-03-12', 1),
(958, 'TH17', 'HBL056746', 'MOG083559', '08:29:39', '2025-03-12', 1),
(959, 'TH17', 'HBL056746', 'MOG083559', '08:30:30', '2025-03-12', 1),
(960, 'TH17', 'HBL056746', 'MOG083559', '09:12:43', '2025-03-12', 1),
(961, 'TH17', 'HBL056746', 'MOG083559', '09:12:48', '2025-03-12', 1),
(962, 'TH17', 'HBL056746', 'MOG083559', '09:13:30', '2025-03-12', 1),
(963, 'TH17', 'HBL056746', 'MOG083559', '09:16:04', '2025-03-12', 1),
(964, 'TH17', 'HBL056746', 'MOG083559', '11:02:26', '2025-03-12', 1),
(965, 'TH17', 'HBL056746', 'MOG083559', '11:07:01', '2025-03-12', 1),
(966, 'TH17', 'HBL056746', 'MOG083559', '11:13:57', '2025-03-12', 1),
(967, 'TH17', 'HBL056743', 'MOG083551', '11:14:08', '2025-03-12', 1),
(968, 'TH17', 'HBL056740', 'MOG083548', '11:14:17', '2025-03-12', 1),
(969, 'TH17', 'HBL056746', 'MOG083559', '11:16:49', '2025-03-12', 1),
(970, 'TH17', 'HBL056746', 'MOG083559', '11:20:43', '2025-03-12', 1),
(971, 'TH17', 'HBL056746', 'MOG083559', '11:21:50', '2025-03-12', 1),
(972, 'TH17', 'HBL056746', 'MOG083559', '11:23:21', '2025-03-12', 1),
(973, 'TH17', 'HBL056746', 'MOG083559', '11:24:44', '2025-03-12', 1),
(974, 'TH17', 'HBL056746', 'MOG083559', '11:27:17', '2025-03-12', 1),
(975, 'TH17', 'HBL056746', 'MOG083559', '11:30:35', '2025-03-12', 1),
(976, 'TH17', 'HBL056746', 'MOG083559', '12:21:12', '2025-03-12', 1),
(977, 'TH17', 'HBL056746', 'MOG083559', '12:29:43', '2025-03-12', 1),
(978, 'TH17', 'HBL056746', 'MOG083559', '12:31:11', '2025-03-12', 1),
(979, 'TH17', 'HBL056746', 'MOG083559', '12:32:56', '2025-03-12', 1),
(980, 'TH17', 'HBL056746', 'MOG083559', '12:35:01', '2025-03-12', 1),
(981, 'TH17', 'HBL056746', 'MOG083559', '12:38:26', '2025-03-12', 1),
(982, 'TH17', 'HBL056746', 'MOG083559', '12:39:52', '2025-03-12', 1),
(983, 'TH17', 'HBL056746', 'MOG083559', '12:42:06', '2025-03-12', 1),
(984, 'TH17', 'HBL056746', 'MOG083559', '12:44:29', '2025-03-12', 1),
(985, 'TH17', 'HBL056746', 'MOG083559', '13:35:34', '2025-03-12', 1),
(986, 'TH17', 'HBL056746', 'MOG083559', '13:38:53', '2025-03-12', 1),
(987, 'TH17', 'HBL056746', 'MOG083559', '13:49:25', '2025-03-12', 1),
(988, 'TH17', 'HBL056746', 'MOG083559', '13:55:04', '2025-03-12', 1),
(989, 'TH17', 'HBL056746', 'MOG083559', '14:01:46', '2025-03-12', 1),
(990, 'TH17', 'HBL056746', 'MOG083559', '14:02:22', '2025-03-12', 1),
(991, 'TH17', 'HBL056746', 'MOG083559', '14:03:54', '2025-03-12', 1),
(992, 'TH17', 'HBL056746', 'MOG083559', '14:05:00', '2025-03-12', 1),
(993, 'TH17', 'HBL056746', 'MOG083559', '14:06:06', '2025-03-12', 1),
(994, 'TH17', 'HBL056746', 'MOG083559', '14:08:43', '2025-03-12', 1),
(995, 'TH17', 'HBL056746', 'MOG083559', '14:16:26', '2025-03-12', 1),
(996, 'TH17', 'HBL056746', 'MOG083559', '14:17:31', '2025-03-12', 1),
(997, 'TH17', 'HBL056746', 'MOG083559', '14:18:26', '2025-03-12', 1),
(998, 'TH17', 'HBL056746', 'MOG083559', '14:19:02', '2025-03-12', 1),
(999, 'TH17', 'HBL056746', 'MOG083559', '14:20:13', '2025-03-12', 1),
(1000, 'TH17', 'HBL056746', 'MOG083559', '14:22:14', '2025-03-12', 1),
(1001, 'TH17', 'HBL056746', 'MOG083559', '14:23:24', '2025-03-12', 1),
(1002, 'TH17', 'HBL056746', 'MOG083559', '14:24:31', '2025-03-12', 1),
(1003, 'TH17', 'HBL056746', 'MOG083559', '14:25:11', '2025-03-12', 1),
(1004, 'TH17', 'HBL056746', 'MOG083559', '14:26:14', '2025-03-12', 1),
(1005, 'TH17', 'HBL056746', 'MOG083559', '14:27:54', '2025-03-12', 1),
(1006, 'TH17', 'HBL056746', 'MOG083559', '07:49:44', '2025-03-13', 1),
(1007, 'TH17', 'HBL056746', 'MOG083559', '07:55:11', '2025-03-13', 1),
(1008, 'TH17', 'HBL056746', 'MOG083559', '08:42:55', '2025-03-13', 1),
(1009, 'TH17', 'HBL056746', 'MOG083559', '08:47:11', '2025-03-13', 1),
(1010, 'TH17', 'HBL056746', 'MOG083559', '08:48:24', '2025-03-13', 1),
(1011, 'TH17', 'HBL056746', 'MOG083559', '08:49:19', '2025-03-13', 1),
(1013, 'TH17', 'HBL056746', 'MOG083559', '08:53:54', '2025-03-13', 1),
(1016, 'TP08', 'MOG083559', 'MOG083559', '09:00:33', '2025-03-13', 1),
(1017, 'TH17', 'HBL056746', 'MOG083559', '09:01:48', '2025-03-13', 1),
(1018, 'TH17', 'HBL056746', 'MOG083559', '09:09:14', '2025-03-13', 1),
(1019, 'TH17', 'HBL056746', 'MOG083559', '09:10:53', '2025-03-13', 1),
(1020, 'TH17', 'HBL056746', 'MOG083559', '09:15:07', '2025-03-13', 1),
(1021, 'TH17', 'HBL056746', 'MOG083559', '09:16:19', '2025-03-13', 1),
(1022, 'TH17', 'HBL056746', 'MOG083559', '09:16:57', '2025-03-13', 1),
(1023, 'TH17', 'HBL056746', 'MOG083559', '09:18:12', '2025-03-13', 1),
(1024, 'TH17', 'HBL056746', 'MOG083559', '09:19:43', '2025-03-13', 1),
(1025, 'TH17', 'HBL056746', 'MOG083559', '09:21:20', '2025-03-13', 1),
(1026, 'TH17', 'HBL056746', 'MOG083559', '10:15:38', '2025-03-13', 1),
(1027, 'TH17', 'HBL056746', 'MOG083559', '10:17:33', '2025-03-13', 1),
(1028, 'TH17', 'HBL056746', 'MOG083559', '10:18:04', '2025-03-13', 1),
(1029, 'TH17', 'HBL056746', 'MOG083559', '10:18:24', '2025-03-13', 1),
(1030, 'TP08', 'MOG083559', 'MOG083559', '10:27:27', '2025-03-13', 1),
(1031, 'TH17', 'HBL056746', 'MOG083559', '10:45:42', '2025-03-13', 1),
(1032, 'TH17', 'HBL056744', 'MOG083557', '10:47:29', '2025-03-13', 1),
(1033, 'TH17', 'HBL056744', 'MOG083557', '10:50:47', '2025-03-13', 1),
(1034, 'TH17', 'HBL056746', 'MOG083559', '10:52:49', '2025-03-13', 1),
(1035, 'TH17', 'HBL056746', 'MOG083559', '11:32:16', '2025-03-13', 1),
(1036, 'TH17', 'HBL056746', 'MOG083559', '12:25:26', '2025-03-13', 1),
(1037, 'TH17', 'HBL056746', 'MOG083559', '12:33:10', '2025-03-13', 1),
(1038, 'TH17', 'HBL056746', 'MOG083559', '12:34:00', '2025-03-13', 1),
(1039, 'TH17', 'HBL056746', 'MOG083559', '14:17:28', '2025-03-13', 1),
(1040, 'TH17', 'HBL056746', 'MOG083559', '14:20:38', '2025-03-13', 1),
(1041, 'TH17', 'HBL056746', 'MOG083559', '15:06:00', '2025-03-13', 1),
(1042, 'TH17', 'HBL056746', 'MOG083559', '15:17:29', '2025-03-13', 1),
(1043, 'TH17', 'HBL056746', 'MOG083559', '15:37:44', '2025-03-13', 1),
(1044, 'TH17', 'HBL056746', 'MOG083559', '15:40:57', '2025-03-13', 1),
(1045, 'TH17', 'HBL056746', 'MOG083559', '15:44:28', '2025-03-13', 1),
(1046, 'TH17', 'HBL056746', 'MOG083559', '15:46:35', '2025-03-13', 1),
(1047, 'TH17', 'HBL056746', 'MOG083559', '15:52:36', '2025-03-13', 1),
(1048, 'TH17', 'HBL056746', 'MOG083559', '07:25:41', '2025-03-14', 1),
(1049, 'TH17', 'HBL056746', 'MOG083559', '07:29:34', '2025-03-14', 1),
(1050, 'TH17', 'HBL056746', 'MOG083559', '07:31:27', '2025-03-14', 1),
(1051, 'TH17', 'HBL056746', 'MOG083559', '07:33:53', '2025-03-14', 1),
(1052, 'TH17', 'HBL056746', 'MOG083559', '07:35:53', '2025-03-14', 1),
(1053, 'TH17', 'HBL056746', 'MOG083559', '07:37:31', '2025-03-14', 1),
(1054, 'TH17', 'HBL056746', 'MOG083559', '08:59:10', '2025-03-14', 1),
(1055, 'TH17', 'HBL056746', 'MOG083559', '09:01:24', '2025-03-14', 1),
(1056, 'TH17', 'HBL056746', 'MOG083559', '09:06:44', '2025-03-14', 1),
(1057, 'TH17', 'HBL056746', 'MOG083559', '09:11:06', '2025-03-14', 1),
(1058, 'TH17', 'HBL056746', 'MOG083559', '09:16:13', '2025-03-14', 1),
(1059, 'TH17', 'HBL056746', 'MOG083559', '09:21:58', '2025-03-14', 1),
(1060, 'TH17', 'HBL056746', 'MOG083559', '09:23:39', '2025-03-14', 1),
(1061, 'TH17', 'HBL056746', 'MOG083559', '10:05:30', '2025-03-14', 1),
(1062, 'TH17', 'HBL056746', 'MOG083559', '10:10:31', '2025-03-14', 1),
(1063, 'TH17', 'HBL056746', 'MOG083559', '10:15:23', '2025-03-14', 1),
(1064, 'TH17', 'HBL056746', 'MOG083559', '10:16:57', '2025-03-14', 1),
(1065, 'TH17', 'HBL056746', 'MOG083559', '10:18:54', '2025-03-14', 1),
(1066, 'TH17', 'HBL056746', 'MOG083559', '10:23:38', '2025-03-14', 1),
(1067, 'TH17', 'HBL056746', 'MOG083559', '10:25:45', '2025-03-14', 1),
(1068, 'TH17', 'HBL056746', 'MOG083559', '10:26:43', '2025-03-14', 1),
(1069, 'TH17', 'HBL056746', 'MOG083559', '10:28:17', '2025-03-14', 1),
(1070, 'TH17', 'HBL056746', 'MOG083559', '10:30:15', '2025-03-14', 1),
(1071, 'TH17', 'HBL056746', 'MOG083559', '10:36:40', '2025-03-14', 1),
(1072, 'TH17', 'HBL056746', 'MOG083559', '10:42:21', '2025-03-14', 1),
(1073, 'TH17', 'HBL056746', 'MOG083559', '10:47:31', '2025-03-14', 1),
(1074, 'TH17', 'HBL056746', 'MOG083559', '10:48:56', '2025-03-14', 1),
(1075, 'TH17', 'HBL056746', 'MOG083559', '10:51:35', '2025-03-14', 1),
(1076, 'TH17', 'HBL056746', 'MOG083559', '10:54:08', '2025-03-14', 1),
(1077, 'TH17', 'HBL056746', 'MOG083559', '10:55:36', '2025-03-14', 1),
(1078, 'TH17', 'HBL056746', 'MOG083559', '11:01:54', '2025-03-14', 1),
(1079, 'TH17', 'HBL056746', 'MOG083559', '11:03:09', '2025-03-14', 1),
(1080, 'TH17', 'HBL056746', 'MOG083559', '11:04:24', '2025-03-14', 1),
(1081, 'TH17', 'HBL056746', 'MOG083559', '11:05:13', '2025-03-14', 1),
(1082, 'TH17', 'HBL056746', 'MOG083559', '11:08:41', '2025-03-14', 1),
(1083, 'TH17', 'HBL056746', 'MOG083559', '11:09:34', '2025-03-14', 1),
(1084, 'TH17', 'HBL056746', 'MOG083559', '11:12:33', '2025-03-14', 1),
(1085, 'TH17', 'HBL056746', 'MOG083559', '11:13:22', '2025-03-14', 1),
(1086, 'TH17', 'HBL056746', 'MOG083559', '11:30:00', '2025-03-14', 1),
(1087, 'TH17', 'HBL056746', 'MOG083559', '11:36:59', '2025-03-14', 1),
(1088, 'TH17', 'HBL056746', 'MOG083559', '11:38:33', '2025-03-14', 1),
(1089, 'TH17', 'HBL056746', 'MOG083559', '11:39:52', '2025-03-14', 1),
(1090, 'TH17', 'HBL056746', 'MOG083559', '11:45:59', '2025-03-14', 1),
(1091, 'TH17', 'HBL056746', 'MOG083559', '11:48:14', '2025-03-14', 1),
(1092, 'TH17', 'HBL056746', 'MOG083559', '11:49:31', '2025-03-14', 1),
(1093, 'TH17', 'HBL056746', 'MOG083559', '11:50:28', '2025-03-14', 1),
(1094, 'TH17', 'HBL056746', 'MOG083559', '11:54:24', '2025-03-14', 1),
(1095, 'TH17', 'HBL056746', 'MOG083559', '11:56:38', '2025-03-14', 1),
(1096, 'TH17', 'HBL056746', 'MOG083559', '11:58:46', '2025-03-14', 1),
(1097, 'TH17', 'HBL056746', 'MOG083559', '12:00:30', '2025-03-14', 1),
(1098, 'TH17', 'HBL056746', 'MOG083559', '12:18:02', '2025-03-14', 1),
(1099, 'TH17', 'HBL056746', 'MOG083559', '12:26:52', '2025-03-14', 1),
(1100, 'TH17', 'HBL056746', 'MOG083559', '12:29:46', '2025-03-14', 1),
(1101, 'TH17', 'HBL056746', 'MOG083559', '12:31:25', '2025-03-14', 1),
(1102, 'TH17', 'HBL056746', 'MOG083559', '12:34:35', '2025-03-14', 1),
(1103, 'TH17', 'HBL056746', 'MOG083559', '12:41:04', '2025-03-14', 1),
(1104, 'TH17', 'HBL056746', 'MOG083559', '12:45:02', '2025-03-14', 1),
(1105, 'TH17', 'HBL056746', 'MOG083559', '12:47:06', '2025-03-14', 1),
(1106, 'TH17', 'HBL056746', 'MOG083559', '13:38:25', '2025-03-14', 1),
(1107, 'TH17', 'HBL056746', 'MOG083559', '14:24:00', '2025-03-14', 1),
(1108, 'TH17', 'HBL056746', 'MOG083559', '14:25:56', '2025-03-14', 1),
(1109, 'TH17', 'HBL056746', 'MOG083559', '14:26:50', '2025-03-14', 1),
(1110, 'TH17', 'HBL056746', 'MOG083559', '14:31:23', '2025-03-14', 1),
(1111, 'TH17', 'HBL056746', 'MOG083559', '14:32:29', '2025-03-14', 1),
(1112, 'TH17', 'HBL056746', 'MOG083559', '07:09:40', '2025-03-18', 1),
(1113, 'TH17', 'HBL056746', 'MOG083559', '07:23:21', '2025-03-18', 1),
(1114, 'TH17', 'HBL056746', 'MOG083559', '07:27:00', '2025-03-18', 1),
(1115, 'TH17', 'HBL056746', 'MOG083559', '07:29:28', '2025-03-18', 1),
(1116, 'TH17', 'HBL056746', 'MOG083559', '07:30:44', '2025-03-18', 1),
(1117, 'TH17', 'HBL056746', 'MOG083559', '07:32:57', '2025-03-18', 1),
(1118, 'TH17', 'HBL056746', 'MOG083559', '07:38:45', '2025-03-18', 1),
(1119, 'TH17', 'HBL056746', 'MOG083559', '07:54:31', '2025-03-18', 1),
(1120, 'TP08', 'MOG083559', 'MOG083559', '08:19:04', '2025-03-18', 1),
(1121, 'TP08', 'MOG083559', 'MOG083559', '08:21:41', '2025-03-18', 1),
(1122, 'TP08', 'MOG083559', 'MOG083559', '08:23:16', '2025-03-18', 1),
(1123, 'TP08', 'MOG083559', 'MOG083559', '08:24:59', '2025-03-18', 1),
(1124, 'TH17', 'HBL056746', 'MOG083559', '08:26:14', '2025-03-18', 1),
(1125, 'TP08', 'MOG083559', 'MOG083559', '08:28:22', '2025-03-18', 1),
(1126, 'TP08', 'MOG083559', 'MOG083559', '08:30:17', '2025-03-18', 1),
(1127, 'TP08', 'MOG083559', 'MOG083559', '08:41:13', '2025-03-18', 1),
(1128, 'TP08', 'MOG083559', 'MOG083559', '08:42:17', '2025-03-18', 1),
(1129, 'TP08', 'MOG083559', 'MOG083559', '08:43:57', '2025-03-18', 1),
(1130, 'TP08', 'MOG083559', 'MOG083559', '08:46:51', '2025-03-18', 1),
(1131, 'TP08', 'MOG083559', 'MOG083559', '08:48:07', '2025-03-18', 1),
(1132, 'TP08', 'MOG083559', 'MOG083559', '08:50:50', '2025-03-18', 1),
(1133, 'TP08', 'MOG083559', 'MOG083559', '08:52:12', '2025-03-18', 1),
(1134, 'TH17', 'HBL056746', 'MOG083559', '08:57:24', '2025-03-18', 1),
(1135, 'TH17', 'HBL056746', 'MOG083559', '09:00:21', '2025-03-18', 1),
(1136, 'TP08', 'MOG083559', 'MOG083559', '09:01:06', '2025-03-18', 1),
(1137, 'TP08', 'MOG083559', 'MOG083559', '12:14:02', '2025-03-18', 1),
(1138, 'TH17', 'HBL056746', 'MOG083559', '12:15:18', '2025-03-18', 1),
(1139, 'TP08', 'MOG083559', 'MOG083559', '12:16:11', '2025-03-18', 1),
(1140, 'TP08', 'MOG083559', 'MOG083559', '12:18:23', '2025-03-18', 1),
(1141, 'TP08', 'MOG083559', 'MOG083559', '12:20:03', '2025-03-18', 1),
(1142, 'TP08', 'MOG083559', 'MOG083559', '12:22:23', '2025-03-18', 1),
(1143, 'TH17', 'HBL056746', 'MOG083559', '12:27:27', '2025-03-18', 1),
(1144, 'TH17', 'HBL056746', 'MOG083559', '12:54:27', '2025-03-18', 1),
(1145, 'TH17', 'HBL056746', 'MOG083559', '12:59:08', '2025-03-18', 1),
(1146, 'TP08', 'MOG083559', 'MOG083559', '12:59:50', '2025-03-18', 1),
(1147, 'TH17', 'HBL056746', 'MOG083559', '13:02:24', '2025-03-18', 1),
(1148, 'TP08', 'MOG083559', 'MOG083559', '13:04:02', '2025-03-18', 1),
(1149, 'TH17', 'HBL056746', 'MOG083559', '13:06:22', '2025-03-18', 1),
(1150, 'TP08', 'MOG083559', 'MOG083559', '13:07:24', '2025-03-18', 1),
(1151, 'TH17', 'HBL056746', 'MOG083559', '13:10:33', '2025-03-18', 1),
(1152, 'TH17', 'HBL056746', 'MOG083559', '13:12:08', '2025-03-18', 1),
(1153, 'TP08', 'MOG083559', 'MOG083559', '13:28:37', '2025-03-18', 1),
(1154, 'TP08', 'MOG083559', 'MOG083559', '13:29:53', '2025-03-18', 1),
(1155, 'TP08', 'MOG083559', 'MOG083559', '13:31:55', '2025-03-18', 1),
(1156, 'TH17', 'HBL056746', 'MOG083559', '13:34:08', '2025-03-18', 1),
(1157, 'TH17', 'HBL056746', 'MOG083559', '13:34:56', '2025-03-18', 1),
(1158, 'TP08', 'MOG083559', 'MOG083559', '13:46:50', '2025-03-18', 1),
(1159, 'TP08', 'MOG083559', 'MOG083559', '13:47:14', '2025-03-18', 1),
(1160, 'TP08', 'MOG083559', 'MOG083559', '13:49:48', '2025-03-18', 1),
(1161, 'TP08', 'MOG083559', 'MOG083559', '13:53:28', '2025-03-18', 1),
(1162, 'TP08', 'MOG083559', 'MOG083559', '13:59:06', '2025-03-18', 1),
(1163, 'TP08', 'MOG083559', 'MOG083559', '14:01:18', '2025-03-18', 1),
(1164, 'TP08', 'MOG083559', 'MOG083559', '14:03:10', '2025-03-18', 1),
(1165, 'TP08', 'MOG083559', 'MOG083559', '14:06:11', '2025-03-18', 1),
(1166, 'TP08', 'MOG083559', 'MOG083559', '14:11:23', '2025-03-18', 1),
(1167, 'TP08', 'MOG083559', 'MOG083559', '14:11:44', '2025-03-18', 1),
(1168, 'TP08', 'MOG083559', 'MOG083559', '14:13:15', '2025-03-18', 1),
(1169, 'TP08', 'MOG083559', 'MOG083559', '14:19:23', '2025-03-18', 1),
(1170, 'TP08', 'MOG083559', 'MOG083559', '14:21:27', '2025-03-18', 1),
(1171, 'TH17', 'HBL056746', 'MOG083559', '14:28:59', '2025-03-18', 1),
(1172, 'TH17', 'HBL056746', 'MOG083559', '15:04:38', '2025-03-18', 1),
(1173, 'TP08', 'MOG083559', 'MOG083559', '15:08:24', '2025-03-18', 1),
(1174, 'TP08', 'MOG083559', 'MOG083559', '15:09:11', '2025-03-18', 1),
(1175, 'TH17', 'HBL056746', 'MOG083559', '15:10:20', '2025-03-18', 1),
(1176, 'TP08', 'MOG083559', 'MOG083559', '15:11:17', '2025-03-18', 1),
(1177, 'TH17', 'HBL056746', 'MOG083559', '15:21:22', '2025-03-18', 1),
(1178, 'TH17', 'HBL056746', 'MOG083559', '15:22:07', '2025-03-18', 1),
(1179, 'TH17', 'HBL056746', 'MOG083559', '15:24:03', '2025-03-18', 1),
(1180, 'TH17', 'HBL056746', 'MOG083559', '15:25:07', '2025-03-18', 1),
(1181, 'TH17', 'HBL056746', 'MOG083559', '15:26:32', '2025-03-18', 1),
(1182, 'TH17', 'HBL056746', 'MOG083559', '15:27:42', '2025-03-18', 1),
(1183, 'TH17', 'HBL056746', 'MOG083559', '15:28:59', '2025-03-18', 1),
(1184, 'TP08', 'MOG083559', 'MOG083559', '07:12:42', '2025-03-19', 1),
(1185, 'TP08', 'MOG083559', 'MOG083559', '07:15:51', '2025-03-19', 1),
(1186, 'TP08', 'MOG083559', 'MOG083559', '07:17:34', '2025-03-19', 1),
(1187, 'TP08', 'MOG083559', 'MOG083559', '07:23:33', '2025-03-19', 1),
(1188, 'TP08', 'MOG083559', 'MOG083559', '07:26:03', '2025-03-19', 1),
(1189, 'TP08', 'MOG083559', 'MOG083559', '07:28:41', '2025-03-19', 1),
(1190, 'TP08', 'MOG083559', 'MOG083559', '07:29:49', '2025-03-19', 1),
(1191, 'TP08', 'MOG083559', 'MOG083559', '07:30:55', '2025-03-19', 1),
(1192, 'TP08', 'MOG083559', 'MOG083559', '07:37:02', '2025-03-19', 1),
(1193, 'TP08', 'MOG083559', 'MOG083559', '07:39:47', '2025-03-19', 1),
(1194, 'TP08', 'MOG083559', 'MOG083559', '07:45:14', '2025-03-19', 1),
(1195, 'TP08', 'MOG083559', 'MOG083559', '07:47:36', '2025-03-19', 1),
(1196, 'TH17', 'HBL056746', 'MOG083559', '07:48:23', '2025-03-19', 1),
(1197, 'TH17', 'HBL056746', 'MOG083559', '07:50:06', '2025-03-19', 1),
(1198, 'TP08', 'MOG083559', 'MOG083559', '07:50:17', '2025-03-19', 1),
(1199, 'TP08', 'MOG083559', 'MOG083559', '07:53:34', '2025-03-19', 1),
(1200, 'TP08', 'MOG083559', 'MOG083559', '07:54:31', '2025-03-19', 1),
(1201, 'TH17', 'HBL056746', 'MOG083559', '07:54:46', '2025-03-19', 1),
(1202, 'TP08', 'MOG083376', 'MOG083376', '07:56:27', '2025-03-19', 1),
(1203, 'TH17', 'HBL056746', 'MOG083559', '07:57:15', '2025-03-19', 1),
(1204, 'TH17', 'HBL056746', 'MOG083559', '08:09:17', '2025-03-19', 1),
(1205, 'TP08', 'MOG083912', 'MOG083912', '08:11:00', '2025-03-19', 1),
(1206, 'TP08', 'MOG083559', 'MOG083559', '08:12:18', '2025-03-19', 1),
(1207, 'TH17', 'HBL056746', 'MOG083559', '08:12:59', '2025-03-19', 1),
(1208, 'tp08', 'MOG001622', 'MOG001622', '08:37:47', '2025-03-19', 1),
(1209, 'TP08', 'MOG001622', 'MOG001622', '08:39:42', '2025-03-19', 1),
(1210, 'TH17', 'HBL056746', 'MOG083559', '09:13:02', '2025-03-19', 1),
(1211, 'TH17', 'HBL056746', 'MOG083559', '09:16:11', '2025-03-19', 1),
(1212, 'TH17', 'HBL056746', 'MOG083559', '09:18:20', '2025-03-19', 1),
(1213, 'TH17', 'HBL056746', 'MOG083559', '09:22:25', '2025-03-19', 1),
(1214, 'TH17', 'HBL056746', 'MOG083559', '09:27:20', '2025-03-19', 1),
(1215, 'TP08', 'MOG083559', 'MOG083559', '09:46:28', '2025-03-19', 1),
(1216, 'TP08', 'MOG083559', 'MOG083559', '09:55:47', '2025-03-19', 1),
(1217, 'TP08', 'MOG083559', 'MOG083559', '09:56:49', '2025-03-19', 1),
(1218, 'TH17', 'HBL056746', 'MOG083559', '10:00:48', '2025-03-19', 1),
(1219, 'TH17', 'HBL056746', 'MOG083559', '10:17:11', '2025-03-19', 1),
(1220, 'TP08', 'MOG083559', 'MOG083559', '10:36:53', '2025-03-19', 1),
(1221, 'TH17', 'HBL056746', 'MOG083559', '12:00:05', '2025-03-19', 1),
(1222, 'TP08', 'MOG083559', 'MOG083559', '12:20:36', '2025-03-19', 1),
(1223, 'TP08', 'MOG083559', 'MOG083559', '12:23:59', '2025-03-19', 1),
(1224, 'TP08', 'MOG083559', 'MOG083559', '12:26:48', '2025-03-19', 1),
(1225, 'TP08', 'MOG083559', 'MOG083559', '12:32:51', '2025-03-19', 1),
(1226, 'TH17', 'HBL056746', 'MOG083559', '14:06:01', '2025-03-19', 1),
(1227, 'TH17', 'HBL056746', 'MOG083559', '14:07:37', '2025-03-19', 1),
(1228, 'TP08', 'MOG083559', 'MOG083559', '14:08:01', '2025-03-19', 1),
(1229, 'TH17', 'HBL056746', 'MOG083559', '14:08:56', '2025-03-19', 1),
(1230, 'TP08', 'MOG083559', 'MOG083559', '14:10:51', '2025-03-19', 1),
(1231, 'TH17', 'HBL056746', 'MOG083559', '14:11:02', '2025-03-19', 1),
(1232, 'TP08', 'MOG083559', 'MOG083559', '14:12:27', '2025-03-19', 1),
(1233, 'TP08', 'MOG083559', 'MOG083559', '14:14:58', '2025-03-19', 1),
(1234, 'TP08', 'MOG083559', 'MOG083559', '14:21:37', '2025-03-19', 1),
(1235, 'TP08', 'MOG083559', 'MOG083559', '14:24:42', '2025-03-19', 1),
(1236, 'TP08', 'MOG083559', 'MOG083559', '14:26:19', '2025-03-19', 1),
(1237, 'TP08', 'MOG083559', 'MOG083559', '14:29:39', '2025-03-19', 1),
(1238, 'TP08', 'MOG083559', 'MOG083559', '14:39:08', '2025-03-19', 1),
(1239, 'TP08', 'MOG083559', 'MOG083559', '14:43:29', '2025-03-19', 1),
(1240, 'TP08', 'MOG083559', 'MOG083559', '14:45:14', '2025-03-19', 1),
(1241, 'TP08', 'MOG083559', 'MOG083559', '14:47:57', '2025-03-19', 1),
(1242, 'TP08', 'MOG083559', 'MOG083559', '14:56:34', '2025-03-19', 1),
(1243, 'TH17', 'HBL056746', 'MOG083559', '16:01:02', '2025-03-19', 1),
(1244, 'TH17', 'HBL056790', 'MOG083620', '07:20:10', '2025-03-20', 1),
(1245, 'TH17', 'HBL054879', 'MOG080708', '07:26:38', '2025-03-20', 1),
(1246, 'TP08', 'MOG083559', 'MOG083559', '07:39:06', '2025-03-20', 1),
(1247, 'TP08', 'MOG083559', 'MOG083559', '07:40:43', '2025-03-20', 1),
(1248, 'TH17', 'HBL045789', 'MOG066270', '07:43:27', '2025-03-20', 1),
(1249, 'TP08', 'MOG083559', 'MOG083559', '07:47:37', '2025-03-20', 1),
(1250, 'TP08', 'MOG083559', 'MOG083559', '07:54:46', '2025-03-20', 1),
(1251, 'TP08', 'MOG083559', 'MOG083559', '07:55:51', '2025-03-20', 1),
(1252, 'TP08', 'MOG083559', 'MOG083559', '07:57:56', '2025-03-20', 1),
(1253, 'TH17', 'HBL056746', 'MOG083559', '07:58:55', '2025-03-20', 1),
(1254, 'TP08', 'MOG083559', 'MOG083559', '08:02:26', '2025-03-20', 1),
(1255, 'TP08', 'MOG083559', 'MOG083559', '08:04:15', '2025-03-20', 1),
(1256, 'TP08', 'MOG083559', 'MOG083559', '08:06:20', '2025-03-20', 1),
(1257, 'TP08', 'MOG083559', 'MOG083559', '08:07:54', '2025-03-20', 1),
(1258, 'TH17', 'HBL056746', 'MOG083559', '08:27:02', '2025-03-20', 1),
(1259, 'TP08', 'MOG083559', 'MOG083559', '09:10:44', '2025-03-20', 1),
(1260, 'TP08', 'MOG083559', 'MOG083559', '10:29:29', '2025-03-20', 1),
(1261, 'TH17', 'HBL056746', 'MOG083559', '10:33:43', '2025-03-20', 1),
(1262, 'TP08', 'MOG083559', 'MOG083559', '11:13:13', '2025-03-20', 1),
(1263, 'TH17', 'HBL056746', 'MOG083559', '11:13:55', '2025-03-20', 1),
(1264, 'TH17', 'HBL056746', 'MOG083559', '11:19:13', '2025-03-20', 1),
(1265, 'TH17', 'HBL056746', 'MOG083559', '11:23:47', '2025-03-20', 1),
(1266, 'TP08', 'MOG083559', 'MOG083559', '11:26:55', '2025-03-20', 1),
(1267, 'TP08', 'MOG083559', 'MOG083559', '11:27:49', '2025-03-20', 1),
(1268, 'TP08', 'MOG083559', 'MOG083559', '11:32:33', '2025-03-20', 1),
(1269, 'TP08', 'MOG083559', 'MOG083559', '11:36:03', '2025-03-20', 1),
(1270, 'TP08', 'MOG083559', 'MOG083559', '11:36:17', '2025-03-20', 1),
(1271, 'TH17', 'HBL056746', 'MOG083559', '11:44:09', '2025-03-20', 1),
(1272, 'TP08', 'MOG083559', 'MOG083559', '12:12:03', '2025-03-20', 1),
(1273, 'TP08', 'MOG083559', 'MOG083559', '12:12:21', '2025-03-20', 1),
(1274, 'TP08', 'MOG083559', 'MOG083559', '12:15:36', '2025-03-20', 1),
(1275, 'TP08', 'MOG083559', 'MOG083559', '12:16:52', '2025-03-20', 1),
(1276, 'TH17', 'HBL056746', 'MOG083559', '12:17:29', '2025-03-20', 1),
(1277, 'TP08', 'MOG083559', 'MOG083559', '12:19:01', '2025-03-20', 1),
(1278, 'TH17', 'HBL056746', 'MOG083559', '12:23:29', '2025-03-20', 1),
(1279, 'TP08', 'MOG083559', 'MOG083559', '12:36:34', '2025-03-20', 1),
(1280, 'TP08', 'MOG083559', 'MOG083559', '12:38:12', '2025-03-20', 1),
(1281, 'TP08', 'MOG083559', 'MOG083559', '12:39:50', '2025-03-20', 1),
(1282, 'TP08', 'MOG083559', 'MOG083559', '12:40:55', '2025-03-20', 1),
(1283, 'TP08', 'MOG083559', 'MOG083559', '12:44:39', '2025-03-20', 1),
(1284, 'TP08', 'MOG083559', 'MOG083559', '12:46:14', '2025-03-20', 1),
(1285, 'TP08', 'MOG083559', 'MOG083559', '12:48:14', '2025-03-20', 1),
(1286, 'TP08', 'MOG083559', 'MOG083559', '13:49:46', '2025-03-20', 1),
(1287, 'TP08', 'MOG083559', 'MOG083559', '13:56:55', '2025-03-20', 1),
(1288, 'TP08', 'MOG083559', 'MOG083559', '14:01:10', '2025-03-20', 1),
(1289, 'TH17', 'HBL056746', 'MOG083559', '14:17:44', '2025-03-20', 1),
(1290, 'TP08', 'MOG083559', 'MOG083559', '14:20:06', '2025-03-20', 1),
(1291, 'TP08', 'MOG083559', 'MOG083559', '14:20:27', '2025-03-20', 1),
(1292, 'TH17', 'HBL056746', 'MOG083559', '14:21:01', '2025-03-20', 1),
(1293, 'TP08', 'MOG083559', 'MOG083559', '14:21:03', '2025-03-20', 1),
(1294, 'TH17', 'HBL056746', 'MOG083559', '14:22:16', '2025-03-20', 1),
(1295, 'TP08', 'MOG083559', 'MOG083559', '14:23:00', '2025-03-20', 1),
(1296, 'TP08', 'MOG083559', 'MOG083559', '14:23:15', '2025-03-20', 1),
(1297, 'TP08', 'MOG083559', 'MOG083559', '14:28:18', '2025-03-20', 1),
(1298, 'TH17', 'HBL056746', 'MOG083559', '14:30:11', '2025-03-20', 1),
(1299, 'TP08', 'MOG083559', 'MOG083559', '14:36:30', '2025-03-20', 1),
(1300, 'TH17', 'HBL056746', 'MOG083559', '14:37:24', '2025-03-20', 1),
(1301, 'TP08', 'MOG083559', 'MOG083559', '14:39:32', '2025-03-20', 1),
(1302, 'TP08', 'MOG083546', 'MOG083546', '14:50:30', '2025-03-20', 1),
(1303, 'TH17', 'HBL056746', 'MOG083559', '14:53:10', '2025-03-20', 1),
(1304, 'TH17', 'HBL056746', 'MOG083559', '14:57:49', '2025-03-20', 1),
(1305, 'TP08', 'MOG083547', 'MOG083547', '14:58:13', '2025-03-20', 1),
(1306, 'TP08', 'MOG083559', 'MOG083559', '15:00:49', '2025-03-20', 1),
(1307, 'TP08', 'MOG083559', 'MOG083559', '15:01:09', '2025-03-20', 1),
(1308, 'TP08', 'MOG083559', 'MOG083559', '15:01:21', '2025-03-20', 1),
(1309, 'TP08', 'MOG083559', 'MOG083559', '15:01:28', '2025-03-20', 1),
(1310, 'TP08', 'MOG083559', 'MOG083559', '15:01:40', '2025-03-20', 1),
(1311, 'TH17', 'HBL056746', 'MOG083559', '15:05:30', '2025-03-20', 1),
(1312, 'TH17', 'HBL056746', 'MOG083559', '15:12:42', '2025-03-20', 1),
(1313, 'TH17', 'HBL056746', 'MOG083559', '15:26:23', '2025-03-20', 1),
(1314, 'TH17', 'HBL056746', 'MOG083559', '15:31:30', '2025-03-20', 1),
(1315, 'TH17', 'HBL056746', 'MOG083559', '15:32:32', '2025-03-20', 1),
(1316, 'TH17', 'HBL056746', 'MOG083559', '15:42:39', '2025-03-20', 1),
(1317, 'TH17', 'HBL056746', 'MOG083559', '15:44:14', '2025-03-20', 1),
(1318, 'TH17', 'HBL056746', 'MOG083559', '15:48:57', '2025-03-20', 1),
(1319, 'TH17', 'HBL056746', 'MOG083559', '15:53:08', '2025-03-20', 1),
(1320, 'TH17', 'HBL056746', 'MOG083559', '15:57:56', '2025-03-20', 1),
(1321, 'TH17', 'HBL056746', 'MOG083559', '16:00:12', '2025-03-20', 1),
(1322, 'TH17', 'HBL056746', 'MOG083559', '16:00:12', '2025-03-20', 1),
(1323, 'TH17', 'HBL056746', 'MOG083559', '16:07:42', '2025-03-20', 1),
(1324, 'TH17', 'HBL056746', 'MOG083559', '16:09:40', '2025-03-20', 1),
(1325, 'TH17', 'HBL056746', 'MOG083559', '16:10:47', '2025-03-20', 1),
(1326, 'TH17', 'HBL056746', 'MOG083559', '07:09:28', '2025-03-21', 1),
(1327, 'TH17', 'HBL056746', 'MOG083559', '07:39:12', '2025-03-21', 1),
(1328, 'TH17', 'HBL056746', 'MOG083559', '08:03:36', '2025-03-21', 1),
(1329, 'TH17', 'HBL056746', 'MOG083559', '08:20:01', '2025-03-21', 1),
(1330, 'TH17', 'HBL056746', 'MOG083559', '08:26:59', '2025-03-21', 1),
(1331, 'TH17', 'HBL056746', 'MOG083559', '08:32:22', '2025-03-21', 1),
(1332, 'TH17', 'HBL056746', 'MOG083559', '10:13:05', '2025-03-21', 1),
(1333, 'TH17', 'HBL056746', 'MOG083559', '12:10:34', '2025-03-21', 1),
(1334, 'TH17', 'HBL056746', 'MOG083559', '12:17:18', '2025-03-21', 1),
(1335, 'TH17', 'HBL056746', 'MOG083559', '12:24:11', '2025-03-21', 1),
(1336, 'TH17', 'HBL056746', 'MOG083559', '12:27:58', '2025-03-21', 1),
(1337, 'TH17', 'HBL056746', 'MOG083559', '12:33:08', '2025-03-21', 1),
(1338, 'TH17', 'HBL056746', 'MOG083559', '12:33:08', '2025-03-21', 1),
(1339, 'TH17', 'HBL056746', 'MOG083559', '12:35:04', '2025-03-21', 1),
(1340, 'TH17', 'HBL056746', 'MOG083559', '12:37:06', '2025-03-21', 1),
(1341, 'TH17', 'HBL056746', 'MOG083559', '12:41:43', '2025-03-21', 1),
(1342, 'TH17', 'HBL056746', 'MOG083559', '13:59:36', '2025-03-21', 1),
(1343, 'TH17', 'HBL056746', 'MOG083559', '14:17:29', '2025-03-21', 1),
(1344, 'TH17', 'HBL056746', 'MOG083559', '15:02:48', '2025-03-21', 1),
(1345, 'TH17', 'HBL056746', 'MOG083559', '15:07:59', '2025-03-21', 1),
(1346, 'TH17', 'HBL056746', 'MOG083559', '15:12:57', '2025-03-21', 1),
(1347, 'TH17', 'HBL056746', 'MOG083559', '15:15:01', '2025-03-21', 1),
(1348, 'TH17', 'HBL056746', 'MOG083559', '15:18:16', '2025-03-21', 1),
(1349, 'TH17', 'HBL056746', 'MOG083559', '15:19:54', '2025-03-21', 1),
(1350, 'TH17', 'HBL056746', 'MOG083559', '15:22:20', '2025-03-21', 1),
(1351, 'TH17', 'HBL056746', 'MOG083559', '15:24:42', '2025-03-21', 1),
(1352, 'TH17', 'HBL056746', 'MOG083559', '15:30:23', '2025-03-21', 1),
(1353, 'TH17', 'HBL056746', 'MOG083559', '15:32:42', '2025-03-21', 1),
(1354, 'TH17', 'HBL056746', 'MOG083559', '15:38:31', '2025-03-21', 1),
(1355, 'TH17', 'HBL056746', 'MOG083559', '15:39:31', '2025-03-21', 1),
(1356, 'TH17', 'HBL056746', 'MOG083559', '16:15:54', '2025-03-21', 1),
(1357, 'TH17', 'HBL056746', 'MOG083559', '16:17:44', '2025-03-21', 1),
(1358, 'TH17', 'HBL056746', 'MOG083559', '16:20:33', '2025-03-21', 1),
(1359, 'TH17', 'HBL056746', 'MOG083559', '07:12:43', '2025-03-24', 1),
(1360, 'TP08', 'MOG083542', 'MOG083542', '07:37:09', '2025-03-24', 1),
(1361, 'TP08', 'MOG083407', 'MOG083407', '07:39:43', '2025-03-24', 1),
(1362, 'TP08', 'MOG083559', 'MOG083559', '07:40:20', '2025-03-24', 1),
(1363, 'TP08', 'MOG083569', 'MOG083569', '07:40:33', '2025-03-24', 1),
(1364, 'TP08', 'MOG083518', 'MOG083518', '07:41:13', '2025-03-24', 1),
(1365, 'TP08', 'MOG083561', 'MOG083561', '07:42:00', '2025-03-24', 1),
(1366, 'TP08', 'MOG083559', 'MOG083559', '07:42:31', '2025-03-24', 1),
(1367, 'TP08', 'MOG083559', 'MOG083559', '07:42:37', '2025-03-24', 1),
(1368, 'TP08', 'MOG083497', 'MOG083497', '07:43:11', '2025-03-24', 1),
(1369, 'TP08', 'MOG083525', 'MOG083525', '07:50:20', '2025-03-24', 1),
(1370, 'TP08', 'MOG083525', 'MOG083525', '07:53:34', '2025-03-24', 1),
(1371, 'TP08', 'MOG083525', 'MOG083525', '07:55:51', '2025-03-24', 1),
(1372, 'TP08', 'MOG083525', 'MOG083525', '08:09:33', '2025-03-24', 1),
(1373, 'TH17', 'HBL056746', 'MOG083559', '08:55:50', '2025-03-24', 1),
(1374, 'TH17', 'HBL056746', 'MOG083559', '08:58:23', '2025-03-24', 1),
(1375, 'TP08', 'MOG083521', 'MOG083521', '08:59:06', '2025-03-24', 1),
(1376, 'TP08', 'MOG083521', 'MOG083521', '09:00:14', '2025-03-24', 1),
(1377, 'TH17', 'HBL056746', 'MOG083559', '09:00:59', '2025-03-24', 1),
(1378, 'TH17', 'HBL056746', 'MOG083559', '09:03:41', '2025-03-24', 1),
(1379, 'TH17', 'HBL056746', 'MOG083559', '09:06:03', '2025-03-24', 1),
(1380, 'TH17', 'HBL056746', 'MOG083559', '09:07:50', '2025-03-24', 1),
(1381, 'TH17', 'HBL056746', 'MOG083559', '09:15:47', '2025-03-24', 1),
(1382, 'TH17', 'HBL056746', 'MOG083559', '09:17:36', '2025-03-24', 1),
(1383, 'TH17', 'HBL056746', 'MOG083559', '09:18:12', '2025-03-24', 1),
(1384, 'TH17', 'HBL056746', 'MOG083559', '10:04:25', '2025-03-24', 1),
(1385, 'TH17', 'HBL056746', 'MOG083559', '10:08:55', '2025-03-24', 1),
(1386, 'TH17', 'HBL056746', 'MOG083559', '10:15:05', '2025-03-24', 1),
(1387, 'TH17', 'HBL056746', 'MOG083559', '10:18:00', '2025-03-24', 1),
(1388, 'TH17', 'HBL056746', 'MOG083559', '10:20:10', '2025-03-24', 1),
(1389, 'TH17', 'HBL056746', 'MOG083559', '10:22:45', '2025-03-24', 1),
(1390, 'TH17', 'HBL056746', 'MOG083559', '10:34:24', '2025-03-24', 1),
(1391, 'TH17', 'HBL056746', 'MOG083559', '10:37:09', '2025-03-24', 1),
(1392, 'TH17', 'HBL056746', 'MOG083559', '10:55:43', '2025-03-24', 1),
(1393, 'TH17', 'HBL056746', 'MOG083559', '10:59:33', '2025-03-24', 1),
(1394, 'TH17', 'HBL056746', 'MOG083559', '11:03:04', '2025-03-24', 1),
(1395, 'TH17', 'HBL056746', 'MOG083559', '11:06:49', '2025-03-24', 1),
(1396, 'TH17', 'HBL056746', 'MOG083559', '11:10:09', '2025-03-24', 1),
(1397, 'TP08', 'MOG083520', 'MOG083520', '12:00:18', '2025-03-24', 1),
(1398, 'TP08', 'MOG083520', 'MOG083520', '12:44:27', '2025-03-24', 1),
(1399, 'TP08', 'MOG083520', 'MOG083520', '13:42:33', '2025-03-24', 1),
(1400, 'TP08', 'MOG083520', 'MOG083520', '13:45:01', '2025-03-24', 1),
(1401, 'TH17', 'HBL056746', 'MOG083559', '14:36:12', '2025-03-24', 1),
(1402, 'TH17', 'HBL056746', 'MOG083559', '14:42:52', '2025-03-24', 1),
(1403, 'TH17', 'HBL056746', 'MOG083559', '14:53:10', '2025-03-24', 1),
(1404, 'TH17', 'HBL056746', 'MOG083559', '14:56:50', '2025-03-24', 1),
(1405, 'TH17', 'HBL056746', 'MOG083559', '15:01:26', '2025-03-24', 1),
(1406, 'TH17', 'HBL056746', 'MOG083559', '15:18:32', '2025-03-24', 1),
(1407, 'TH17', 'HBL056746', 'MOG083559', '15:49:26', '2025-03-24', 1),
(1408, 'TH17', 'HBL056746', 'MOG083559', '15:52:11', '2025-03-24', 1),
(1409, 'TH17', 'HBL056746', 'MOG083559', '16:08:52', '2025-03-24', 1),
(1410, 'TH17', 'HBL056746', 'MOG083559', '16:13:28', '2025-03-24', 1),
(1411, 'TH17', 'HBL056746', 'MOG083559', '07:10:56', '2025-03-25', 1),
(1412, 'TH17', 'HBL056746', 'MOG083559', '08:26:23', '2025-03-25', 1),
(1413, 'TH17', 'HBL056746', 'MOG083559', '08:38:27', '2025-03-25', 1),
(1414, 'TH17', 'HBL056746', 'MOG083559', '08:45:28', '2025-03-25', 1),
(1415, 'TH17', 'HBL056746', 'MOG083559', '08:47:02', '2025-03-25', 1),
(1416, 'TH17', 'HBL056746', 'MOG083559', '08:48:26', '2025-03-25', 1),
(1417, 'TH17', 'HBL056746', 'MOG083559', '08:51:11', '2025-03-25', 1),
(1418, 'TH17', 'HBL056746', 'MOG083559', '08:55:18', '2025-03-25', 1),
(1419, 'TH17', 'HBL056746', 'MOG083559', '09:01:00', '2025-03-25', 1),
(1420, 'TP08', 'MOG083520', 'MOG083520', '09:06:33', '2025-03-25', 1),
(1421, 'TH17', 'HBL056746', 'MOG083559', '12:41:31', '2025-03-25', 1),
(1422, 'TH17', 'HBL056746', 'MOG083559', '13:42:49', '2025-03-25', 1),
(1423, 'TP08', 'MOG083502', 'MOG083502', '13:52:05', '2025-03-25', 1),
(1424, 'TH17', 'HBL056746', 'MOG083559', '14:15:11', '2025-03-25', 1),
(1425, 'TP08', 'MOG083504', 'MOG083504', '14:16:32', '2025-03-25', 1),
(1426, 'TH17', 'HBL056746', 'MOG083559', '14:32:52', '2025-03-25', 1),
(1427, 'TH17', 'HBL056746', 'MOG083559', '14:40:19', '2025-03-25', 1),
(1428, 'TH17', 'HBL056746', 'MOG083559', '14:44:05', '2025-03-25', 1),
(1429, 'TH17', 'HBL056746', 'MOG083559', '14:47:00', '2025-03-25', 1),
(1430, 'TH17', 'HBL056746', 'MOG083559', '14:49:04', '2025-03-25', 1),
(1431, 'TH17', 'HBL056746', 'MOG083559', '14:53:05', '2025-03-25', 1),
(1432, 'TH17', 'HBL056746', 'MOG083559', '14:56:05', '2025-03-25', 1),
(1433, 'TH17', 'HBL056746', 'MOG083559', '14:58:45', '2025-03-25', 1),
(1434, 'TH17', 'HBL056746', 'MOG083559', '15:03:26', '2025-03-25', 1),
(1435, 'TH17', 'HBL056746', 'MOG083559', '15:11:24', '2025-03-25', 1),
(1436, 'TH17', 'HBL056746', 'MOG083559', '15:19:11', '2025-03-25', 1),
(1437, 'TH17', 'HBL056746', 'MOG083559', '15:20:30', '2025-03-25', 1),
(1438, 'TH17', 'HBL056746', 'MOG083559', '15:31:15', '2025-03-25', 1);
...
CREATE TABLE `das` (
  ...
  `piezas_meta` int(11) NOT NULL DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci;
...
INSERT INTO `das` (`id_das`, `linea`, `empleado_id_empleado`, `id_keeper`, `id_inspector`, `fecha`, `activaSupervisor`, `activaOperador`, `proceso`, `turno`, `piezas_meta`) VALUES
(1, 'TP08', 737, 737, 737, '2020-10-30', 1, 1, 'PRENSA', NULL, 0),
(2, 'TP08', 737, 737, 737, '2020-10-30', 1, 0, 'PRENSA', NULL, 0),
(3, 'TP08', 758, 758, 758, '2020-10-30', 1, 1, 'PRENSA', NULL, 0),
(4, 'TP08', 758, 758, 758, '2020-10-30', 1, 0, 'PRENSA', NULL, 0),
(5, 'TP08', 758, 758, 758, '2020-11-02', 1, 1, 'PRENSA', NULL, 0),
(6, 'TP08', 745, 745, 745, '2020-11-02', 1, 1, 'PRENSA', NULL, 0),
(7, 'TP08', 745, 745, 745, '2020-11-02', 1, 1, 'PRENSA', NULL, 0),
(8, 'TP08', 758, 758, 758, '2020-11-02', 1, 1, 'PRENSA', NULL, 0),
(9, 'TP08', 745, 745, 745, '2020-11-02', 1, 1, 'PRENSA', NULL, 0),
(10, 'TP08', 745, 745, 745, '2020-11-02', 1, 1, 'PRENSA', NULL, 0),
(11, 'TP01', 600, 600, 600, '2020-11-02', 1, 1, 'PRENSA', NULL, 0),
(12, 'TP03', 753, 753, 753, '2020-11-03', 1, 1, 'PRENSA', NULL, 0),
(13, 'TP03', 753, 753, 753, '2020-11-03', 1, 1, 'PRENSA', NULL, 0),
(14, 'TP03', 753, 753, 753, '2020-11-03', 1, 1, 'PRENSA', NULL, 0),
(15, 'TP03', 753, 753, 753, '2020-11-03', 1, 0, 'PRENSA', NULL, 0),
(16, 'TP03', 756, 756, 756, '2020-11-03', 1, 1, 'PRENSA', NULL, 0),
(17, 'TP03', 756, 756, 756, '2020-11-03', 1, 1, 'PRENSA', NULL, 0),
(18, 'TP03', 756, 756, 756, '2020-11-03', 1, 1, 'PRENSA', NULL, 0),
(19, 'TP01', 745, 745, 745, '2020-11-04', 1, 1, 'PRENSA', NULL, 0),
(20, 'TP01', 145, 145, 145, '2020-11-04', 1, 1, 'PRENSA', NULL, 0),
(21, 'TP01', 145, 145, 145, '2020-11-04', 1, 0, 'PRENSA', NULL, 0),
(22, 'TP01', 745, 745, 745, '2020-11-04', 1, 1, 'PRENSA', NULL, 0),
(23, 'TP01', 145, 145, 145, '2020-11-04', 1, 1, 'PRENSA', NULL, 0),
(24, 'TP01', 145, 145, 145, '2020-11-04', 1, 1, 'PRENSA', NULL, 0),
(25, 'TP03', 742, 742, 742, '2020-11-04', 1, 1, 'PRENSA', NULL, 0),
(26, 'TP03', 742, 742, 742, '2020-11-04', 1, 1, 'PRENSA', NULL, 0),
(27, 'TP05', 746, 746, 746, '2020-11-05', 1, 1, 'PRENSA', NULL, 0),
(28, 'TP05', 746, 746, 746, '2020-11-05', 1, 1, 'PRENSA', NULL, 0),
(29, 'TP05', 149, 149, 149, '2020-11-05', 1, 1, 'PRENSA', NULL, 0),
(30, 'TP05', 149, 149, 149, '2020-11-05', 1, 1, 'PRENSA', NULL, 0),
(31, 'TP01', 745, 745, 745, '2020-11-06', 1, 1, 'PRENSA', NULL, 0),
(32, 'TP01', 745, 745, 745, '2020-11-06', 1, 1, 'PRENSA', NULL, 0),
(33, 'TP01', 145, 145, 145, '2020-11-06', 1, 1, 'PRENSA', NULL, 0),
(34, 'TP01', 145, 145, 145, '2020-11-06', 1, 1, 'PRENSA', NULL, 0),
(35, 'TP05', 149, 149, 149, '2020-11-10', 1, 1, 'PRENSA', NULL, 0),
(36, 'TP05', 149, 149, 149, '2020-11-10', 1, 1, 'PRENSA', NULL, 0),
(37, 'TP05', 739, 739, 739, '2020-11-10', 1, 1, 'PRENSA', NULL, 0),
(38, 'TP05', 739, 739, 739, '2020-11-10', 1, 0, 'PRENSA', NULL, 0),
(39, 'TP01', 145, 145, 145, '2020-11-10', 1, 1, 'PRENSA', NULL, 0),
(40, 'TP01', 145, 145, 145, '2020-11-10', 1, 1, 'PRENSA', NULL, 0),
(41, 'TP03', 174, 174, 174, '2020-11-10', 1, 1, 'PRENSA', NULL, 0),
(42, 'TP03', 174, 174, 174, '2020-11-10', 1, 1, 'PRENSA', NULL, 0),
(43, 'TP04', 737, 737, 737, '2020-11-11', 1, 1, 'PRENSA', NULL, 0),
(44, 'TP04', 737, 737, 737, '2020-11-11', 1, 1, 'PRENSA', NULL, 0),
(45, 'TP05', 749, 749, 749, '2020-11-12', 1, 1, 'PRENSA', NULL, 0),
(46, 'TP05', 749, 749, 749, '2020-11-12', 1, 0, 'PRENSA', NULL, 0),
(47, 'TP07', 734, 734, 734, '2020-11-13', 1, 1, 'PRENSA', NULL, 0),
(48, 'TP07', 734, 734, 734, '2020-11-13', 1, 1, 'PRENSA', NULL, 0),
(49, 'TP07', 740, 740, 740, '2020-11-13', 1, 1, 'PRENSA', NULL, 0),
(50, 'TP07', 740, 740, 740, '2020-11-13', 1, 1, 'PRENSA', NULL, 0),
(51, 'TP10', 752, 752, 752, '2020-11-17', 1, 1, 'PRENSA', NULL, 0),
(52, 'TP10', 752, 752, 752, '2020-11-17', 1, 1, 'PRENSA', NULL, 0),
(53, 'TP10', 746, 746, 746, '2020-11-19', 1, 1, 'PRENSA', NULL, 0),
(54, 'TP01', 145, 145, 145, '2020-11-19', 1, 1, 'PRENSA', NULL, 0),
(55, 'TP10', 746, 746, 746, '2020-11-19', 1, 1, 'PRENSA', NULL, 0),
(56, 'TP01', 145, 145, 145, '2020-11-19', 1, 1, 'PRENSA', NULL, 0),
(57, 'TP01', 149, 149, 149, '2020-11-21', 1, 1, 'PRENSA', NULL, 0),
(58, 'TP01', 149, 149, 149, '2020-11-21', 1, 1, 'PRENSA', NULL, 0),
(59, 'TP41', 148, 148, 148, '2020-11-21', 1, 1, 'PRENSA', NULL, 0),
(60, 'TP41', 148, 148, 148, '2020-11-21', 1, 1, 'PRENSA', NULL, 0),
(61, 'TP01', 145, 145, 145, '2020-11-21', 1, 1, 'PRENSA', NULL, 0),
(62, 'TP01', 145, 145, 145, '2020-11-21', 1, 1, 'PRENSA', NULL, 0),
(63, 'TP41', 739, 739, 739, '2020-11-23', 1, 1, 'PRENSA', NULL, 0),
(64, 'TP41', 739, 739, 739, '2020-11-23', 1, 0, 'PRENSA', NULL, 0),
(65, 'TP41', 148, 148, 148, '2020-11-23', 1, 1, 'PRENSA', NULL, 0),
(66, 'TP41', 148, 148, 148, '2020-11-23', 1, 1, 'PRENSA', NULL, 0),
(67, 'TP03', 753, 753, 753, '2020-11-24', 1, 1, 'PRENSA', NULL, 0),
(68, 'TP03', 753, 753, 753, '2020-11-24', 1, 0, 'PRENSA', NULL, 0),
(69, 'TP03', 756, 756, 756, '2020-11-24', 1, 1, 'PRENSA', NULL, 0),
(70, 'TP03', 756, 756, 756, '2020-11-24', 1, 0, 'PRENSA', NULL, 0),
(71, 'TP10', 169, 169, 169, '2020-11-25', 1, 1, 'PRENSA', NULL, 0),
(72, 'TP10', 169, 169, 169, '2020-11-25', 1, 1, 'PRENSA', NULL, 0),
(73, 'TP02', 170, 170, 170, '2020-11-30', 1, 1, 'PRENSA', NULL, 0),
(74, 'TP02', 170, 170, 170, '2020-11-30', 1, 1, 'PRENSA', NULL, 0),
(75, 'TP02', 148, 148, 148, '2020-11-30', 1, 1, 'PRENSA', NULL, 0),
(76, 'TP02', 148, 148, 148, '2020-11-30', 1, 1, 'PRENSA', NULL, 0),
(77, 'TP09', 755, 755, 755, '2020-12-02', 1, 1, 'PRENSA', NULL, 0),
(78, 'TP09', 755, 755, 755, '2020-12-02', 1, 1, 'PRENSA', NULL, 0),
(79, 'TP09', 747, 747, 747, '2020-12-02', 1, 1, 'PRENSA', NULL, 0),
(80, 'TP09', 747, 747, 747, '2020-12-02', 1, 1, 'PRENSA', NULL, 0),
(81, 'TP04', 745, 745, 745, '2020-12-07', 1, 1, 'PRENSA', NULL, 0),
(82, 'TP04', 745, 745, 745, '2020-12-07', 1, 0, 'PRENSA', NULL, 0),
(83, 'TP04', 748, 748, 748, '2020-12-07', 1, 1, 'PRENSA', NULL, 0),
(84, 'TP04', 748, 748, 748, '2020-12-07', 1, 0, 'PRENSA', NULL, 0),
(85, 'TP01', 170, 170, 170, '2020-12-08', 1, 1, 'PRENSA', NULL, 0),
(86, 'TP41', 752, 752, 752, '2020-12-08', 1, 1, 'PRENSA', NULL, 0),
(87, 'TP41', 752, 752, 752, '2020-12-08', 1, 0, 'PRENSA', NULL, 0),
(88, 'TP41', 738, 738, 738, '2020-12-08', 1, 1, 'PRENSA', NULL, 0),
(89, 'TP41', 738, 738, 738, '2020-12-08', 1, 0, 'PRENSA', NULL, 0),
(90, 'TP09', 755, 755, 755, '2020-12-09', 1, 1, 'PRENSA', NULL, 0),
(91, 'TP09', 755, 755, 755, '2020-12-09', 1, 0, 'PRENSA', NULL, 0),
(92, 'TP09', 747, 747, 747, '2020-12-09', 1, 1, 'PRENSA', NULL, 0),
(93, 'TP09', 747, 747, 747, '2020-12-09', 1, 1, 'PRENSA', NULL, 0),
(94, 'TP09', 755, 755, 755, '2020-12-09', 1, 1, 'PRENSA', NULL, 0),
(95, 'TP09', 755, 755, 755, '2020-12-09', 1, 1, 'PRENSA', NULL, 0),
(96, 'TP05', 739, 739, 739, '2020-12-10', 1, 1, 'PRENSA', NULL, 0),
(97, 'TP05', 739, 739, 739, '2020-12-10', 1, 0, 'PRENSA', NULL, 0),
(98, 'TP05', 739, 739, 739, '2020-12-10', 1, 1, 'PRENSA', NULL, 0),
(99, 'TP05', 739, 739, 739, '2020-12-10', 1, 0, 'PRENSA', NULL, 0),
(100, 'TP02', 740, 740, 740, '2020-12-11', 1, 1, 'PRENSA', NULL, 0),
(101, 'TP02', 740, 740, 740, '2020-12-11', 1, 1, 'PRENSA', NULL, 0),
(102, 'TP02', 740, 740, 740, '2020-12-11', 1, 0, 'PRENSA', NULL, 0),
(103, 'TP02', 740, 740, 740, '2020-12-11', 1, 0, 'PRENSA', NULL, 0),
(104, 'TP04', 738, 738, 738, '2020-12-16', 1, 1, 'PRENSA', NULL, 0),
(105, 'TP04', 738, 738, 738, '2020-12-16', 1, 0, 'PRENSA', NULL, 0),
(106, 'TP41', 740, 740, 740, '2020-12-18', 1, 1, 'PRENSA', NULL, 0),
(107, 'TP41', 740, 740, 740, '2020-12-18', 1, 0, 'PRENSA', NULL, 0),
(108, 'TP05', 746, 746, 746, '2021-01-05', 1, 1, 'PRENSA', NULL, 0),
(109, 'TP05', 746, 746, 746, '2021-01-05', 1, 1, 'PRENSA', NULL, 0),
(110, 'TP10', 757, 757, 757, '2021-01-05', 1, 1, 'PRENSA', NULL, 0),
(111, 'TP10', 757, 757, 757, '2021-01-05', 1, 0, 'PRENSA', NULL, 0),
(112, 'TP05', 738, 738, 738, '2021-01-06', 1, 1, 'PRENSA', NULL, 0),
(113, 'TP05', 738, 738, 738, '2021-01-06', 1, 0, 'PRENSA', NULL, 0),
(114, 'TP09', 747, 747, 747, '2021-01-08', 1, 1, 'PRENSA', NULL, 0),
(115, 'TP09', 747, 747, 747, '2021-01-08', 1, 0, 'PRENSA', NULL, 0),
(117, 'TP41', 738, 738, 738, '2021-01-13', 1, 0, 'PRENSA', NULL, 0),
(119, 'TP41', 738, 738, 738, '2021-01-13', 1, 0, 'PRENSA', NULL, 0),
(120, 'TP03', 742, 742, 742, '2021-01-14', 1, 1, 'PRENSA', NULL, 0),
(121, 'TP03', 742, 742, 742, '2021-01-14', 1, 0, 'PRENSA', NULL, 0),
(122, 'TP02', 174, 174, 174, '2021-01-14', 1, 1, 'PRENSA', NULL, 0),
(123, 'TP02', 174, 174, 174, '2021-01-14', 1, 0, 'PRENSA', NULL, 0),
(124, 'TP01', 746, 746, 746, '2021-01-15', 1, 1, 'PRENSA', NULL, 0),
(125, 'TP01', 746, 746, 746, '2021-01-15', 1, 0, 'PRENSA', NULL, 0),
(126, 'TP03', 755, 755, 755, '2021-01-15', 1, 1, 'PRENSA', NULL, 0),
(127, 'TP03', 755, 755, 755, '2021-01-15', 1, 0, 'PRENSA', NULL, 0),
(128, 'TP01', 753, 753, 753, '2021-01-18', 1, 1, 'PRENSA', NULL, 0),
(129, 'TP01', 753, 753, 753, '2021-01-18', 1, 0, 'PRENSA', NULL, 0),
(130, 'TP02', 174, 174, 174, '2021-01-18', 1, 1, 'PRENSA', NULL, 0),
(131, 'TP02', 174, 174, 174, '2021-01-18', 1, 0, 'PRENSA', NULL, 0),
(132, 'TP11', 740, 740, 740, '2021-01-21', 1, 1, 'PRENSA', NULL, 0),
(133, 'TP11', 740, 740, 740, '2021-01-21', 1, 0, 'PRENSA', NULL, 0),
(134, 'TP11', 735, 735, 735, '2021-01-21', 1, 1, 'PRENSA', NULL, 0),
(135, 'TP11', 735, 735, 735, '2021-01-21', 1, 0, 'PRENSA', NULL, 0),
(136, 'TP09', 755, 755, 755, '2021-01-22', 1, 1, 'PRENSA', NULL, 0),
(137, 'TP09', 755, 755, 755, '2021-01-22', 1, 0, 'PRENSA', NULL, 0),
(138, 'TP10', 743, 743, 743, '2021-01-22', 1, 1, 'PRENSA', NULL, 0),
(139, 'TP10', 743, 743, 743, '2021-01-22', 1, 0, 'PRENSA', NULL, 0),
(140, 'TP02', 746, 746, 746, '2021-01-27', 1, 1, 'PRENSA', NULL, 0),
(141, 'TP02', 746, 746, 746, '2021-01-27', 1, 0, 'PRENSA', NULL, 0),
(142, 'TP04', 740, 740, 740, '2021-01-27', 1, 1, 'PRENSA', NULL, 0),
(143, 'TP04', 740, 740, 740, '2021-01-27', 1, 0, 'PRENSA', NULL, 0),
(144, 'TP09', 756, 756, 756, '2021-02-18', 1, 1, 'PRENSA', NULL, 0),
(145, 'TP09', 756, 756, 756, '2021-02-18', 1, 0, 'PRENSA', NULL, 0),
(146, 'TP02', 747, 747, 747, '2021-02-18', 1, 1, 'PRENSA', NULL, 0),
(147, 'TP02', 747, 747, 747, '2021-02-18', 1, 0, 'PRENSA', NULL, 0),
(148, 'TP11', 740, 740, 740, '2021-02-24', 1, 1, 'PRENSA', NULL, 0),
(149, 'TP11', 740, 740, 740, '2021-02-24', 1, 0, 'PRENSA', NULL, 0),
(150, 'TP01', 745, 745, 745, '2021-04-19', 1, 1, 'PRENSA', NULL, 0),
(151, 'TP01', 745, 745, 745, '2021-04-19', 1, 0, 'PRENSA', NULL, 0),
(152, 'TP01', 169, 169, 169, '2021-06-14', 1, 1, 'PRENSA', NULL, 0),
(153, 'TP01', 169, 169, 169, '2021-06-14', 1, 1, 'PRENSA', NULL, 0),
(154, 'TP01', 169, 169, 169, '2021-06-14', 1, 1, 'PRENSA', NULL, 0),
(155, 'TP01', 169, 169, 169, '2021-06-14', 1, 1, 'PRENSA', NULL, 0),
(156, 'TP05', 758, 758, 758, '2021-06-15', 1, 1, 'PRENSA', NULL, 0),
(157, 'TP05', 758, 758, 758, '2021-06-15', 1, 1, 'PRENSA', NULL, 0),
(158, 'TP02', 745, 745, 745, '2021-06-15', 1, 1, 'PRENSA', NULL, 0),
(159, 'TP02', 745, 745, 745, '2021-06-15', 1, 1, 'PRENSA', NULL, 0),
(160, 'TP02', 745, 745, 745, '2021-06-15', 1, 1, 'PRENSA', NULL, 0),
(161, 'TP02', 745, 745, 745, '2021-06-15', 1, 0, 'PRENSA', NULL, 0),
(162, 'TP08', 738, 738, 738, '2021-06-17', 1, 1, 'PRENSA', NULL, 0),
(163, 'TP01', 745, 745, 745, '2021-06-21', 1, 1, 'PRENSA', NULL, 0),
(164, 'TP01', 745, 745, 745, '2021-06-21', 1, 0, 'PRENSA', NULL, 0),
(165, 'TP04', 748, 746, 746, '2021-06-22', 1, 1, 'PRENSA', NULL, 0),
(166, 'TP04', 746, 746, 746, '2021-06-22', 1, 0, 'PRENSA', NULL, 0),
(167, 'TP02', 741, 741, 741, '2021-06-24', 1, 1, 'PRENSA', NULL, 0),
(168, 'TP02', 741, 741, 741, '2021-06-24', 1, 0, 'PRENSA', NULL, 0),
(169, 'TP11', 753, 753, 753, '2021-06-30', 1, 1, 'PRENSA', NULL, 0),
(170, 'TP11', 753, 753, 753, '2021-06-30', 1, 0, 'PRENSA', NULL, 0),
(171, 'TP10', 758, 758, 758, '2021-07-21', 1, 1, 'PRENSA', NULL, 0),
(172, 'TP10', 758, 758, 758, '2021-07-21', 1, 0, 'PRENSA', NULL, 0),
(173, 'TP10', 758, 758, 758, '2021-07-22', 1, 1, 'PRENSA', NULL, 0),
(174, 'TP10', 758, 758, 758, '2021-07-22', 1, 0, 'PRENSA', NULL, 0),
(175, 'TH02', 66, 66, 66, '2021-08-03', 1, 0, 'MAQUINADO', NULL, 0),
(176, 'TH13', 69, 69, 69, '2021-08-04', 1, 0, 'MAQUINADO', NULL, 0),
(177, 'TH02', 42, 42, 42, '2021-08-06', 1, 0, 'MAQUINADO', NULL, 0),
(178, 'TH12', 35, 35, 35, '2021-08-10', 1, 1, 'MAQUINADO', NULL, 0),
(179, 'TH11', 47, 49, 49, '2021-08-10', 1, 0, 'MAQUINADO', NULL, 0),
(180, 'TH13', 27, 27, 27, '2021-08-10', 1, 1, 'MAQUINADO', NULL, 0),
(181, 'TH03', 131, 34, 131, '2021-08-18', 1, 1, 'MAQUINADO', NULL, 0),
(182, 'TH03', 131, 34, 34, '2021-08-18', 1, 0, 'MAQUINADO', NULL, 0),
(183, 'TP02', 756, 756, 756, '2021-08-19', 1, 1, 'PRENSA', NULL, 0),
(184, 'TP02', 756, 756, 756, '2021-08-19', 1, 1, 'PRENSA', NULL, 0),
(185, 'TH10', 42, 42, 42, '2021-08-19', 1, 1, 'MAQUINADO', NULL, 0),
(186, 'TH06', 37, 122, 122, '2021-08-20', 1, 1, 'MAQUINADO', NULL, 0),
(187, 'TH06', 122, 122, 122, '2021-08-20', 1, 0, 'MAQUINADO', NULL, 0),
(188, 'TH06', 123, 66, 122, '2021-08-20', 1, 1, 'MAQUINADO', NULL, 0),
(189, 'TH17', 58, 66, 30, '2021-08-20', 1, 1, 'MAQUINADO', NULL, 0),
(190, 'TH12', 66, 66, 122, '2021-08-20', 1, 1, 'MAQUINADO', NULL, 0),
(191, 'TP41', 747, 747, 747, '2021-08-24', 1, 0, 'PRENSA', NULL, 0),
(192, 'TP41', 741, 741, 741, '2021-08-24', 1, 0, 'PRENSA', NULL, 0),
(193, 'TP11', 745, 745, 745, '2021-08-25', 1, 1, 'PRENSA', NULL, 0),
(194, 'TP11', 745, 745, 745, '2021-08-25', 1, 1, 'PRENSA', NULL, 0),
(195, 'TP05', 174, 148, 148, '2021-08-25', 1, 1, 'PRENSA', NULL, 0),
(196, 'TP05', 148, 148, 148, '2021-08-25', 1, 1, 'PRENSA', NULL, 0),
(197, 'TH05', 66, 66, 66, '2021-08-26', 1, 0, 'MAQUINADO', NULL, 0),
(198, 'TH14', 69, 69, 69, '2021-08-26', 1, 1, 'MAQUINADO', NULL, 0),
(199, 'TP11', 745, 745, 745, '2021-08-26', 1, 1, 'PRENSA', NULL, 0),
(200, 'TP11', 745, 745, 745, '2021-08-26', 1, 0, 'PRENSA', NULL, 0),
(201, 'TP01', 757, 757, 757, '2021-08-26', 1, 0, 'PRENSA', NULL, 0),
(202, 'TP08', 170, 170, 170, '2021-08-26', 1, 1, 'PRENSA', NULL, 0),
(203, 'TH10', 131, 53, 131, '2021-08-26', 1, 1, 'MAQUINADO', NULL, 0),
(204, 'TH10', 131, 53, 131, '2021-08-26', 1, 1, 'MAQUINADO', NULL, 0),
(205, 'TH05', 35, 42, 35, '2021-08-27', 1, 0, 'MAQUINADO', NULL, 0),
(206, 'TP08', 738, 756, 756, '2021-08-27', 1, 1, 'PRENSA', NULL, 0),
(207, 'TP08', 756, 756, 756, '2021-08-27', 1, 0, 'PRENSA', NULL, 0),
(208, 'TP41', 148, 148, 148, '2021-08-27', 1, 0, 'PRENSA', NULL, 0),
(209, 'TP01', 750, 750, 750, '2021-08-27', 1, 0, 'PRENSA', NULL, 0),
(210, 'TP10', 752, 752, 752, '2021-08-27', 1, 1, 'PRENSA', NULL, 0),
(211, 'TP04', 737, 743, 743, '2021-08-30', 1, 1, 'PRENSA', NULL, 0),
(212, 'TP04', 743, 743, 743, '2021-08-30', 1, 0, 'PRENSA', NULL, 0),
(213, 'TP04', 744, 744, 744, '2021-08-30', 1, 0, 'PRENSA', NULL, 0),
(214, 'TP41', 740, 740, 740, '2021-08-30', 1, 0, 'PRENSA', NULL, 0),
(215, 'TH07', 42, 42, 48, '2021-08-31', 1, 0, 'MAQUINADO', NULL, 0),
(216, 'TH02', 131, 27, 131, '2021-09-02', 1, 0, 'MAQUINADO', NULL, 0),
(217, 'TP09', 174, 755, 755, '2021-09-21', 1, 1, 'PRENSA', NULL, 0),
(218, 'TP09', 755, 755, 755, '2021-09-21', 1, 0, 'PRENSA', NULL, 0),
(219, 'TP08', 735, 735, 735, '2021-09-21', 1, 0, 'PRENSA', NULL, 0),
(220, 'TP03', 735, 735, 735, '2021-09-21', 1, 0, 'PRENSA', NULL, 0),
(221, 'TP03', 751, 751, 751, '2021-09-21', 1, 1, 'PRENSA', NULL, 0),
(222, 'TP01', 755, 753, 753, '2021-09-22', 1, 1, 'PRENSA', NULL, 0),
(223, 'TP01', 753, 753, 753, '2021-09-22', 1, 0, 'PRENSA', NULL, 0),
(224, 'TP04', 174, 174, 174, '2021-09-22', 1, 0, 'PRENSA', NULL, 0),
(225, 'TP09', 746, 746, 746, '2021-09-23', 1, 1, 'PRENSA', NULL, 0),
(226, 'TP09', 746, 746, 746, '2021-09-23', 1, 1, 'PRENSA', NULL, 0),
(227, 'TH42', 37, 2, 37, '2021-09-28', 1, 1, 'MAQUINADO', NULL, 0),
(228, 'TP05', 752, 752, 752, '2021-09-28', 1, 1, 'PRENSA', NULL, 0),
(229, 'TP05', 752, 752, 752, '2021-09-28', 1, 1, 'PRENSA', NULL, 0),
(230, 'TH17', 122, 36, 129, '2021-09-29', 1, 0, 'MAQUINADO', NULL, 0),
(231, 'TH17', 31, 36, 48, '2021-09-29', 1, 0, 'MAQUINADO', NULL, 0),
(232, 'TH17', 58, 2, 58, '2021-10-12', 1, 1, 'MAQUINADO', NULL, 0),
(233, 'TH17', 58, 2, 58, '2021-10-12', 1, 1, 'MAQUINADO', NULL, 0),
(234, 'TH17', 58, 2, 58, '2021-10-12', 1, 1, 'MAQUINADO', NULL, 0),
(235, 'th10', 28, 45, 54, '2022-05-19', 1, 1, 'MAQUINADO', NULL, 0),
(236, 'TH10', 28, 28, 28, '2022-05-19', 1, 1, 'MAQUINADO', NULL, 0),
(237, 'TH10', 28, 45, 45, '2022-05-20', 1, 1, 'MAQUINADO', NULL, 0),
(238, 'TH10', 45, 45, 45, '2022-05-23', 1, 1, 'MAQUINADO', NULL, 0),
(239, 'TH10', 34, 34, 34, '2022-05-23', 1, 1, 'MAQUINADO', NULL, 0),
(240, 'TG01', 701, 701, 701, '2022-05-24', 1, 1, 'T. SUPERFICIES', NULL, 0),
(241, 'TH10', 28, 28, 28, '2022-05-25', 1, 1, 'MAQUINADO', NULL, 0),
(242, 'TH10', 28, 28, 28, '2022-05-25', 1, 1, 'MAQUINADO', NULL, 0),
(243, 'TH10', 28, 28, 28, '2022-05-25', 1, 1, 'MAQUINADO', NULL, 0),
(244, 'TH10', 28, 28, 28, '2022-05-26', 1, 1, 'MAQUINADO', NULL, 0),
(245, 'TH10', 28, 28, 28, '2022-05-26', 1, 1, 'MAQUINADO', NULL, 0),
(246, 'TH10', 47, 47, 47, '2022-05-26', 1, 1, 'MAQUINADO', NULL, 0),
(247, 'TH10', 28, 28, 28, '2022-05-26', 1, 1, 'MAQUINADO', NULL, 0),
(248, 'TH10', 28, 28, 28, '2022-05-30', 1, 1, 'MAQUINADO', NULL, 0),
(249, 'TH10', 35, 35, 35, '2022-05-30', 1, 1, 'MAQUINADO', NULL, 0),
(250, 'TH10', 17, 17, 17, '2022-05-30', 1, 1, 'MAQUINADO', NULL, 0),
(251, 'TH10', 25, 25, 25, '2022-05-30', 1, 1, 'MAQUINADO', NULL, 0),
(252, 'TH17', 25, 25, 25, '2022-05-30', 1, 1, 'MAQUINADO', NULL, 0),
(253, 'TH10', 19, 19, 19, '2022-05-30', 1, 1, 'MAQUINADO', NULL, 0),
(254, 'TH10', 18, 18, 18, '2022-06-14', 1, 1, 'MAQUINADO', NULL, 0),
(255, 'TP01', 600, 600, 600, '2024-01-10', 1, 1, 'PRENSA', NULL, 0),
(256, 'TP01', 600, 600, 600, '2024-01-10', 1, 1, 'PRENSA', NULL, 0),
(257, 'TH17', 31, 2, 2, '2024-01-10', 1, 1, 'MAQUINADO', NULL, 0),
(258, 'TH17', 31, 2, 2, '2024-01-10', 1, 0, 'MAQUINADO', NULL, 0),
(259, 'TP11', 735, 735, 735, '2024-01-17', 1, 1, 'PRENSA', NULL, 0),
(260, 'TP11', 735, 735, 735, '2024-01-17', 1, 1, 'PRENSA', NULL, 0),
(261, 'TP11', 600, 600, 600, '2024-01-17', 1, 1, 'PRENSA', NULL, 0),
(262, 'TP11', 735, 735, 735, '2024-01-17', 1, 1, 'PRENSA', NULL, 0),
(263, 'TP11', 734, 734, 734, '2024-01-17', 1, 1, 'PRENSA', NULL, 0),
(264, 'TP11', 600, 600, 600, '2024-02-12', 1, 1, 'PRENSA', NULL, 0),
(267, 'TP08', 735, 600, 600, '2025-03-05', 1, 1, 'PRENSA', NULL, 0),
(268, 'TP08', 149, 600, 600, '2025-03-06', 1, 1, 'PRENSA', NULL, 0),
(269, 'TH17', 49, 45, 107, '2025-03-06', 1, 1, 'MAQUINADO', NULL, 0),
(270, 'TP05', 149, 600, 600, '2025-03-06', 1, 1, 'PRENSA', NULL, 0),
(271, 'TH17', 49, 45, 107, '2025-03-07', 1, 1, 'MAQUINADO', NULL, 0),
(272, 'TP08', 761, 600, 600, '2025-03-07', 1, 1, 'PRENSA', NULL, 0),
(273, 'MOG08', 761, 600, 600, '2025-03-07', 1, 1, 'PRENSA', NULL, 0),
(274, 'TP02', 761, 600, 600, '2025-03-10', 1, 1, 'PRENSA', 0, 0),
(275, 'TH17', 49, 45, 107, '2025-03-10', 1, 1, 'MAQUINADO', 0, 0),
(276, 'TH17', 49, 45, 107, '2025-03-14', 1, 1, 'MAQUINADO', 1, 0),
(282, 'TH17', 49, 45, 107, '2025-03-14', 1, 0, 'MAQUINADO', 2, 0),
(288, 'TH17', 49, 45, 107, '2025-03-18', 1, 0, 'MAQUINADO', 1, 0),
(289, 'TH17', 49, 49, 49, '2025-03-18', 1, 1, 'MAQUINADO', 2, 0),
(290, 'TP08', 761, 600, 600, '2025-03-18', 1, 1, 'PRENSA', 0, 0),
(292, 'TH17', 49, 45, 107, '2025-03-19', 1, 0, 'MAQUINADO', 1, 0),
(293, 'TH17', 49, 45, 107, '2025-03-19', 1, 0, 'MAQUINADO', 2, 0),
(303, 'TP08', 761, 600, 600, '2025-03-20', 1, 1, 'PRENSA', 0, 0),
(304, 'TP08', 761, 600, 600, '2025-03-20', 1, 1, 'PRENSA', 1, 0),
(305, 'TH17', 49, 49, 49, '2025-03-20', 1, 1, 'MAQUINADO', 1, 0),
(307, 'TH17', 49, 45, 107, '2025-03-20', 1, 1, 'MAQUINADO', 2, 0),
(308, 'TH17', 49, 49, 49, '2025-03-21', 1, 1, 'MAQUINADO', 1, 0),
(309, 'TH17', 49, 45, 107, '2025-03-24', 1, 1, 'MAQUINADO', 1, 10000),
(310, 'TP08', 761, 600, 600, '2025-03-24', 1, 1, 'PRENSA', 0, 0),
(311, 'TH17', 49, 49, 49, '2025-03-24', 1, 1, 'MAQUINADO', 2, 0),
(312, 'TH17', 49, 45, 107, '2025-03-25', 1, 1, 'MAQUINADO', 1, 23000),
(313, 'TP08', 761, 600, 600, '2025-03-25', 1, 1, 'PRENSA', 0, 0);
...
--
-- Estructura de tabla para la tabla `das_prod_prensa`
--

CREATE TABLE `das_prod_prensa` (
  `id_daspropren` int(11) NOT NULL,
  `inicio_tp` varchar(30) COLLATE utf8_spanish_ci NOT NULL,
  `fin_tp` varchar(30) COLLATE utf8_spanish_ci NOT NULL,
  `centroEstampado` varchar(10) COLLATE utf8_spanish_ci DEFAULT NULL,
  ...
  `das_iddas` int(10) NOT NULL,
  `mog_idmog` int(11) NOT NULL,
  `rbp_id_rbp` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci;

--
-- Volcado de datos para la tabla `das_prod_prensa`
--

INSERT INTO `das_prod_prensa` (`id_daspropren`, `inicio_tp`, `fin_tp`, `centroEstampado`, `extremo`, `sello`, `pzasTotales`, `bm_kg`, `pcs_bm`, `pza_ok`, `ng_kg`, `pcs_ng`, `das_iddas`, `mog_idmog`, `rbp_id_rbp`) VALUES
(1, '15:50:45', '15:55', NULL, NULL, '', 6829, 4.5, 274, 6525, 0.5, 30, 2, 31, NULL),
(2, '15:50:45', '15:55', NULL, NULL, '', 6555, 4.5, 274, 6525, 0.5, 30, 2, 31, NULL),
(3, '15:58:08', '16:00', NULL, NULL, '', 2025, 0, 0, 2025, 0, 0, 2, 31, NULL),
(4, '15:50:45', '15:55', NULL, NULL, '', 6555, 4.5, 274, 6525, 0.5, 30, 2, 31, NULL),
(5, '15:58:08', '16:00', NULL, NULL, '', 2025, 0, 0, 2025, 0, 0, 2, 31, NULL),
(6, '16:01:28', '16:07', NULL, NULL, '', 11505, 8, 487, 11475, 0.5, 30, 2, 31, NULL),
(7, '16:05:09', '16:11', NULL, NULL, '', 4335, 1, 60, 4275, 0, 0, 4, 32, NULL),
(8, '16:21:14', '16:24', NULL, NULL, '', 15768, 9, 548, 15750, 0.3, 18, 4, 32, NULL),
(9, '18:07:14', '18:09', NULL, NULL, '', 7060, 32, 1842, 4950, 3, 268, 11, 35, NULL),
(10, '13:41:17', '13:44', NULL, NULL, '', 4905, 4, 141, 4750, 0, 14, 13, 37, NULL),
(11, '13:40:49', '13:47', NULL, NULL, '', 5168, 6.5, 230, 4918, 0, 20, 15, 36, NULL),
(12, '13:48:23', '13:51', NULL, NULL, '', 11079, 21.5, 762, 10247, 1, 70, 15, 36, NULL),
(13, '14:10:15', '14:13', NULL, NULL, '', 5785, 1.5, 69, 5670, 0.5, 46, 21, 39, NULL),
(14, '14:19:18', '14:23', NULL, NULL, '', 6553, 3.5, 161, 6300, 1, 92, 21, 39, NULL),
(15, '11:50:29', '11:54', NULL, NULL, '', 16444, 11, 1023, 15400, 0, 21, 28, 44, NULL),
(16, '11:50:29', '11:54', NULL, NULL, '', 5719, 11, 1023, 4675, 0, 21, 28, 44, NULL),
(17, '13:51:36', '13:56', NULL, NULL, '', 12943, 8, 368, 12552, 0.5, 23, 34, 45, NULL),
(18, '13:52:03', '13:55', NULL, NULL, '', 22344, 14, 645, 21630, 1.5, 69, 32, 46, NULL),
(19, '13:51:36', '13:56', NULL, NULL, '', 8161, 8, 368, 7770, 0.5, 23, 34, 45, NULL),
(20, '13:56:58', '15:17', NULL, NULL, '', 8066, 4, 184, 7770, 1.2, 57, 34, 45, NULL),
(21, '11:14:36', '11:26', NULL, NULL, '', 16430, 14.5, 1348, 14850, 2.5, 232, 36, 47, NULL),
(22, '11:35:48', '12:01', NULL, NULL, '', 5768, 5, 465, 5225, 0.45, 78, 36, 47, NULL),
(23, '12:09:09', '12:13', NULL, NULL, '', 21144, 11, 1023, 20075, 0.5, 46, 38, 48, NULL),
(24, '15:32:06', '15:44', NULL, NULL, '', 16985, 19, 1767, 15125, 1, 93, 46, 51, NULL),
(25, '15:32:06', '15:44', NULL, NULL, '', 6810, 19, 1767, 4950, 1, 93, 46, 51, NULL),
(26, '15:55:58', '16:03', NULL, NULL, '', 5554, 6, 558, 4950, 0.5, 46, 46, 51, NULL),
(27, '15:32:06', '15:44', NULL, NULL, '', 16985, 19, 1767, 15125, 1, 93, 46, 51, NULL),
(28, '15:55:58', '16:03', NULL, NULL, '', 5554, 6, 558, 4950, 0.5, 46, 46, 51, NULL),
(29, '16:07:08', '16:15', NULL, NULL, '', 13443, 13.5, 1255, 12100, 0.95, 88, 46, 52, NULL),
(30, '16:07:08', '16:15', NULL, NULL, '', 9318, 13.5, 1255, 7975, 0.95, 88, 46, 52, NULL),
(31, '16:16:02', '16:20', NULL, NULL, '', 8713, 7.5, 697, 7975, 0.45, 41, 46, 52, NULL),
(32, '15:18:03', '15:23', NULL, NULL, '', 14330, 0.5, 25, 14280, 0.5, 25, 48, 53, NULL),
(33, '15:18:03', '15:23', NULL, NULL, '', 5930, 0.5, 25, 5880, 0.5, 25, 48, 53, NULL),
(34, '15:26:31', '15:28', NULL, NULL, '', 6060, 3, 155, 5880, 0.5, 25, 48, 53, NULL),
(35, '15:25:34', '15:33', NULL, NULL, '', 2413, 2, 103, 2310, 0, 0, 50, 54, NULL),
(36, '15:25:34', '15:33', NULL, NULL, '', 2833, 2, 103, 2730, 0, 0, 50, 54, NULL),
(37, '15:33:48', '15:36', NULL, NULL, '', 4329, 2.5, 129, 4200, 0, 0, 50, 54, NULL),
(38, '15:25:34', '15:33', NULL, NULL, '', 2203, 2, 103, 2100, 0, 0, 50, 54, NULL),
(39, '15:33:48', '15:36', NULL, NULL, '', 8949, 2.5, 129, 8820, 0, 0, 50, 54, NULL),
(40, '15:36:39', '15:41', NULL, NULL, '', 4303, 2, 103, 4200, 0, 0, 50, 54, NULL),
(41, '15:25:34', '15:33', NULL, NULL, '', 2413, 2, 103, 2310, 0, 0, 50, 54, NULL),
(42, '15:33:48', '15:36', NULL, NULL, '', 2859, 2.5, 129, 2730, 0, 0, 50, 54, NULL),
(43, '15:36:39', '15:41', NULL, NULL, '', 4303, 2, 103, 4200, 0, 0, 50, 54, NULL),
(44, '15:41:59', '15:46', NULL, NULL, '', 2176, 1, 51, 2100, 0.5, 25, 50, 54, NULL),
(45, '15:25:34', '15:33', NULL, NULL, '', 2413, 2, 103, 2310, 0, 0, 50, 54, NULL),
(46, '15:33:48', '15:36', NULL, NULL, '', 2859, 2.5, 129, 2730, 0, 0, 50, 54, NULL),
(47, '15:36:39', '15:41', NULL, NULL, '', 4303, 2, 103, 4200, 0, 0, 50, 54, NULL),
(48, '15:41:59', '15:46', NULL, NULL, '', 2176, 1, 51, 2100, 0.5, 25, 50, 54, NULL),
(49, '15:47:28', '15:51', NULL, NULL, '', 8855, 0.5, 25, 8820, 0.2, 10, 50, 54, NULL),
(50, '15:38:03', '15:44', NULL, NULL, '', 4596, 2.5, 115, 4458, 0.5, 23, 52, 56, NULL),
(51, '15:44:55', '15:52', NULL, NULL, '', 16210, 8.5, 391, 15750, 1.5, 69, 52, 56, NULL),
(52, '15:44:55', '15:52', NULL, NULL, '', 16210, 8.5, 391, 15750, 1.5, 69, 52, 56, NULL),
(53, '07:35:33', '10:25', NULL, NULL, '', 5252, 4.5, 252, 5000, 0, 0, 56, 59, NULL),
(54, '07:26:05', '10:28', NULL, NULL, '', 6786, 5, 230, 6510, 1, 46, 55, 58, NULL),
(55, '07:26:05', '10:28', NULL, NULL, '', 6786, 5, 230, 6510, 1, 46, 55, 58, NULL),
(56, '10:29:41', '13:04', NULL, NULL, '', 11268, 2.5, 115, 11130, 0.5, 23, 55, 58, NULL),
(57, '07:35:33', '10:25', NULL, NULL, '', 15654, 4.5, 252, 15402, 0, 0, 56, 59, NULL),
(58, '10:44:32', '15:36', NULL, NULL, '', 15879, 8, 449, 15402, 0.5, 28, 56, 59, NULL),
(59, '08:11:27', '12:53:11', NULL, NULL, '', 10115, 29, 614, 9480, 1, 21, 58, 60, NULL),
(60, '13:01:04', '14:05', NULL, NULL, '', 11200, 65.5, 1387, 9686, 6, 127, 62, 60, NULL),
(61, '08:40:17', '14:55', NULL, NULL, '', 22951, 6, 638, 22260, 0.5, 53, 60, 61, NULL),
(62, '15:16:08', '15:21', NULL, NULL, '', 20197, 7, 648, 19522, 0.3, 27, 64, 62, NULL),
(63, '15:28:23', '15:32', NULL, NULL, '', 21033, 4.5, 416, 20580, 0.4, 37, 66, 63, NULL),
(64, '16:05:04', '16:09', NULL, NULL, '', 20664, 9, 548, 20025, 1.5, 91, 68, 64, NULL),
(65, '16:06:08', '16:12', NULL, NULL, '', 14817, 8.5, 518, 14275, 0.4, 24, 70, 65, NULL),
(66, '16:14:49', '16:18', NULL, NULL, '', 3250, 6.5, 396, 2830, 0.4, 24, 70, 65, NULL),
(67, '15:49:31', '15:53', NULL, NULL, '', 16921, 18, 666, 16200, 1.5, 55, 72, 66, NULL),
(68, '15:49:31', '15:53', NULL, NULL, '', 16921, 18, 666, 16200, 1.5, 55, 72, 66, NULL),
(69, '10:18:48', '10:22', NULL, NULL, '', 21118, 16, 829, 20160, 2.5, 129, 74, 69, NULL),
(70, '10:09:56', '11:06', NULL, NULL, '', 15942, 12, 588, 15330, 0.5, 24, 76, 68, NULL),
(71, '11:07:44', '11:12', NULL, NULL, '', 4991, 3, 147, 4830, 0.3, 14, 76, 68, NULL),
(73, '15:33:08', '15:38', NULL, NULL, '', 18522, 17, 833, 17640, 1, 49, 80, 71, NULL),
(74, '15:33:02', '15:37', NULL, NULL, '', 19265, 24, 1176, 18060, 0.6, 29, 78, 70, NULL),
(75, '15:37:46', '15:41', NULL, NULL, '', 2100, 0, 0, 2100, 0, 0, 78, 70, NULL),
(77, '15:41:28', '15:45', NULL, NULL, '', 5017, 5, 245, 4772, 0, 0, 80, 71, NULL),
(78, '14:32:15', '14:37', NULL, NULL, '', 11653, 10, 628, 11000, 0.4, 25, 82, 72, NULL),
(79, '14:34:35', '14:39', NULL, NULL, '', 16662, 10, 869, 15750, 0.5, 43, 84, 73, NULL),
(80, '14:32:15', '14:37', NULL, NULL, '', 9653, 10, 628, 9000, 0.4, 25, 82, 72, NULL),
(81, '14:38:16', '14:42', NULL, NULL, '', 10663, 26, 1635, 9000, 0.46, 28, 82, 72, NULL),
(82, '14:40:25', '14:44', NULL, NULL, '', 4362, 1, 86, 4250, 0.3, 26, 84, 73, NULL),
(86, '11:26:07', '11:29', NULL, NULL, '', 1680, 0, 0, 1680, 0, 0, 87, 75, NULL),
(87, '11:33:44', '11:38', NULL, NULL, '', 19594, 7, 648, 18900, 0.5, 46, 87, 75, NULL),
(88, '11:39:42', '11:44', NULL, NULL, '', 20714, 4.5, 416, 20160, 1.5, 138, 87, 76, NULL),
(89, '13:33:16', '13:38', NULL, NULL, '', 16542, 6, 555, 15960, 0.3, 27, 89, 77, NULL),
(90, '13:33:16', '13:38', NULL, NULL, '', 6267, 6, 555, 5685, 0.3, 27, 89, 77, NULL),
(91, '13:39:18', '14:25', NULL, NULL, '', 5978, 3, 277, 5685, 0.183, 16, 89, 77, NULL),
(92, '11:38:10', '11:41', NULL, NULL, '', 14315, 8.5, 440, 13860, 0.3, 15, 91, 78, NULL),
(93, '11:38:10', '11:41', NULL, NULL, '', 6755, 8.5, 440, 6300, 0.3, 15, 91, 78, NULL),
(94, '11:42:01', '11:46', NULL, NULL, '', 6739, 8, 414, 6300, 0.5, 25, 91, 78, NULL),
(95, '12:32:21', '12:36', NULL, NULL, '', 17524, 13, 673, 16800, 1, 51, 93, 79, NULL),
(96, '12:37:22', '12:40', NULL, NULL, '', 5138, 3.5, 181, 4932, 0.5, 25, 95, 79, NULL),
(97, '11:53:03', '12:02', NULL, NULL, '', 6239, 1.5, 139, 5635, 5, 465, 97, 80, NULL),
(98, '11:59:34', '12:05', NULL, NULL, '', 6866, 3.5, 325, 6523, 0.2, 18, 99, 81, NULL),
(99, '12:06:15', '12:08', NULL, NULL, '', 14215, 4, 372, 13750, 1, 93, 99, 81, NULL),
(100, '12:03:50', '12:13', NULL, NULL, '', 15574, 10, 995, 14440, 1.5, 139, 97, 80, NULL),
(101, '15:45:03', '15:48', NULL, NULL, '', 20919, 17.5, 870, 20000, 1, 49, 102, 82, NULL),
(102, '15:45:31', '15:50', NULL, NULL, '', 16098, 28.5, 863, 15129, 3.5, 106, 103, 83, NULL),
(103, '15:45:31', '15:50', NULL, NULL, '', 5869, 28.5, 863, 4900, 3.5, 106, 103, 83, NULL),
(104, '15:51:45', '15:54', NULL, NULL, '', 5202, 9, 272, 4900, 1, 30, 103, 83, NULL),
(106, '12:27:31', '12:32', NULL, NULL, '', 4145, 14, 1302, 2750, 1, 93, 105, 84, NULL),
(107, '12:34:20', '12:38', NULL, NULL, '', 2843, 1, 93, 2750, 0, 0, 105, 84, NULL),
(108, '15:06:22', '15:09', 'YU', '', '', 21254, 10, 1063, 20160, 0.3, 31, 191, 147, NULL),
(109, '15:14:27', '15:22', 'XN', '', '', 19198, 15, 691, 18480, 0.6, 27, 192, 148, NULL),
(110, '15:25:43', '15:39', 'XT', '', '', 2155, 1, 46, 2100, 0.2, 9, 192, 148, NULL),
(111, '17:14:16', '17:24', 'WB', '', '', 2791, 1, 51, 2730, 0.2, 10, 196, 150, NULL),
(112, '08:43:31', '08:57', 'N', '', '', 7794, 6.5, 625, 7150, 0.2, 19, 200, 154, NULL),
(113, '08:59:00', '09:07', 'Q', '', '', 7086, 2, 192, 6875, 0.2, 19, 200, 154, NULL),
(114, '09:09:33', '09:15', '0', '', '', 6231, 9, 213, 6000, 0.8, 18, 201, 155, NULL),
(115, '09:16:08', '09:24', '0', '', '', 11369, 16.5, 501, 10850, 0.6, 18, 202, 156, NULL),
(116, '08:27:31', '08:31', 'L', '', '', 7802, 9, 272, 7500, 1, 30, 207, 159, NULL),
(117, '08:31:45', '08:33', 'P', '', '', 12827, 7, 212, 12600, 0.5, 15, 207, 159, NULL),
(118, '09:22:40', '09:25', 'WC', '', '', 15180, 5, 462, 14700, 0.2, 18, 208, 160, NULL),
(119, '09:25:42', '09:54', 'WF', '', '', 2109, 0, 0, 2100, 0.1, 9, 208, 160, NULL),
(120, '09:55:00', '09:58', 'WF', '', '', 3693, 3.5, 324, 3360, 0.1, 9, 208, 160, NULL),
(121, '', '12:28', '5', '', '', 7183, 13, 308, 6840, 1.5, 35, 209, 161, NULL),
(122, '12:34:12', '12:37', 'WP', '', '', 5877, 4.5, 207, 5670, 0, 0, 210, 162, NULL),
(123, '12:37:50', '12:41', 'WL', '', '', 10369, 6, 276, 10080, 0.3, 13, 210, 162, NULL),
(124, '12:42:43', '12:46', 'WL', '', '', 4874, 3, 138, 4725, 0.24, 11, 210, 162, NULL),
(125, '', '08:49', 'J', '', '', 20493, 4, 372, 20075, 0.5, 46, 212, 163, NULL),
(126, '', '18:29', 'J', '', '', 20660, 6, 558, 20075, 0.3, 27, 213, 165, NULL),
(127, '18:32:29', '18:36', 'W', '', '', 20946, 8, 740, 20160, 0.5, 46, 214, 166, NULL),
(128, '14:05:54', '14:14', 'P', '', '', 13390, 3, 279, 12925, 2, 186, 218, 169, NULL),
(129, '14:16:08', '15:21', 'P', '', '', 7828, 6.5, 604, 7150, 0.8, 74, 218, 169, NULL),
(130, '15:23:55', '15:51', 'H', '', '', 8113, 16, 692, 7400, 0.5, 21, 219, 170, NULL),
(131, '15:53:12', '16:03', 'H', '', '', 2120, 2.5, 108, 2000, 0.3, 12, 219, 170, NULL),
(132, '16:04:08', '16:12', 'H', '', '', 5937, 3, 129, 5800, 0.2, 8, 219, 170, NULL),
(133, '16:15:27', '16:22', 'K', '', '', 15528, 15, 454, 15050, 0.8, 24, 220, 171, NULL),
(134, '16:23:09', '16:28', 'L', '', '', 5396, 10, 303, 5075, 0.6, 18, 221, 171, NULL),
(135, '13:41:34', '14:21', 'T', '', '', 8990, 6.5, 137, 8760, 4.4, 93, 223, 172, NULL),
(136, '14:21:40', '14:31', 'R', '', '', 2610, 4, 84, 2520, 0.3, 6, 223, 172, NULL),
(137, '14:34:36', '15:12', 'E', '', '', 7019, 5, 294, 6720, 0.1, 5, 224, 173, NULL),
(138, '15:13:05', '15:15', 'J', '', '', 3488, 2, 117, 3360, 0.2, 11, 224, 173, NULL),
(139, '', '14:25', 'A', '', '', 3254, 8.5, 173, 3000, 4, 81, 226, 174, NULL),
(140, '09:00:16', '09:08:34', 'E', 'Si', '', 25032, 6.4, 100, 24378, 7.5, 200, 310, 250, 253),
(141, '13:45:03', '13:48:03', 'E', NULL, '', 8000, 6.8, 300, 6800, 7.3, 400, 310, 251, 254),
(142, '13:52:07', '13:56:15', 'M', 'Si', '', 101499, 8.7, 89, 100399, 7.4, 98, 313, 252, 255),
(143, '14:16:33', '14:19:54', 'L', 'Si', '', 20747, 0, 4, 20103, 0, 7, 313, 253, 256);
...
INSERT INTO `defecto1` (`id_defecto`, `registro_rbp_id_registro_rbp`, `razon_rechazo_id_razon_rechazo`, `cantidad_defecto`, `columna`) VALUES
(1, 31, 18, 9, NULL),
(2, 31, 19, 4, NULL),
(3, 31, 27, 10, NULL),
(4, 31, 29, 7, NULL),
...
INSERT INTO `lote_coil` (`id_lote_coil`, `registro_rbp_id_registro_rbp`, `lote_coil`, `metros`, `f_terminado`, `das_id_das`, `mog_id_mog`, `tiempo_insercion`, `ITEM`) VALUES
...
(225, 252, 'I2572001-31', 600, 1, NULL, NULL, NULL, ''),
(226, 252, 'I2572003-31', 700, 1, NULL, NULL, NULL, ''),
(227, 253, 'Y7185-17.7-TC', 600, 0, 310, NULL, NULL, 'A6041-19.0-TC'),
(228, 253, 'I2572002-31', 200, 1, 310, NULL, NULL, 'A6041-19.0-TC'),
(229, 254, 'Y7185-17.7-TC', 800, 1, 310, NULL, NULL, 'Y7700-17.6-TC'),
(230, 254, 'I2572011-31', 100, 0, 310, NULL, NULL, 'Y7700-17.6-TC'),
(231, 255, 'I2572101-31', 1000, 1, 313, NULL, NULL, 'A6041-20.2-TC'),
(232, 255, 'I2572031-31', 158, 0, 313, NULL, NULL, 'A6041-20.2-TC'),
(233, 256, 'I2573001-31', 400, 0, 313, NULL, NULL, 'Y7118-22.0-TC');
...
INSERT INTO `mog` (`id_mog`, `mog`, `descripcion`, `num_dibujo`, `no_parte`, `modelo`, `STD`, `cantidad_planeada`, `peso`, `largo`, `lote`) VALUES
...
(216, 'MOG083559', 'HB 122Y B/M STD 2', 'A-146209-1', '11911-F0010-02-TP', '122Y B/M STD 2', 'STD 2', 30000, 10.8, 1.1, 'LTM776J'),
...
(231, 'MOG047460', 'HB RNA (NP4)', 'A-170507-1', '13212RNAA010', 'RNA (NP4)', 'STD B', 20000, 12.83, 1.1, ''),
(232, 'MOG083560', 'HB 122Y B/M STD 2', 'A-146209-1', '11911-F0010-02', '122Y B/M STD 2', 'STD 2', 30000, 10.8, 1.1, ''),
(233, 'MOG083911', 'HB ISB6.7LC/RU STD', 'K-216194-02', '5663852', 'ISB6.7LC/RU STD', 'STD', 15000, 60.93, 1.1, ''),
(234, 'MOG083557', 'HB HB NP0 C/R STD D', 'A-171698-01', '13214-R70-D011-M1', 'HB NP0 C/R STD D', 'STD D', 20000, 17.47, 1.1, ''),
(235, 'MOG083376', 'HB I4MPCGTDICRUSTD2', 'A-292795-3-00', 'K2GE-6211-BC', 'I4MPCGTDICRUSTD2', 'STD 2', 20000, 16.37, 1.1, ''),
(236, 'MOG083912', 'HB HB NP0 C/R STD D', 'A-171698-01', '13214-R70-D011-M1', 'HB NP0 C/R STD D', 'STD D', 20000, 17.47, 1.1, ''),
(237, 'MOG083620', 'HB 5R0(AP2)C/RSTDC', 'A-170930-00', '13213-5R0-0230', '5R0(AP2)C/RSTDC', 'STD C', 20000, 10.3, 1.1, ''),
(238, 'MOG080708', 'HB I4MPCGTDICRUSTD2', 'A-292795-3-00', 'K2GE-6211-BC', 'I4MPCGTDICRUSTD2', 'STD 2', 20000, 16.37, 1.1, ''),
(239, 'MOG066270', 'HB XH C/R L STD 4', 'A-155994', '12111-5RB4A', 'XH C/R L STD 4', 'STD 4', 20000, 11.5, 1.1, ''),
(240, 'MOG083546', 'HB I4 C/R STD 1', 'A-291339-01', '8E5G6211AA', 'I4 C/R STD 1', 'STD 1', 20000, 16.4, 1.1, ''),
(241, 'MOG083547', 'HB I4 C/R STD 1', 'A-291339-01', '8E5G6211AA', 'I4 C/R STD 1', 'STD 1', 20000, 16.4, 1.1, ''),
(242, 'MOG083571', 'HB 130Y133YMNU STD4', 'A-146481', '11711-F2011-04', '130Y133YMNU STD4', 'STD 4', 18975, 23.1, 1.1, ''),
(243, 'MOG083580', 'HB I4MPCGTDICRUSTD3', 'A-292795-3-00', 'K2GE-6211-CC', 'I4MPCGTDICRUSTD3', 'STD 3', 20025, 16.37, 1.1, ''),
(244, 'MOG083581', 'HB I4MPCGTDICRUSTD3', 'A-292795-3-00', 'K2GE-6211-CC', 'I4MPCGTDICRUSTD3', 'STD 3', 7200, 16.37, 1.1, ''),
(245, 'MOG083542', 'HB AP4 M/N L STD D', 'A-170808-03', '13344-RPY-G021-M1', 'AP4 M/N L STD D', 'STD D', 20000, 28.2, 1.1, ''),
(246, 'MOG083407', 'HB CSS 675T B/C STD', 'A-223062-03', '12665019', 'CSS 675T B/C STD', 'STD', 30000, 9.4, 1.1, ''),
(247, 'MOG083569', 'HB 5R0(AP2)C/RSTDC', 'A-170930-00', '13213-5R0-0230', '5R0(AP2)C/RSTDC', 'STD C', 20000, 10.3, 1.1, ''),
(248, 'MOG083497', 'HB PENTASTARCR STD1', 'A-215253', '04893951AA', 'PENTASTARCR STD1', 'STD 1', 20000, 19.16, 1.1, ''),
(249, 'MOG083525', 'HB CSS M/N L STD W', 'A-223060-4', '12663477', 'CSS M/N L STD W', 'STD W', 20000, 30.8, 1.1, ''),
(250, 'MOG083521', 'HB HURRICANEMNUSTD2', 'A-215288-04', '04893522AD', 'HURRICANEMNUSTD2', 'STD 2', 20000, 29.5, 1.1, ''),
(251, 'MOG083520', 'HB 123Y C/R STD 2', 'A-146362', '13281-70031-2', '123Y C/R STD 2', 'STD 2', 1000, 17, 1.1, ''),
(252, 'MOG083502', 'HB I4GTDI MNL STD 4', 'A-291365-2', '9E5G-6A338-DA', 'I4GTDI MNL STD 4', 'STD 4', 20000, 32.81, 1.1, ''),
(253, 'MOG083504', 'HB COYOTE C/R L STD', 'A-292752-1', 'JR3E-6211-DA', 'COYOTE C/R L STD', 'STD', 20000, 27, 1.1, '');
...
--
-- Estructura de tabla para la tabla `piezas_procesadas_maquinado`
--

CREATE TABLE `piezas_procesadas_maquinado` (
  `id_piezas_procesadas_maquinado` int(10) UNSIGNED NOT NULL,
  `registro_rbp_id_registro_rbp` int(10) UNSIGNED NOT NULL,
  `das_id_das` int(10) NOT NULL DEFAULT 0,
  `linea` varchar(10) NOT NULL,
  `rango_canasta_1` int(10) UNSIGNED DEFAULT NULL,
  `rango_canasta_2` int(10) UNSIGNED DEFAULT NULL,
  `cantidad_piezas_procesadas` double DEFAULT NULL,
  ...
  `cambioMOG` int(11) NOT NULL,
  `activo` tinyint(1) DEFAULT NULL,
  `sobrante` int(11) DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Volcado de datos para la tabla `piezas_procesadas_maquinado`
--

INSERT INTO `piezas_procesadas_maquinado` (`id_piezas_procesadas_maquinado`, `registro_rbp_id_registro_rbp`, `das_id_das`, `linea`, `rango_canasta_1`, `rango_canasta_2`, `cantidad_piezas_procesadas`, `piezasxfila`, `filas`, `niveles`, `canastas`, `niveles_completos`, `filas_completas`, `cambioMOG`, `activo`, `sobrante`) VALUES
(115, 220, 309, 'TH17', 1, 3, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(116, 220, 309, 'TH17', 3, 5, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(117, 220, 309, 'TH17', 5, 7, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(118, 220, 309, 'TH17', 7, 9, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(119, 220, 309, 'TH17', 9, 11, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(120, 220, 309, 'TH17', 11, 13, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(121, 220, 309, 'TH17', 13, 15, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(122, 220, 312, 'TH17', 15, 17, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(123, 220, 312, 'TH17', 17, 19, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(124, 220, 312, 'TH17', 19, 21, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(125, 220, 312, 'TH17', 21, 23, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(126, 220, 312, 'TH17', 23, 25, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(127, 220, 312, 'TH17', 25, 27, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(128, 220, 312, 'TH17', 27, 29, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(129, 220, 312, 'TH17', 29, 31, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(130, 220, 312, 'TH17', 31, 33, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(131, 220, 312, 'TH17', 33, 35, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(132, 220, 312, 'TH17', 35, 37, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(133, 220, 312, 'TH17', 37, 39, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(134, 220, 312, 'TH17', 39, 41, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(135, 220, 312, 'TH17', 41, 43, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(136, 220, 312, 'TH17', 43, 45, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(137, 220, 312, 'TH17', 45, 47, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(138, 220, 312, 'TH17', 47, 49, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(139, 220, 312, 'TH17', 49, 51, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(140, 220, 312, 'TH17', 51, 53, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(141, 220, 312, 'TH17', 53, 55, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(142, 220, 312, 'TH17', 55, 57, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(143, 220, 312, 'TH17', 57, 59, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(144, 220, 312, 'TH17', 59, 61, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(145, 220, 312, 'TH17', 61, 63, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(146, 220, 312, 'TH17', 63, 65, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2),
(147, 220, 312, 'TH17', 65, 67, 16, 2, 2, 2, 2, 1, 1, 0, NULL, 2);

-- --------------------------------------------------------

--
-- Estructura de tabla para la tabla `piezas_procesadas_prensa`
--

CREATE TABLE `piezas_procesadas_prensa` (
  `idpiezas_procesadas` int(10) UNSIGNED NOT NULL,
  `registro_rbp_id_registro_rbp` int(10) UNSIGNED NOT NULL,
  `linea` varchar(10) NOT NULL,
  `cantidad_piezas_procesadas` double DEFAULT NULL,
  ...
  `activo` tinyint(1) DEFAULT NULL,
  `dasiddas` int(10) DEFAULT NULL,
  `pzas_bandeja_completa` int(11) NOT NULL,
  `numero_bandejas_completas` int(11) NOT NULL,
  `piezas_bandeja_incompleta` int(11) NOT NULL DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Volcado de datos para la tabla `piezas_procesadas_prensa`
--

INSERT INTO `piezas_procesadas_prensa` (`idpiezas_procesadas`, `registro_rbp_id_registro_rbp`, `linea`, `cantidad_piezas_procesadas`, `sobra_fin`, `activo`, `dasiddas`, `pzas_bandeja_completa`, `numero_bandejas_completas`, `piezas_bandeja_incompleta`) VALUES
(1, 1, 'TI30', 11239, NULL, NULL, NULL, 0, 0, 0),
(2, 3, 'TI30', 50416, NULL, NULL, NULL, 0, 0, 0),
(3, 4, 'TB02B', 2387, NULL, NULL, NULL, 0, 0, 0),
(4, 2, 'TB02F', 2519, NULL, NULL, NULL, 0, 0, 0),
(5, 8, 'TI30', 25857, NULL, NULL, NULL, 0, 0, 0),
(6, 4, 'TB02B', 814, NULL, NULL, NULL, 0, 0, 0),
(7, 2, 'TB02F', 3190, NULL, NULL, NULL, 0, 0, 0),
(8, 2, 'TB02F', 3190, NULL, NULL, NULL, 0, 0, 0),
(9, 2, 'TB02F', 3190, NULL, NULL, NULL, 0, 0, 0),
(10, 14, 'TB02F', 2380, NULL, NULL, NULL, 0, 0, 0),
(11, 15, 'TI30', 25200, NULL, NULL, NULL, 0, 0, 0),
(12, 12, 'TB02F', 12536, NULL, NULL, NULL, 0, 0, 0),
(13, 11, 'TB02B', 12536, NULL, NULL, NULL, 0, 0, 0),
(14, 16, 'TI30', 17142, NULL, NULL, NULL, 0, 0, 0),
(15, 17, 'TB02F', 1773, NULL, NULL, NULL, 0, 0, 0),
(16, 17, 'TB02F', 1073, NULL, NULL, NULL, 0, 0, 0),
(17, 17, 'TB02F', 700, NULL, NULL, NULL, 0, 0, 0),
(19, 25, 'TI30', 36507, NULL, NULL, NULL, 0, 0, 0),
(20, 25, 'TI30', 36507, NULL, NULL, NULL, 0, 0, 0),
(21, 26, 'TI30', 28523, NULL, NULL, NULL, 0, 0, 0),
(22, 21, 'TI30', 15000, NULL, NULL, NULL, 0, 0, 0),
(23, 23, 'TB02F', 12736, NULL, NULL, NULL, 0, 0, 0),
(24, 24, 'TB02B', 13487, NULL, NULL, NULL, 0, 0, 0),
(25, 28, 'TI30', 34321, NULL, NULL, NULL, 0, 0, 0),
(26, 31, 'TB02F', 1707, NULL, NULL, NULL, 0, 0, 0),
(27, 30, 'TB02B', 1955, NULL, NULL, NULL, 0, 0, 0),
(28, 30, 'TB02B', 13460, NULL, NULL, NULL, 0, 0, 0),
(29, 31, 'TB02F', 12047, NULL, NULL, NULL, 0, 0, 0),
(30, 30, 'TB02B', -61, NULL, NULL, NULL, 0, 0, 0),
(31, 31, 'TB02F', 12047, NULL, NULL, NULL, 0, 0, 0),
(32, 33, 'TI30', 6151, NULL, NULL, NULL, 0, 0, 0),
(33, 37, 'TI30', 24227, NULL, NULL, NULL, 0, 0, 0),
(34, 38, 'TI30', 16000, NULL, NULL, NULL, 0, 0, 0),
(35, 35, 'TB02F', 2725, NULL, NULL, NULL, 0, 0, 0),
(36, 34, 'TB02B', 3634, NULL, NULL, NULL, 0, 0, 0),
(37, 36, 'TB31', 2725, NULL, NULL, NULL, 0, 0, 0),
(38, 35, 'TB02F', 16058, NULL, NULL, NULL, 0, 0, 0),
(39, 35, 'TB02F', 16058, NULL, NULL, NULL, 0, 0, 0),
(40, 36, 'TB31', 16058, NULL, NULL, NULL, 0, 0, 0),
(41, 36, 'TB31', 16000, NULL, NULL, NULL, 0, 0, 0),
(42, 36, 'TB31', 16000, NULL, NULL, NULL, 0, 0, 0),
(43, 41, 'TI30', 24861, NULL, NULL, NULL, 0, 0, 0),
(44, 34, 'TB02B', 18730, NULL, NULL, NULL, 0, 0, 0),
(46, 34, 'TB02B', 6658, NULL, NULL, NULL, 0, 0, 0),
(47, 35, 'TB02F', 5366, NULL, NULL, NULL, 0, 0, 0),
(48, 35, 'TB02F', 5366, NULL, NULL, NULL, 0, 0, 0),
(49, 36, 'TB31', 5366, NULL, NULL, NULL, 0, 0, 0),
(50, 43, 'TB02B', 2811, NULL, NULL, NULL, 0, 0, 0),
(51, 43, 'TB02B', 3244, NULL, NULL, NULL, 0, 0, 0),
(52, 43, 'TB02B', 3244, NULL, NULL, NULL, 0, 0, 0),
(53, 45, 'TI30', 28589, NULL, NULL, NULL, 0, 0, 0),
(54, 45, 'TI30', 0, NULL, NULL, NULL, 0, 0, 0),
(55, 45, 'TI30', 0, NULL, NULL, NULL, 0, 0, 0),
(56, 50, 'TI30', 20899, NULL, NULL, NULL, 0, 0, 0),
(57, 43, 'TB02B', 25171, NULL, NULL, NULL, 0, 0, 0),
(58, 46, 'TB31', 24738, NULL, NULL, NULL, 0, 0, 0),
(59, 43, 'TB02B', 25171, NULL, NULL, NULL, 0, 0, 0),
(60, 44, 'TB02F', 2811, NULL, NULL, NULL, 0, 0, 0),
(61, 43, 'TB02B', 4135, NULL, NULL, NULL, 0, 0, 0),
(62, 44, 'TB02F', 28440, NULL, NULL, NULL, 0, 0, 0),
(63, 46, 'TB31', 2964, NULL, NULL, NULL, 0, 0, 0),
(64, 51, 'TB51', 8004, NULL, NULL, NULL, 0, 0, 0),
(65, 51, 'TB51', 8004, NULL, NULL, NULL, 0, 0, 0),
(67, 52, 'TB02B', 3366, NULL, NULL, NULL, 0, 0, 0),
(70, 53, 'TB02F', 10367, NULL, NULL, NULL, 0, 0, 0),
(71, 54, 'TB31', 0, NULL, NULL, NULL, 0, 0, 0),
(72, 54, 'TB31', 10101, NULL, NULL, NULL, 0, 0, 0),
(73, 64, 'TI30', 16157, NULL, NULL, NULL, 0, 0, 0),
(74, 64, 'TI30', 0, NULL, NULL, NULL, 0, 0, 0),
(75, 65, 'TI30', 26393, NULL, NULL, NULL, 0, 0, 0),
(76, 66, 'TI30', 32000, NULL, NULL, NULL, 0, 0, 0),
(77, 52, 'TB02B', 11025, NULL, NULL, NULL, 0, 0, 0),
(78, 52, 'TB02B', 734, NULL, NULL, NULL, 0, 0, 0),
(79, 52, 'TB02B', -251, NULL, NULL, NULL, 0, 0, 0),
(80, 52, 'TB02B', 0, NULL, NULL, NULL, 0, 0, 0),
(81, 52, 'TB02B', 0, NULL, NULL, NULL, 0, 0, 0),
(82, 53, 'TB02F', 674, NULL, NULL, NULL, 0, 0, 0),
(83, 54, 'TB31', 674, NULL, NULL, NULL, 0, 0, 0),
(84, 67, 'TI30', 27599, NULL, NULL, NULL, 0, 0, 0),
(85, 70, 'TI30', 7135, NULL, NULL, NULL, 0, 0, 0),
(86, 68, 'TB02B', 9376, NULL, NULL, NULL, 0, 0, 0),
(87, 68, 'TB02B', 8843, NULL, NULL, NULL, 0, 0, 0),
(88, 73, 'TI30', 1323, NULL, NULL, NULL, 0, 0, 0),
(89, 69, 'TB02F', 16237, NULL, NULL, NULL, 0, 0, 0),
(90, 68, 'TB02B', 0, NULL, NULL, NULL, 0, 0, 0),
(91, 71, 'TB31', 16237, NULL, NULL, NULL, 0, 0, 0),
(92, 75, 'TB02F', 2631, NULL, NULL, NULL, 0, 0, 0),
(93, 76, 'TB31', 2631, NULL, NULL, NULL, 0, 0, 0),
(94, 57, 'TI30', 8212, NULL, NULL, NULL, 0, 0, 0),
(95, 74, 'TB02B', 15537, NULL, NULL, NULL, 0, 0, 0),
(96, 76, 'TB31', 13535, NULL, NULL, NULL, 0, 0, 0),
(98, 79, 'TI30', 5100, NULL, NULL, NULL, 0, 0, 0),
(99, 77, 'TB71', 2810, NULL, NULL, NULL, 0, 0, 0),
(101, 83, 'TB02F', 5000, NULL, NULL, NULL, 0, 0, 0),
(102, 84, 'TB02B', 1000, NULL, NULL, NULL, 0, 0, 0),
(103, 77, 'TB71', 5052, NULL, NULL, NULL, 0, 0, 0),
(104, 77, 'TB71', 5389, NULL, NULL, NULL, 0, 0, 0),
(105, 81, 'TB02F', 7566, NULL, NULL, NULL, 0, 0, 0),
(106, 82, 'TB31', 7566, NULL, NULL, NULL, 0, 0, 0),
(107, 81, 'TB02F', 5705, NULL, NULL, NULL, 0, 0, 0),
(108, 82, 'TB31', 5139, NULL, NULL, NULL, 0, 0, 0),
(109, 80, 'TB02B', 15468, NULL, NULL, NULL, 0, 0, 0),
(110, 77, 'TB71', -210, NULL, NULL, NULL, 0, 0, 0),
(111, 86, 'TB02F', 0, NULL, NULL, NULL, 0, 0, 0),
(112, 85, 'TB02B', 10700, NULL, NULL, NULL, 0, 0, 0),
(113, 93, 'TI30', 25386, NULL, NULL, NULL, 0, 0, 0),
(114, 86, 'TB02F', 9692, NULL, NULL, NULL, 0, 0, 0),
(115, 92, 'TB71', 7020, NULL, NULL, NULL, 0, 0, 0),
(116, 89, 'TB31', 9692, NULL, NULL, NULL, 0, 0, 0),
(117, 92, 'TB71', 7020, NULL, NULL, NULL, 0, 0, 0),
(118, 86, 'TB02F', 6138, NULL, NULL, NULL, 0, 0, 0),
(119, 89, 'TB31', 6138, NULL, NULL, NULL, 0, 0, 0),
(120, 95, 'TI30', 32188, NULL, NULL, NULL, 0, 0, 0),
(121, 97, 'TB02F', 12783, NULL, NULL, NULL, 0, 0, 0),
(122, 100, 'TB31', 12783, NULL, NULL, NULL, 0, 0, 0),
(123, 96, 'TB02B', 7607, NULL, NULL, NULL, 0, 0, 0),
(124, 96, 'TB02B', 7607, NULL, NULL, NULL, 0, 0, 0),
(125, 97, 'TB02F', 6824, NULL, NULL, NULL, 0, 0, 0),
(126, 100, 'TB31', 6824, NULL, NULL, NULL, 0, 0, 0),
(127, 101, 'TI30', 40043, NULL, NULL, NULL, 0, 0, 0),
(128, 99, 'TB71', 2055, NULL, NULL, NULL, 0, 0, 0),
(129, 99, 'TB71', 7066, NULL, NULL, NULL, 0, 0, 0),
(130, 99, 'TB71', 7045, NULL, NULL, NULL, 0, 0, 0),
(131, 102, 'TB02B', 12040, NULL, NULL, NULL, 0, 0, 0),
(132, 103, 'TB02F', 10892, NULL, NULL, NULL, 0, 0, 0),
(133, 102, 'TB02B', 142, NULL, NULL, NULL, 0, 0, 0),
(134, 103, 'TB02F', 99, NULL, NULL, NULL, 0, 0, 0),
(135, 102, 'TB02B', -191, NULL, NULL, NULL, 0, 0, 0),
(136, 105, 'TI30', 7310, NULL, NULL, NULL, 0, 0, 0),
(137, 109, 'TI30', 24729, NULL, NULL, NULL, 0, 0, 0),
(138, 108, 'TB02B', 8759, NULL, NULL, NULL, 0, 0, 0),
(139, 108, 'TB02B', -39, NULL, NULL, NULL, 0, 0, 0),
(140, 110, 'TI30', 15000, NULL, NULL, NULL, 0, 0, 0),
(141, 106, 'TB71', 9170, NULL, NULL, NULL, 0, 0, 0),
(142, 106, 'TB71', 4101, NULL, NULL, NULL, 0, 0, 0),
(144, 111, 'TB02B', 12690, NULL, NULL, NULL, 0, 0, 0),
(145, 112, 'TB02F', 11605, NULL, NULL, NULL, 0, 0, 0),
(146, 114, 'TB31', 11605, NULL, NULL, NULL, 0, 0, 0),
(147, 90, 'TB71', 5437, NULL, NULL, NULL, 0, 0, 0),
(148, 90, 'TB71', 8213, NULL, NULL, NULL, 0, 0, 0),
(149, 90, 'TB71', 2180, NULL, NULL, NULL, 0, 0, 0),
(150, 117, 'TI30', 14049, NULL, NULL, NULL, 0, 0, 0),
(151, 117, 'TI30', 14049, NULL, NULL, NULL, 0, 0, 0),
(152, 117, 'TI30', 14049, NULL, NULL, NULL, 0, 0, 0),
(153, 78, 'TB51', 11497, NULL, NULL, NULL, 0, 0, 0),
(154, 118, 'TB71', 7422, NULL, NULL, NULL, 0, 0, 0),
(155, 118, 'TB71', 3569, NULL, NULL, NULL, 0, 0, 0),
(156, 120, 'TB71', 6727, NULL, NULL, NULL, 0, 0, 0),
(157, 120, 'TB71', 1105, NULL, NULL, NULL, 0, 0, 0),
(158, 111, 'TB02B', 18613, NULL, NULL, NULL, 0, 0, 0),
(159, 112, 'TB02F', 17614, NULL, NULL, NULL, 0, 0, 0),
(160, 114, 'TB31', 17614, NULL, NULL, NULL, 0, 0, 0),
(161, 123, 'TI30', 34643, NULL, NULL, NULL, 0, 0, 0),
(162, 126, 'TI30', 35948, NULL, NULL, NULL, 0, 0, 0),
(163, 127, 'TI30', 7319, NULL, NULL, NULL, 0, 0, 0),
(164, 128, 'TI30', 3120, NULL, NULL, NULL, 0, 0, 0),
(165, 124, 'TB02B', 20580, NULL, NULL, NULL, 0, 0, 0),
(166, 119, 'TB51', 4840, NULL, NULL, NULL, 0, 0, 0),
(167, 125, 'TB02F', 7991, NULL, NULL, NULL, 0, 0, 0),
(168, 129, 'TB31', 7991, NULL, NULL, NULL, 0, 0, 0),
(169, 130, 'TI30', 6829, 97, NULL, NULL, 0, 0, 0),
(170, 124, 'TB02B', 12259, NULL, NULL, NULL, 0, 0, 0),
(171, 125, 'TB02F', 30140, NULL, NULL, NULL, 0, 0, 0),
(172, 129, 'TB31', 30140, NULL, NULL, NULL, 0, 0, 0),
(173, 131, 'TB02B', 11915, NULL, NULL, NULL, 0, 0, 0),
(174, 132, 'TB02F', 0, NULL, NULL, NULL, 0, 0, 0),
(175, 131, 'TB02B', 2363, NULL, NULL, NULL, 0, 0, 0),
(176, 131, 'TB02B', -462, NULL, NULL, NULL, 0, 0, 0),
(177, 132, 'TB02F', 11000, NULL, NULL, NULL, 0, 0, 0),
(178, 132, 'TB02F', 2262, NULL, NULL, NULL, 0, 0, 0),
(179, 133, 'TI30', 25002, NULL, NULL, NULL, 0, 0, 0),
(180, 136, 'TI30', 37386, NULL, NULL, NULL, 0, 0, 0),
(181, 137, 'TB71', 5600, NULL, NULL, NULL, 0, 0, 0),
(182, 137, 'TB71', 7662, NULL, NULL, NULL, 0, 0, 0),
(183, 135, 'TB02F', 10022, NULL, NULL, NULL, 0, 0, 0),
(184, 134, 'TB02B', 10879, NULL, NULL, NULL, 0, 0, 0),
(185, 134, 'TB02B', 3481, NULL, NULL, NULL, 0, 0, 0),
(186, 135, 'TB02F', 3282, NULL, NULL, NULL, 0, 0, 0),
(187, 137, 'TB71', 0, NULL, NULL, NULL, 0, 0, 0),
(188, 139, 'TB02F', 13434, NULL, NULL, NULL, 0, 0, 0),
(189, 138, 'TB02B', 15024, NULL, NULL, NULL, 0, 0, 0),
(190, 140, 'TB71', 1800, NULL, NULL, NULL, 0, 0, 0),
(191, 143, 'TI30', 28581, NULL, NULL, NULL, 0, 0, 0),
(192, 141, 'TB02B', 15007, NULL, NULL, NULL, 0, 0, 0),
(193, 142, 'TB02F', 13231, NULL, NULL, NULL, 0, 0, 0),
(194, 144, 'TI30', 6000, NULL, NULL, NULL, 0, 0, 0),
(195, 140, 'TB71', 11504, NULL, NULL, NULL, 0, 0, 0),
(196, 147, 'TB71', 5910, NULL, NULL, NULL, 0, 0, 0),
(197, 145, 'TB02B', 12413, NULL, NULL, NULL, 0, 0, 0),
(198, 146, 'TB02F', 10353, NULL, NULL, NULL, 0, 0, 0),
(199, 148, 'TB02B', 8523, NULL, NULL, NULL, 0, 0, 0),
(200, 149, 'TB02F', 7614, NULL, NULL, NULL, 0, 0, 0),
(201, 147, 'TB71', 7524, NULL, NULL, NULL, 0, 0, 0),
(202, 155, 'TI30', 4101, NULL, NULL, NULL, 0, 0, 0),
(205, 150, 'TB71', 9269, NULL, NULL, NULL, 0, 0, 0),
(206, 150, 'TB71', 3158, NULL, NULL, NULL, 0, 0, 0),
(207, 176, 'TI30', 4908, NULL, NULL, NULL, 0, 0, 0),
(208, 170, 'TB71', 6000, NULL, NULL, NULL, 0, 0, 0),
(209, 170, 'TB71', 9365, NULL, NULL, NULL, 0, 0, 0),
(210, 172, 'TB71', 3144, NULL, NULL, NULL, 0, 0, 0),
(211, 172, 'TB71', 4334, NULL, NULL, NULL, 0, 0, 0),
(212, 171, 'TB51', 2000, NULL, NULL, NULL, 0, 0, 0),
(213, 171, 'TB51', 8305, NULL, NULL, NULL, 0, 0, 0),
(214, 177, 'TI30', 5482, NULL, NULL, NULL, 0, 0, 0),
(215, 179, 'TI30', 8544, NULL, NULL, NULL, 0, 0, 0),
(216, 173, 'TB51', 7603, NULL, NULL, NULL, 0, 0, 0),
(217, 177, 'TI30', 843, NULL, NULL, NULL, 0, 0, 0),
(223, 180, 'TI30', 7033, NULL, NULL, NULL, 0, 0, 0),
(225, 151, 'TB02B', 2844, NULL, NULL, NULL, 0, 0, 0),
(226, 152, 'TB02F', 1750, NULL, NULL, NULL, 0, 0, 0),
(227, 153, 'TB31', 1750, NULL, NULL, NULL, 0, 0, 0),
(228, 157, 'TB51', 5274, NULL, NULL, NULL, 0, 0, 0),
(229, 181, 'TI30', 10263, NULL, NULL, NULL, 0, 0, 0),
(230, 151, 'TB02B', 1936, NULL, NULL, NULL, 0, 0, 0),
(231, 182, 'TI30', 6037, NULL, NULL, NULL, 0, 0, 0),
(232, 152, 'TB02F', 1401, NULL, NULL, NULL, 0, 0, 0),
(233, 153, 'TB31', 1401, NULL, NULL, NULL, 0, 0, 0),
(234, 157, 'TB51', 12808, NULL, NULL, NULL, 0, 0, 0),
(235, 25, 'TI30', 10006, NULL, NULL, NULL, 0, 0, 0),
(236, 25, 'TI30', 1007, NULL, NULL, NULL, 0, 0, 0),
(237, 185, 'TI30', 2004, NULL, NULL, NULL, 0, 0, 0),
(238, 185, 'TI30', 6006, NULL, NULL, NULL, 0, 0, 0),
(239, 189, 'TI30', 7544, NULL, NULL, NULL, 0, 0, 0),
(240, 189, 'TI30', 0, NULL, NULL, NULL, 0, 0, 0),
(241, 190, 'TI30', 1684, NULL, NULL, NULL, 0, 0, 0),
(242, 190, 'TI30', 4152, NULL, NULL, NULL, 0, 0, 0),
(243, 191, 'TI30', 30115, NULL, NULL, NULL, 0, 0, 0),
(244, 192, 'TI30', 54713, NULL, NULL, NULL, 0, 0, 0),
(245, 193, 'TI30', 48342, NULL, NULL, NULL, 0, 0, 0),
(246, 156, 'TB51', 13414, NULL, NULL, NULL, 0, 0, 0),
(247, 183, 'TB02B', 7440, NULL, NULL, NULL, 0, 0, 0),
(248, 184, 'TB02F', 6731, NULL, NULL, NULL, 0, 0, 0),
(249, 186, 'TB31', 6731, NULL, NULL, NULL, 0, 0, 0),
(250, 183, 'TB02B', 11890, NULL, NULL, NULL, 0, 0, 0),
(251, 184, 'TB02F', 9777, NULL, NULL, NULL, 0, 0, 0),
(252, 186, 'TB31', 9777, NULL, NULL, NULL, 0, 0, 0),
(253, 154, 'TB71', 1246, NULL, NULL, NULL, 0, 0, 0),
(254, 154, 'TB71', 1905, NULL, NULL, NULL, 0, 0, 0),
(255, 199, 'TI30', 33349, NULL, NULL, NULL, 0, 0, 0),
(256, 198, 'TB51', 13216, NULL, NULL, NULL, 0, 0, 0),
(257, 198, 'TB51', 12800, NULL, NULL, NULL, 0, 0, 0),
(258, 198, 'TB51', 12800, NULL, NULL, NULL, 0, 0, 0),
(259, 198, 'TB51', 12800, NULL, NULL, NULL, 0, 0, 0),
(260, 198, 'TB51', 0, NULL, NULL, NULL, 0, 0, 0),
(261, 200, 'TI30', 5144, NULL, NULL, NULL, 0, 0, 0),
(262, 194, 'TB02B', 10597, NULL, NULL, NULL, 0, 0, 0),
(263, 195, 'TB02F', 9090, NULL, NULL, NULL, 0, 0, 0),
(264, 196, 'TB31', 9090, NULL, NULL, NULL, 0, 0, 0),
(265, 201, 'TI30', 36337, NULL, NULL, NULL, 0, 0, 0),
(266, 202, 'TI30', 32106, NULL, NULL, NULL, 0, 0, 0),
(267, 194, 'TB02B', 8742, NULL, NULL, NULL, 0, 0, 0),
(268, 195, 'TB02F', 7361, NULL, NULL, NULL, 0, 0, 0),
(269, 196, 'TB31', 7361, NULL, NULL, NULL, 0, 0, 0),
(270, 203, 'TI30', 40205, NULL, NULL, NULL, 0, 0, 0),
(271, 205, 'TB02F', 0, NULL, NULL, NULL, 0, 0, 0),
(272, 206, 'TB31', 0, NULL, NULL, NULL, 0, 0, 0),
(273, 204, 'TB02B', 17528, NULL, NULL, NULL, 0, 0, 0),
(274, 204, 'TB02B', 3219, NULL, NULL, NULL, 0, 0, 0),
(275, 204, 'TB02B', 0, NULL, NULL, NULL, 0, 0, 0),
(276, 187, 'TB71', 8724, NULL, NULL, NULL, 0, 0, 0),
(277, 187, 'TB71', 7784, NULL, NULL, NULL, 0, 0, 0),
(279, 205, 'TB02F', 15488, NULL, NULL, NULL, 0, 0, 0),
(280, 206, 'TB31', 15488, NULL, NULL, NULL, 0, 0, 0),
(281, 204, 'TB02B', 0, NULL, NULL, NULL, 0, 0, 0),
(282, 197, 'TB71', 258, NULL, NULL, NULL, 0, 0, 0),
(283, 204, 'TB02B', 0, NULL, NULL, NULL, 0, 0, 0),
(284, 205, 'TB02F', 2306, NULL, NULL, NULL, 0, 0, 0),
(285, 206, 'TB31', 2306, NULL, NULL, NULL, 0, 0, 0),
(286, 210, 'TI30', 36081, NULL, NULL, NULL, 0, 0, 0),
(287, 217, 'TB51', 4536, NULL, NULL, NULL, 0, 0, 0),
(288, 216, 'TI30', 32673, NULL, NULL, NULL, 0, 0, 0),
(289, 219, 'TI30', 300, NULL, NULL, NULL, 0, 0, 0),
(290, 211, 'TB02B', 8827, NULL, NULL, NULL, 0, 0, 0),
(291, 212, 'TB02F', 7779, NULL, NULL, NULL, 0, 0, 0),
(292, 213, 'TB31', 7779, NULL, NULL, NULL, 0, 0, 0),
(293, 197, 'TB71', 6860, NULL, NULL, NULL, 0, 0, 0),
(294, 218, 'TB51', 4454, NULL, NULL, NULL, 0, 0, 0),
(295, 211, 'TB02B', 8791, NULL, NULL, NULL, 0, 0, 0),
(297, 212, 'TB02F', 7932, NULL, NULL, NULL, 0, 0, 0),
(298, 213, 'TB31', 7932, NULL, NULL, NULL, 0, 0, 0),
(299, 197, 'TB71', 9333, NULL, NULL, NULL, 0, 0, 0),
(300, 197, 'TB71', -382, NULL, NULL, NULL, 0, 0, 0),
(301, 224, 'TB71', 10000, NULL, NULL, NULL, 0, 0, 0),
(302, 221, 'TB02B', 17999, NULL, NULL, NULL, 0, 0, 0),
(303, 223, 'TB02F', 16069, NULL, NULL, NULL, 0, 0, 0),
(304, 222, 'TB31', 16069, NULL, NULL, NULL, 0, 0, 0),
(305, 207, 'TB71', 7761, NULL, NULL, NULL, 0, 0, 0),
(306, 225, 'TB02B', 15992, NULL, NULL, NULL, 0, 0, 0),
(307, 226, 'TB02F', 14188, NULL, NULL, NULL, 0, 0, 0),
(308, 227, 'TB31', 14188, NULL, NULL, NULL, 0, 0, 0),
(309, 207, 'TB71', 6400, NULL, NULL, NULL, 0, 0, 0),
(310, 228, 'TI30', 28707, NULL, NULL, NULL, 0, 0, 0),
(311, 207, 'TB71', 3229, NULL, NULL, NULL, 0, 0, 0),
(312, 225, 'TB02B', 5613, NULL, NULL, NULL, 0, 0, 0),
(313, 226, 'TB02F', 5078, NULL, NULL, NULL, 0, 0, 0),
(314, 226, 'TB02F', -66, NULL, NULL, NULL, 0, 0, 0),
(315, 227, 'TB31', 5078, NULL, NULL, NULL, 0, 0, 0),
(316, 214, 'TB71', 6551, NULL, NULL, NULL, 0, 0, 0),
(317, 230, 'TB02B', 0, NULL, NULL, NULL, 0, 0, 0),
(318, 230, 'TB02B', 0, NULL, NULL, NULL, 0, 0, 0),
(319, 231, 'TB02F', 0, NULL, NULL, NULL, 0, 0, 0),
(320, 230, 'TB02B', 0, NULL, NULL, NULL, 0, 0, 0),
(321, 232, 'TI30', 39234, NULL, NULL, NULL, 0, 0, 0),
(322, 214, 'TB71', 9168, NULL, NULL, NULL, 0, 0, 0),
(323, 233, 'TI30', 35255, NULL, NULL, NULL, 0, 0, 0),
(324, 234, 'TI30', 21328, NULL, NULL, NULL, 0, 0, 0),
(325, 236, 'TI30', 33916, NULL, NULL, NULL, 0, 0, 0),
(326, 230, 'TB02B', 2968, NULL, NULL, NULL, 0, 0, 0),
(327, 231, 'TB02F', 2200, NULL, NULL, NULL, 0, 0, 0),
(328, 237, 'TI30', 5244, NULL, NULL, NULL, 0, 0, 0),
(329, 235, 'TB71', 15588, NULL, NULL, NULL, 0, 0, 0),
(330, 238, 'TI30', 5105, NULL, NULL, NULL, 0, 0, 0),
(331, 242, 'TI30', 3055, NULL, NULL, NULL, 0, 0, 0),
(332, 240, 'TI30', 19534, NULL, NULL, NULL, 0, 0, 0),
(333, 243, 'TI30', 34126, NULL, NULL, NULL, 0, 0, 0),
(334, 241, 'TB71', 19266, NULL, NULL, NULL, 0, 0, 0),
(335, 208, 'TB51', 17264, NULL, NULL, NULL, 0, 0, 0),
(336, 215, 'TB51', 15693, NULL, NULL, NULL, 0, 0, 0),
(337, 244, 'TI30', 35356, NULL, NULL, NULL, 0, 0, 0),
(338, 245, 'TB51', 8861, NULL, NULL, NULL, 0, 0, 0),
(339, 245, 'TB51', 7190, NULL, NULL, NULL, 0, 0, 0),
(340, 246, 'TB51', 19084, NULL, NULL, NULL, 0, 0, 0),
(341, 247, 'TI30', 7752, NULL, NULL, NULL, 0, 0, 0),
(342, 249, 'TI30', 33832, NULL, NULL, NULL, 0, 0, 0),
(343, 230, 'TB02B', 8100, NULL, NULL, NULL, 0, 0, 0),
(344, 231, 'TB02F', 6036, NULL, NULL, NULL, 0, 0, 0),
(345, 252, 'TB02B', 3321, NULL, NULL, NULL, 0, 0, 0),
(346, 253, 'TB02F', 2916, NULL, NULL, NULL, 0, 0, 0),
(347, 248, 'TB51', 0, NULL, NULL, NULL, 0, 0, 0),
(348, 252, 'TB02B', 9479, NULL, NULL, NULL, 0, 0, 0),
(349, 253, 'TB02F', 8682, NULL, NULL, NULL, 0, 0, 0),
(352, 255, 'TB71', 2800, NULL, NULL, NULL, 0, 0, 0),
(361, 248, 'TB51', 2398, NULL, NULL, NULL, 0, 0, 0),
(362, 255, 'TB71', 5145, NULL, NULL, NULL, 0, 0, 0),
(363, 255, 'TB71', 291, NULL, NULL, NULL, 0, 0, 0),
(364, 248, 'TB51', 4996, NULL, NULL, NULL, 0, 0, 0),
(576, 252, 'TP08', NULL, NULL, NULL, NULL, 70, 20, 69),
(577, 252, 'TP08', NULL, NULL, NULL, NULL, 400, 300, 3),
(578, 253, 'TP08', NULL, NULL, NULL, NULL, 600, 40, 378),
(579, 254, 'TP08', NULL, NULL, NULL, NULL, 1500, 4, 800),
(580, 254, 'TP08', NULL, NULL, NULL, NULL, 120, 400, 100),
(581, 255, 'TP08', NULL, NULL, NULL, NULL, 500, 200, 399),
(582, 256, 'TP08', NULL, NULL, NULL, NULL, 200, 100, 103);
...
CREATE TABLE `razon_rechazo1` (
  ...
  `subcategoria_rechazo_id_subcategoria_rechazo` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `razon_rechazo1` (`id_razon_rechazo`, `categoria_rechazo_id_categoria_rechazo`, `nombre_rechazo`, `subcategoria_rechazo_id_subcategoria_rechazo`) VALUES
(1, 1, 'Desprendimiento de material', 0),
(2, 1, 'Rayón', 0),
(3, 1, 'Contaminación', 0),
(4, 1, 'Óxido', 0),
(5, 1, 'Otros', 0),
(6, 1, 'Rayón espalda', 0),
(7, 1, 'Rayón aleacion', 0),
(8, 1, 'Otros', 0),
(9, 1, 'Rayón espalda', 0),
(10, 1, 'Rayón aleación', 0),
(11, 1, 'Espesor', 0),
(12, 1, 'Otros', 0),
(13, 1, 'Rayón espalda', 0),
(14, 1, 'Golpe espalda', 0),
(15, 1, 'Dimensiones', 0),
(16, 1, 'Extensión', 0),
(17, 1, 'Deformidad', 0),
(18, 1, 'Torsión', 0),
(19, 1, 'Rebaba', 0),
(20, 1, 'Sello fuera de posición', 0),
(21, 1, 'Sello incompleto', 0),
(22, 1, 'Sello desvanecido', 0),
(23, 1, 'Área de contacto', 0),
(24, 1, 'Falla de alimentador', 0),
(25, 1, 'Bad mark', 0),
(26, 1, 'Óxido', 0),
(27, 1, 'Piezas caídas', 0),
(28, 1, 'Dandori', 0),
(29, 1, 'Inspección de calidad', 0),
(30, 1, 'Otros', 0),
(31, 1, 'Alimentador', 0),
(32, 1, 'Acabado de ancho', 0),
(33, 1, 'Verificador de ancho', 0),
(34, 1, 'Corte de esquinas', 0),
(35, 1, 'Perforación de orificio', 0),
(36, 1, 'Alivio de uña', 0),
(37, 1, 'Expulsión de uña', 0),
(38, 1, 'Canal', 0),
(39, 1, 'Chaflán de orificio', 0),
(40, 1, 'Cepillado', 0),
(41, 1, 'Corte de altura', 0),
(42, 1, 'Autochecador - altura', 0),
(43, 1, 'Autochecador - proceso', 0),
(44, 1, 'Pokayoke boring / broach', 0),
(45, 1, 'Espesor', 0),
(46, 1, 'Golpe cara interna', 0),
(47, 1, 'Daño en chaflán interno', 0),
(48, 1, 'Golpe chaflán ancho', 0),
(49, 1, 'Golpe en chaflan de canal', 0),
(50, 1, 'Golpe chaflán altura', 0),
(51, 1, 'Golpe espalda', 0),
(52, 1, 'Rebaba en perforación', 0),
(53, 1, 'Corte irregular de canal', 0),
(54, 1, 'Rebaba en orificio', 0),
(55, 1, 'Rebaba en uña', 0),
(56, 1, 'Rebaba en canal', 0),
(57, 1, 'Rayón cara interna', 0),
(58, 1, 'Rayón espalda', 0),
(59, 1, 'Rayón cara lateral', 0),
(60, 1, 'Óxido', 0),
(61, 1, 'Mal manejo', 0),
(62, 1, 'Dandori', 0),
(63, 1, 'Inspección de calidad', 0),
(64, 1, 'Otros', 0),
(65, 1, 'Desprendimiento de recubrimiento', 0),
(66, 1, 'Corriente anormal', 0),
(67, 1, 'Espesor', 0),
(68, 1, 'Corrosión electrolítica', 0),
(69, 1, 'Puntos negros', 0),
(70, 1, 'Adhesión de contaminante', 0),
(71, 1, 'Recubrimiento en espalda', 0),
(72, 1, 'Recubrimiento incompleto', 0),
(73, 1, 'Colapso', 0),
(74, 1, 'Rayón cara interna', 0),
(75, 1, 'Rayón espalda', 0),
(76, 1, 'Falla en el equipo', 0),
(77, 1, 'Óxido', 0),
(78, 1, 'Piezas caídas', 0),
(79, 1, 'Inspección de calidad', 0),
(80, 1, 'Burbujas', 0),
(81, 1, 'Blister', 0),
(82, 1, 'Sobrante', 0),
(83, 1, 'Daño por lavadora', 0),
(84, 1, 'Otros', 0),
(85, 1, 'Mal manejo', 0),
(86, 1, 'Rayón de empalme', 0),
(87, 1, 'Rayón por tesa', 0),
(88, 1, 'Desprendimiento de pintura', 0),
(89, 1, 'Pintura fuera de especificación', 0),
(90, 1, 'Rayón por falla de flujo de máquina', 0),
(91, 1, 'Óxido', 0),
(92, 1, 'Piezas caídas', 0),
(93, 1, 'Dandori', 0),
(94, 1, 'Inspección de calidad', 0),
(95, 1, 'Desprendimiento de impresión', 0),
(96, 1, 'Falta de carácter o 2D matriz', 0),
(97, 1, 'Impresión fuera de especificación', 0),
(98, 1, 'Otros', 0),
(99, 1, 'Después de empacar', 0),
(100, 1, 'Inspeccion de calidad', 0),
(101, 1, 'Otros', 0),
(102, 1, 'Bad Mark', 0),
(103, 1, 'Burbuja en polímero', 0),
(104, 1, 'Oxido bimetal', 0),
(105, 1, 'Poro en aleación', 0),
(106, 1, 'Rayón aleación', 0),
(107, 1, 'Rayón espalda', 0),
(108, 1, 'Scrap Cambio Rollo', 0),
(109, 1, 'Variación espesor', 0),
(110, 1, 'Hege', 0),
(111, 1, 'Rayón espalda', 0),
(112, 1, 'Rayón aleación', 0),
(113, 1, 'Otros', 0),
(114, 1, 'Chaflanes fuera de especificación', 0),
(115, 1, 'Mal corte de chaflán', 0),
(116, 1, 'Rebaba en polimero', 0),
(117, 1, 'Rebaba en chaflán', 0),
(118, 1, 'Contaminación aleación', 0),
(119, 1, 'Rayón espalda', 0),
(120, 1, 'Rayón aleación', 0),
(121, 1, 'Espesor', 0),
(122, 1, 'Otros', 0),
(123, 1, 'Ajuste de navajas', 0),
(124, 1, 'Ajustes de dandori', 0),
(125, 1, 'Ajustes de ancho', 0),
(126, 1, 'Canales', 0),
(127, 1, 'Chaflán fuera de especificación', 0),
(128, 1, 'Deformidad', 0),
(129, 1, 'Diámetro exterior', 0),
(130, 1, 'Diámetro interior', 0),
(131, 1, 'Espesor fuera especificación', 0),
(132, 1, 'Golpe en chaflán', 0),
(133, 1, 'Mal Corte chaflán', 0),
(134, 1, 'Mal corte en unión', 0),
(135, 1, 'Mal pulido', 0),
(136, 1, 'Marca de rebaba aleación', 0),
(137, 1, 'Marca de rebaba espalda', 0),
(138, 1, 'Muesca fuera especificación', 0),
(139, 1, 'Óxido', 0),
(140, 1, 'Piezas caídas', 0),
(141, 1, 'Piezas de inspección', 0),
(142, 1, 'Piezas iniciales (30 Piezas)', 0),
(143, 1, 'Piezas unidas (Tipo Cadena)', 0),
(144, 1, 'Rayón en superficie externa', 0),
(145, 1, 'Rayón en superficie Interna', 0),
(146, 1, 'Rebaba en chaflán', 0),
(147, 1, 'Rebaba en orificio', 0),
(148, 1, 'Rebaba polímero', 0),
(149, 1, 'Sello fuera posición', 0),
(150, 1, 'Sello desvanecido', 0),
(151, 1, 'Sello incompleto', 0),
(152, 1, 'Unión fuera de especificación', 0),
(153, 1, 'Otros', 0),
(154, 1, 'Recubrimiento interior', 0),
(155, 1, 'Falla de barril', 0),
(156, 1, 'Óxido', 0),
(157, 1, 'Piezas caídas', 0),
(158, 1, 'Inspección de calidad', 0),
(159, 1, 'Daño por lavadora', 0),
(160, 1, 'Otros', 0),
(161, 1, 'Óxido', 0),
(162, 1, 'Piezas caídas', 0),
(163, 1, 'Inspección de calidad', 0),
(164, 1, 'Otros', 0),
(165, 1, 'Después de empacar', 0),
(166, 1, 'Inspección de calidad', 0),
(167, 1, 'Otros', 0),
(168, 1, 'Marca por ensamble', 0),
(169, 1, 'R/G golpeado', 0),
(170, 1, 'Rechazo fuera de especificación', 0),
(171, 1, 'Otros', 0),
(172, 1, 'Sello defectuoso', 0),
(173, 1, 'Falla después de ensamble', 0),
(174, 2, 'Rayón espalda', 3),
(175, 2, 'Rayón aleación', 3),
(176, 2, 'Marca cortadores', 3),
(177, 2, 'Otros', 3),
(180, 2, 'Mal corte de chaflán', 2),
(181, 2, 'Mal corte de ancho', 2),
(182, 2, 'Torsión', 2),
(183, 2, 'Golpe espalda', 1),
(184, 2, 'Extensión', 1),
(185, 2, 'Sello', 1),
(186, 2, 'Área de contacto', 1),
(187, 2, 'Bad mark', 1),
(188, 2, 'Óxido', 1),
(190, 2, 'Rayón', 1),
(191, 2, 'Dimensiones', 1),
...
(193, 2, 'Redondez', 1),
(194, 2, 'Planicidad', 1),
(195, 2, 'Falla de blank', 1),
(196, 2, 'Piezas caidas', 1),
(197, 2, 'Dandori', 1),
(198, 2, 'Ajustes', 1),
(199, 2, 'Inspección de calidad', 1),
(200, 2, 'Otros', 1),
(201, 2, 'Rayón espalda', 2),
(202, 2, 'Rayón aleación', 2),
(203, 2, 'Otros', 2),
(204, 2, 'Torsión', 3),
(205, 3, 'Desprendimiento', 5),
(206, 3, 'Contaminación', 5),
(207, 3, 'Óxido', 5),
(208, 3, 'Otros', 5),
(209, 3, 'Rayón de espalda', 6),
(210, 3, 'Mal corte de chaflán', 6),
(211, 3, 'Mal corte de ancho', 6),
(212, 3, 'Torsión', 6),
(213, 3, 'Otros', 6),
(214, 3, 'Golpe espalda', 7),
(215, 3, 'Extensión', 7),
(216, 3, 'Torsión', 7),
(217, 3, 'Sello', 7),
(218, 3, 'Área de contacto', 7),
(219, 3, 'Bad mark', 7),
(220, 3, 'Óxido', 7),
(221, 3, 'Moleteado', 7),
(222, 3, 'Otros', 7),
(223, 3, 'Rebaba en orificio', 8),
(224, 3, 'Rebaba en uña', 8),
(225, 3, 'Rebaba en canal', 8),
(226, 3, 'Corte de esquinas', 8),
(227, 3, 'Orificio', 8),
(228, 3, 'Rayón cara interna', 8),
(229, 3, 'Rayón espalda', 8),
(230, 3, 'Rayón cara lateral', 8),
(231, 3, 'Espesor', 8),
(232, 3, 'Golpe en chaflán interno', 8),
(233, 3, 'Golpe en cara lateral', 8),
(234, 3, 'Golpe en chaflán de canal', 8),
(235, 3, 'Golpe en chaflán corte altura', 8),
(236, 3, 'Golpe espalda', 8),
(237, 3, 'Alivio de uña', 8),
(238, 3, 'Expulsión de uña', 8),
(239, 3, 'Canal', 8),
(240, 3, 'Chaflán de orificio', 8),
(241, 3, 'Acabado de ancho', 8),
(242, 3, 'Verificador de ancho', 8),
(243, 3, 'Alimentador', 8),
(244, 3, 'Mal manejo', 8),
(245, 3, 'Cepillado', 8),
(246, 3, 'Corte de altura', 8),
(247, 3, 'Autochecador - altura', 8),
(248, 3, 'Autochecador - proceso', 8),
(249, 3, 'Pokayoke boring / broach', 8),
(250, 3, 'Dandori', 8),
(251, 3, 'Inspección de calidad', 8),
(252, 3, 'Otros', 8);
...
CREATE TABLE `registrocausasparo` (
  ...
  `catalogo_andon_id_andon` int(11) DEFAULT NULL,
  `catalogo_nivel_id_nivel` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci;
...
INSERT INTO `registrocausasparo` (`idregistrocausasparo`, `empleado_idempleado`, `causas_paro_idcausas_paro`, `tiempo`, `detalle`, `hora_inicio`, `fecha`, `linea`, `das_id_das`, `mog_idmog`, `hora_fin`, `catalogo_andon_id_andon`, `catalogo_nivel_id_nivel`) VALUES
(1, 746, 99, 26, '', '07:58', '2020-11-19', 'TP10', 53, 58, NULL, NULL, NULL),
(2, 145, 94, 27, '', '08:24', '2020-11-19', 'TP01', 54, 59, NULL, NULL, NULL),
(3, 746, 103, 22, 'BREAK ', '10:30', '2020-11-19', 'TP10', 55, 58, NULL, NULL, NULL),
(4, 145, 100, 28, '', '11:36', '2020-11-19', 'TP01', 56, 59, NULL, NULL, NULL),
(5, 746, 96, 40, '', '12:05', '2020-11-19', 'TP10', 55, 58, NULL, NULL, NULL),
(6, 149, 73, 22, '', '08:14', '2020-11-21', 'TP01', 57, 60, NULL, NULL, NULL),
(7, 149, 90, 15, '', '08:54', '2020-11-21', 'TP01', 58, 60, NULL, NULL, NULL),
(8, 148, 96, 28, '', '10:34', '2020-11-21', 'TP41', 60, 61, NULL, NULL, NULL),
(9, 149, 103, 28, 'BREAK', '10:55', '2020-11-21', 'TP01', 58, 60, NULL, NULL, NULL),
(10, 743, 100, 40, '', '13:41', '2020-12-08', 'TP41', 89, 77, NULL, NULL, NULL),
(11, 740, 73, 66, '', '07:17', '2020-12-18', 'TP41', 106, 85, NULL, NULL, NULL),
(12, 66, 36, 26, 'E AJUSTE INICIAL', '11:17', '2021-08-03', 'TH02', 175, 128, NULL, NULL, NULL),
(13, 66, 24, 122, '', '11:43', '2021-08-03', 'TH02', 175, 128, NULL, NULL, NULL),
(14, 69, 8, 23, '', '08:40', '2021-08-04', 'TH13', 176, 129, NULL, NULL, NULL),
(15, 131, 25, 26, '', '09:44', '2021-08-18', 'TH03', 181, 139, NULL, NULL, NULL),
(16, 42, 9, 12, '', '11:11', '2021-08-19', 'TH10', 185, 141, NULL, NULL, NULL),
(17, NULL, 73, 0, 'ORA SI WEY. UN POLLO ASADO SI NO JALA', '08:16:56', '2025-03-06', 'TP08', 268, 0, '08:17:26', NULL, NULL),
(18, NULL, 73, 0, 'ORA SI WEY. UN POLLO ASADO SI NO JALA', '08:17:50', '2025-03-06', 'TP08', 268, 0, '08:17:52', NULL, NULL),
(19, NULL, 73, 0, 'ORA SI WEY. UN POLLO ASADO SI NO JALA', '08:16:56', '2025-03-06', 'TP08', 268, 0, '08:17:54', NULL, NULL),
(20, 149, 99, 5, 'GUILY GUONCA Y SUS ENANITOS', '09:14:35', '2025-03-06', 'TP08', 268, 216, '09:20:23', NULL, NULL),
(21, 149, 99, 5, 'cuernavaca', '10:14:07', '2025-03-06', 'TP08', 268, 216, '10:20:05', NULL, NULL),
(22, 149, 73, 0, 'cuernavaca', '10:14:07', '2025-03-06', 'TP08', 268, 216, '10:20:46', NULL, NULL),
(23, NULL, 25, 0, 'ANDO GIRANDO BIEN PADRE AMÁ', '10:28:41', '2025-03-06', 'TH17', 269, 216, '10:28:55', NULL, NULL),
(24, 49, 39, 5, 'MUY HUMILDE', '10:39:34', '2025-03-06', 'TH17', 269, 216, '10:44:39', NULL, NULL),
(25, 149, 98, 5, 'QUIERO UN TACO DE CEVIVHE', '10:40:11', '2025-03-06', 'TP08', 268, 216, '10:45:19', NULL, NULL),
(26, 49, 11, 9, 'EFWRFWER', '10:51:06', '2025-03-06', 'TH17', 269, 216, '11:00:26', NULL, NULL),
(27, 49, 25, 5, 'SE AJUSTÓ LA VERDADERA Y AUTÉNTICA NAVAJA', '14:08:17', '2025-03-06', 'TH17', 269, 216, '14:13:19', NULL, NULL),
(28, 149, 98, 0, 'PINCHE GEMINI QLERO', '14:17:24', '2025-03-06', 'TP08', 268, 216, '14:17:41', NULL, NULL),
(29, 149, 100, 0, 'OTRO TACO DE CEVIVHE POR FAVOR', '14:18:17', '2025-03-06', 'TP08', 268, 216, '14:18:28', NULL, NULL),
(30, 49, 1, 5, 'czvsdvsdvdsv', '14:20:03', '2025-03-06', 'TH17', 269, 216, '14:25:26', NULL, NULL),
(31, 49, 39, 7, 'SI', '07:44:21', '2025-03-07', 'TH17', 271, 216, '07:51:26', NULL, NULL),
(33, 761, 98, 0, 'A VER QUE ROLLO CON EL POLLO', '09:04:04', '2025-03-10', 'TP02', 274, 232, '09:04:14', NULL, NULL),
(34, NULL, 48, 5, 'AAAAAAAAAAAAAA', '09:03:32', '2025-03-13', 'TH17', 0, 216, '09:08:49', NULL, NULL),
(35, NULL, 40, 5, 'AQQAAA', '10:18:43', '2025-03-13', 'TH17', 0, 216, '10:24:21', NULL, NULL),
(36, NULL, 35, 7, 'AAAAAA', '10:24:51', '2025-03-13', 'TH17', 0, 216, '10:32:14', NULL, NULL),
(37, NULL, 1, 9, 'QQQQQQ', '10:35:13', '2025-03-13', 'TH17', 0, 216, '10:44:16', NULL, NULL),
(38, NULL, 40, 5, 'RTHRR', '12:25:34', '2025-03-13', 'TH17', 0, 216, '12:30:44', NULL, NULL),
(39, 49, 1, 13, 'ASADSADADASDSADADSADADAD', '12:34:11', '2025-03-13', 'TH17', 0, 216, '12:47:28', NULL, NULL),
(40, 49, 1, 0, 'ASDASDSADAD', '15:37:49', '2025-03-13', 'TH17', 0, 216, '15:38:02', NULL, NULL),
(41, 49, 40, 0, 'ASCDSACASC', '15:41:02', '2025-03-13', 'TH17', 0, 216, '15:41:08', 2, 2),
(42, 49, 48, 0, 'ASDASDASD', '15:41:29', '2025-03-13', 'TH17', 0, 216, '15:41:40', 4, 1),
(43, 49, 1, 0, 'N,MN,M', '15:44:38', '2025-03-13', 'TH17', 0, 216, '15:44:46', 1, 4),
(44, 49, 9, 0, 'ASDADA', '15:46:40', '2025-03-18', 'TH17', 0, 216, '15:46:46', 1, 4),
(45, 49, 48, 0, 'ASDSADSAD', '15:46:49', '2025-03-18', 'TH17', 0, 216, '15:46:56', 1, 2),
(46, 49, 48, 0, 'AAAAAAAAAAAAAAAAAAAAAAA', '15:46:58', '2025-03-18', 'TH17', 0, 216, '15:47:04', NULL, NULL),
(47, 49, 40, 0, 'UI', '16:02:35', '2025-03-13', 'TH17', 0, 216, '16:03:01', NULL, NULL),
(48, 49, 1, 0, 'OJALA SIRVA', '07:54:41', '2025-03-18', 'TH17', 289, 216, '07:54:52', NULL, NULL),
(49, 49, 21, 0, 'SEGUNDA PRUEBA', '07:55:58', '2025-03-18', 'TH17', 289, 216, '07:56:11', 2, 1),
(50, 761, 73, 0, 'OILO, OILO, OILO', '12:16:27', '2025-03-18', 'TP08', 290, 216, '12:16:37', NULL, 1),
(51, 49, 13, 17, 'AAAAAA', '12:28:06', '2025-03-18', 'TH17', 289, 216, '12:46:03', 2, 4),
(52, 49, 46, 0, 'AJUSTE', '13:12:18', '2025-03-18', 'TH17', 289, 216, '13:12:36', NULL, NULL),
(53, 761, 100, 0, 'HOLA', '07:31:18', '2025-03-19', 'TP08', 291, 216, '10:04:00', NULL, 1),
(54, 761, 99, 3, 'OLIWIIS', '07:40:51', '2025-03-19', 'TP08', 291, 216, '10:09:00', NULL, 1),
(55, 761, 73, 4, 'AL CUATO', '10:05:00', '2025-03-19', 'TP08', 291, 216, '10:09:00', NULL, 1),
(56, 49, 36, 5, 'TAISO', '07:00:00', '2025-03-19', 'TH17', 292, 216, '07:10:00', NULL, NULL),
(57, 49, 25, 19, 'AJUSTE', '07:10:00', '2025-03-19', 'TH17', 292, 216, '07:29:00', 4, 3),
(58, 49, 1, 28, 'ASUNTO', '07:56:00', '2025-03-19', 'TH17', 292, 216, '08:24:00', 1, 2),
(59, 49, 1, 20, 'IKIK', '08:23:00', '2025-03-19', 'TH17', 292, 216, '08:43:00', 1, 1),
(60, 761, 99, 175, 'muy mucho rato', '12:40:00', '2025-03-20', 'TP08', 303, 216, '15:35:00', NULL, 1),
(61, 49, 40, 3, 'AAAAA', '15:33:21', '2025-03-20', 'TH17', 305, 216, '15:36:26', NULL, NULL),
(62, 49, 1, 3, 'AAAAA', '15:33:21', '2025-03-25', 'TH17', 312, 216, '15:56:38', 3, 1),
(63, 49, 1, 3, 'AAAAA', '15:33:21', '2025-03-20', 'TH17', 305, 216, '15:37:16', 3, 1),
(64, 49, 40, 183, 'NADOTA', '10:23:00', '2025-03-20', 'TH17', 305, 216, '13:26:00', 1, 4),
(65, 49, 34, 0, '', '15:49:11', '2025-03-20', 'TH17', 305, 216, '15:49:31', NULL, NULL),
(66, 49, 1, 0, 'rht', '15:53:17', '2025-03-20', 'TH17', 305, 216, '15:53:28', 2, 2),
(67, 49, 34, 13, '', '12:21:00', '2025-03-20', 'TH17', 305, 216, '12:34:00', NULL, NULL),
(68, 761, 100, 96, 'BANANA DE CHOCOFLAN', '13:00:00', '2025-03-24', 'TP08', 310, 249, '14:36:00', NULL, 1),
(69, 761, 97, 70, 'mike guasauski', '13:40:00', '2025-03-24', 'TP08', 310, 250, '14:50:00', NULL, 1),
(70, 761, 100, 30, 'OSTIA TIO, FLIPO, FLIPAS, FLIPAMOS', '10:30:00', '2025-03-24', 'TP08', 310, 251, '11:00:00', NULL, 1),
(71, 761, 99, 60, 'QUESO CON JAPALEÑO', '16:00:00', '2025-03-24', 'TP08', 310, 251, '17:00:00', NULL, 1),
(72, 761, 98, 91, 'ASUNTO?????', '14:59:00', '2025-03-25', 'TP08', 313, 252, '16:30:00', NULL, 1),
(73, 761, 99, 25, 'al tre', '09:33:20', '2025-03-25', 'TP08', 313, 252, '09:59:00', NULL, 1),
(74, 761, 99, 60, 'SE ALEJAN COSITAAS', '14:00:00', '2025-03-25', 'TP08', 313, 253, '15:00:00', NULL, 1);
...
(235, 'PRS', 'PRENSA', 1, 1, 1, 231, 10, 'TM321012461', NULL),
(236, 'PRS054690', 'PRENSA', 1, 1, 1, 232, 10, 'TM325012134', NULL),
(237, 'PRS054896', 'PRENSA', 1, 1, 1, 233, 10, 'TM325020544', NULL),
(238, 'HBL056744', 'MAQUINADO', 1, 1, 1, 234, 20, 'TM325012121', NULL),
(239, 'PRS054578', 'PRENSA', 1, 1, 1, 235, 10, 'TM325011412', NULL),
(240, 'PRS054897', 'PRENSA', 1, 1, 1, 236, 10, 'TM325020549', NULL),
(241, 'HBL056790', 'MAQUINADO', 1, 1, 1, 237, 20, 'TM325012350', NULL),
(242, 'HBL054879', 'MAQUINADO', 1, 1, 1, 238, 20, 'TM324092358', NULL),
(243, 'HBL045789', 'MAQUINADO', 1, 1, 1, 239, 20, 'TM323022386', NULL),
(244, 'PRS054681', 'PRENSA', 1, 1, 1, 240, 10, 'TM325012079', NULL),
(245, 'PRS054682', 'PRENSA', 1, 1, 1, 241, 10, 'TM325012083', NULL),
(246, 'PRS054677', 'PRENSA', 1, 1, 1, 245, 10, 'TM325012061', NULL),
(247, 'PRS054601', 'PRENSA', 1, 1, 1, 246, 10, 'TM325011538', NULL),
(248, 'PRS054699', 'PRENSA', 1, 1, 1, 247, 10, 'TM325012174', NULL),
(249, 'PRS054666', 'PRENSA', 1, 1, 1, 222, 10, 'TM325011943', NULL),
(250, 'PRS054691', 'PRENSA', 1, 1, 1, 223, 10, 'TM325012138', NULL),
(251, 'PRS054650', 'PRENSA', 1, 1, 1, 248, 10, 'TM325011870', NULL),
(252, 'PRS054672', 'PRENSA', 1, 1, 1, 249, 10, 'TM325011971', NULL),
(253, 'PRS054669', 'PRENSA', 1, 1, 1, 250, 10, 'TM325011957', NULL),
(254, 'PRS054668', 'PRENSA', 1, 1, 1, 251, 10, 'TM325011952', NULL),
(255, 'PRS054655', 'PRENSA', 1, 1, 1, 252, 10, 'TM325011890', NULL),
(256, 'PRS054657', 'PRENSA', 1, 1, 1, 253, 10, 'TM325011899', NULL);
...
INSERT INTO `registro_x_hora` (`id_registro_x_hora`, `hora`, `cantidadxhora`, `acumulado`, `comentarios`, `fecha`, `mog_id_mog`, `registro_rbp_id_registro_rbp`, `linea`, `das_id_das`) VALUES
(22, '11:23:43', 1000, 1000, 'ASDF', '2025-03-03', 216, 218, 'TP08', 200),
(23, '11:40:11', 4000, 5000, 'ASFD', '2025-03-03', 216, 218, 'TP08', 200),
(24, '12:30:44', 0, 5000, 'SI NO SALE 0, SOY TOJO', '2025-03-03', 216, 218, 'TP08', 200),
(25, '08:37:08', 1, 5001, 'a poco nomas 1 en do dia', '2025-03-06', 216, 218, 'TP08', 200),
(26, '08:37:22', 1, 5001, 'adsf', '2025-03-06', 216, 218, 'TP08', 200),
(27, '08:37:44', 1, 5001, 'asdf', '2025-03-06', 216, 218, 'TP08', 200),
(28, '10:13:52', 7311, 12312, 'EL CHICO DEL APARTAMENTO 512 O... OO... 52E', '2025-03-06', 216, 218, 'TP08', 200),
(29, '08:54:03', 1, 12313, '', '2025-03-07', 216, 218, 'TP08', 200),
(30, '11:02:25', 1000, 1000, 'ASDJad', '2025-03-07', 216, 218, 'TP08', 200),
(31, '11:28:53', 5000, 5000, 'SI', '2025-03-07', 226, 230, 'TP09', 200),
(32, '08:58:05', 3000, 3000, 'WACHA COMO NO JALA WWEY', '2025-03-10', 232, 236, 'TP08', 200),
(38, '13:50:24', 1000, 1000, 'ombligo', '2025-03-20', 216, 218, 'TP08', 303),
(39, '14:21:59', 1000, 1000, 'un machete borracho', '2025-03-20', 216, 218, 'TP08', 303),
(40, '07:52:14', 7000, 7000, '', '2025-03-24', 249, 252, 'TP08', 310),
(41, '12:45:30', 1000, 8000, 'confleis con jaiba', '2025-03-24', 251, 254, 'TP08', 310),
(42, '13:45:47', 1001, 9001, 'TIRO AL VIENTO CORRE BLANCO', '2025-03-24', 251, 254, 'TP08', 310),
(43, '13:53:32', 10000, 10000, 'asunto?', '2025-03-25', 252, 255, 'TP08', 313),
(44, '14:17:29', 1, 10001, 'melon rojo', '2025-03-25', 253, 256, 'TP08', 313);
...
INSERT INTO `registro_x_hora_maq` (`id_registro_x_hora`, `hora`, `cantidadxhora`, `acumulado`, `ok_ng`, `fecha`, `linea`, `mog_id_mog`, `registro_rbp_id_registro_rbp`, `empleado_id_empleado`, `das_id_das`) VALUES
(46, '08:34:54', 26000, 26000, 'OK', '2025-03-06', 'TH17', 216, 220, 49, 202),
(47, '11:47:18', 64000, 90000, 'NG', '2025-03-06', 'TH17', 216, 220, 49, 202),
(48, '13:54:17', 30000, 120000, 'OK', '2025-03-06', 'TH17', 216, 220, 49, 202),
(49, '07:43:29', 1500, 1500, 'OK', '2025-03-07', 'TH17', 216, 220, 49, 202),
(50, '08:00:32', 200, 1700, 'OK', '2025-03-07', 'TH17', 216, 220, 49, 202),
(51, '10:03:42', 9000, 9000, 'OK', '2025-03-10', 'TH17', 216, 220, 49, 202),
(52, '07:44:12', 7500, 7500, 'OK', '2025-03-11', 'TH17', 216, 220, 49, 202),
(53, '11:06:49', 7800, 7800, 'OK', '2025-03-12', 'TH17', 216, 220, 49, 202),
(54, '07:42:39', 9000, 9000, 'OK', '2025-03-13', 'TH17', 216, 220, 49, 202),
(55, '08:01:32', 1000, 10000, 'NG', '2025-03-13', 'TH17', 216, 220, 49, 202),
(56, '10:52:34', 9000, 19000, 'OK', '2025-03-13', 'TH17', 216, 220, 49, 202),
(57, '08:15:59', 8000, 8000, 'OK', '2025-03-14', 'TH17', 216, 220, 49, 202),
(58, '10:05:16', 11000, 19000, 'OK', '2025-03-14', 'TH17', 216, 220, 107, 202),
(59, '07:07:45', 7000, 7000, 'OK', '2025-03-18', 'TH17', 216, 220, 107, 202),
(60, '10:00:27', 8000, 8000, 'OK', '2025-03-19', 'TH17', 216, 220, 107, 202),
(61, '10:57:22', 5000, 5000, 'OK', '2025-03-20', 'TH17', 216, 220, 107, 305),
(63, '15:26:14', 8000, 8000, 'OK', '2025-03-20', 'TH17', 216, 220, 107, 307),
(70, '13:52:39', 8900, 8900, 'OK', '2025-03-25', 'TH17', 216, 220, 107, 312),
(72, '14:58:38', 1100, 10000, 'NG', '2025-03-25', 'TH17', 216, 220, 107, 312),
(77, '15:31:07', 5600, 15600, 'OK', '2025-03-25', 'TH17', 216, 220, 107, 312);

-- --------------------------------------------------------

--
-- Estructura de tabla para la tabla `subcategoria_rechazo`
--

CREATE TABLE `subcategoria_rechazo` (
  `id_subcategoria` mediumint(8) UNSIGNED NOT NULL,
  `subcategoria` varchar(80) NOT NULL,
  `categoria_rechazo_id_categoria_rechazo` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Volcado de datos para la tabla `subcategoria_rechazo`
--

INSERT INTO `subcategoria_rechazo` (`id_subcategoria`, `subcategoria`, `categoria_rechazo_id_categoria_rechazo`) VALUES
(1, 'Prensa', 2),
(2, 'Coiling', 2),
(3, 'Slitter', 2),
(5, 'Bimetal', 3),
...
--
-- Estructura de tabla para la tabla `work_center_maquina_has_catalogo_grupo`
--

CREATE TABLE `work_center_maquina_has_catalogo_grupo` (
  `catalogo_grupo_maquinado_id_grupo` int(10) UNSIGNED DEFAULT NULL,
  `work_center_maquina_id_work_center_maquina` int(10) UNSIGNED DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Volcado de datos para la tabla `work_center_maquina_has_catalogo_grupo`
--

INSERT INTO `work_center_maquina_has_catalogo_grupo` (`catalogo_grupo_maquinado_id_grupo`, `work_center_maquina_id_work_center_maquina`) VALUES
(1, 31),
(1, 32),
(1, 33),
(1, 46),
...
--
-- Indices de la tabla `cambio_mog`
--
ALTER TABLE `cambio_mog`
  ADD PRIMARY KEY (`id_cambio_mog`),
  ADD KEY `mog_id_mog` (`mog_id_mog`),
  ADD KEY `registro_rbp_id_registro_rbp` (`registro_rbp_id_registro_rbp`);

--
-- Indices de la tabla `catalogo_andon`
--
ALTER TABLE `catalogo_andon`
  ADD PRIMARY KEY (`id_andon`);

--
-- Indices de la tabla `catalogo_andon_has_procesos`
--
ALTER TABLE `catalogo_andon_has_procesos`
  ADD PRIMARY KEY (`catalogo_andon_id_andon`,`procesos_id_proceso`),
  ADD KEY `procesos_id_proceso` (`procesos_id_proceso`);

--
-- Indices de la tabla `catalogo_grupo_maquinado`
--
ALTER TABLE `catalogo_grupo_maquinado`
...
--
-- Indices de la tabla `das_prod_prensa`
--
ALTER TABLE `das_prod_prensa`
  ADD PRIMARY KEY (`id_daspropren`),
  ADD KEY `mog_idmog` (`mog_idmog`),
  ADD KEY `das_iddas` (`das_iddas`);
...
--
-- Indices de la tabla `piezas_procesadas_maquinado`
--
ALTER TABLE `piezas_procesadas_maquinado`
  ADD PRIMARY KEY (`id_piezas_procesadas_maquinado`),
  ADD KEY `piezas_procesadas_FKIndex2` (`registro_rbp_id_registro_rbp`),
  ADD KEY `fk_das_id_das` (`das_id_das`);

--
-- Indices de la tabla `piezas_procesadas_prensa`
--
ALTER TABLE `piezas_procesadas_prensa`
  ADD PRIMARY KEY (`idpiezas_procesadas`),
  ADD KEY `piezas_procesadas_FKIndex2` (`registro_rbp_id_registro_rbp`),
  ADD KEY `dasiddas` (`dasiddas`);
...
ALTER TABLE `registrocausasparo`
  ...
  ADD KEY `mog_idmog` (`mog_idmog`),
  ADD KEY `FK_registrocausasparo_andon` (`catalogo_andon_id_andon`),
  ADD KEY `FK_registrocausasparo_nivel` (`catalogo_nivel_id_nivel`);
...
ALTER TABLE `registro_x_hora`
  ...
  ADD KEY `mog_id_mog` (`mog_id_mog`),
  ADD KEY `registro_rbp_id_registro_rbp` (`registro_rbp_id_registro_rbp`),
  ADD KEY `fk_registro_x_hora_das` (`das_id_das`);
...
--
-- Indices de la tabla `subcategoria_rechazo`
--
ALTER TABLE `subcategoria_rechazo`
...
--
-- Indices de la tabla `work_center_maquina_has_catalogo_grupo`
--
ALTER TABLE `work_center_maquina_has_catalogo_grupo`
  ADD KEY `catalogo_grupo_maquinado_id_grupo` (`catalogo_grupo_maquinado_id_grupo`),
  ADD KEY `work_center_maquina_id_work_center_maquina` (`work_center_maquina_id_work_center_maquina`);
...
--
-- AUTO_INCREMENT de la tabla `cambio_mog`
--
ALTER TABLE `cambio_mog`
  MODIFY `id_cambio_mog` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;

--
-- AUTO_INCREMENT de la tabla `catalogo_andon`
--
ALTER TABLE `catalogo_andon`
  MODIFY `id_andon` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- AUTO_INCREMENT de la tabla `catalogo_grupo_maquinado`
--
ALTER TABLE `catalogo_grupo_maquinado`
...
--
ALTER TABLE `corriendoactualmente`
  MODIFY `id_corriendo` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1439;

--
-- AUTO_INCREMENT de la tabla `das`
--
ALTER TABLE `das`
  MODIFY `id_das` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=314;
...
--
-- AUTO_INCREMENT de la tabla `das_prod_prensa`
--
ALTER TABLE `das_prod_prensa`
  MODIFY `id_daspropren` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=144;
...
--
ALTER TABLE `defecto1`
  MODIFY `id_defecto` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1079;
...
--
ALTER TABLE `lote_coil`
  MODIFY `id_lote_coil` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=234;

--
-- AUTO_INCREMENT de la tabla `mog`
--
ALTER TABLE `mog`
  MODIFY `id_mog` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=254;
...
--
-- AUTO_INCREMENT de la tabla `piezas_procesadas_maquinado`
--
ALTER TABLE `piezas_procesadas_maquinado`
  MODIFY `id_piezas_procesadas_maquinado` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=148;

--
-- AUTO_INCREMENT de la tabla `piezas_procesadas_prensa`
--
ALTER TABLE `piezas_procesadas_prensa`
  MODIFY `idpiezas_procesadas` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=583;
...
--
ALTER TABLE `razon_rechazo1`
  MODIFY `id_razon_rechazo` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=253;

--
-- AUTO_INCREMENT de la tabla `registrocausasparo`
--
ALTER TABLE `registrocausasparo`
  MODIFY `idregistrocausasparo` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=75;
...
--
ALTER TABLE `registro_rbp`
  MODIFY `id_registro_rbp` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=257;

--
-- AUTO_INCREMENT de la tabla `registro_x_hora`
--
ALTER TABLE `registro_x_hora`
  MODIFY `id_registro_x_hora` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=45;

--
-- AUTO_INCREMENT de la tabla `registro_x_hora_maq`
--
ALTER TABLE `registro_x_hora_maq`
  MODIFY `id_registro_x_hora` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=78;

--
-- AUTO_INCREMENT de la tabla `subcategoria_rechazo`
--
ALTER TABLE `subcategoria_rechazo`
  MODIFY `id_subcategoria` mediumint(8) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
...
--
-- Filtros para la tabla `cambio_mog`
--
ALTER TABLE `cambio_mog`
  ADD CONSTRAINT `mog_id_mog` FOREIGN KEY (`mog_id_mog`) REFERENCES `mog` (`id_mog`),
  ADD CONSTRAINT `registro_rbp_id_registro_rbp` FOREIGN KEY (`registro_rbp_id_registro_rbp`) REFERENCES `registro_rbp` (`id_registro_rbp`);

--
-- Filtros para la tabla `catalogo_andon_has_procesos`
--
ALTER TABLE `catalogo_andon_has_procesos`
  ADD CONSTRAINT `catalogo_andon_has_procesos_ibfk_1` FOREIGN KEY (`catalogo_andon_id_andon`) REFERENCES `catalogo_andon` (`id_andon`),
  ADD CONSTRAINT `catalogo_andon_has_procesos_ibfk_2` FOREIGN KEY (`procesos_id_proceso`) REFERENCES `procesos` (`id_proceso`);
...
--
-- Filtros para la tabla `piezas_procesadas_maquinado`
--
ALTER TABLE `piezas_procesadas_maquinado`
  ADD CONSTRAINT `fk_das_id_das` FOREIGN KEY (`das_id_das`) REFERENCES `das` (`id_das`);

--
-- Filtros para la tabla `registrocausasparo`
--
ALTER TABLE `registrocausasparo`
  ADD CONSTRAINT `FK_registrocausasparo_andon` FOREIGN KEY (`catalogo_andon_id_andon`) REFERENCES `catalogo_andon` (`id_andon`),
  ADD CONSTRAINT `FK_registrocausasparo_nivel` FOREIGN KEY (`catalogo_nivel_id_nivel`) REFERENCES `catalogo_nivel` (`id_nivel`);

--
-- Filtros para la tabla `registro_x_hora`
--
ALTER TABLE `registro_x_hora`
  ADD CONSTRAINT `fk_registro_x_hora_das` FOREIGN KEY (`das_id_das`) REFERENCES `das` (`id_das`) ON DELETE CASCADE,
...
--
ALTER TABLE `registro_x_hora_maq`
  ADD CONSTRAINT `das_id_das` FOREIGN KEY (`das_id_das`) REFERENCES `das` (`id_das`),
...
--
-- Filtros para la tabla `work_center_maquina_has_catalogo_grupo`
--
ALTER TABLE `work_center_maquina_has_catalogo_grupo`
  ADD CONSTRAINT `work_center_maquina_has_catalogo_grupo_ibfk_1` FOREIGN KEY (`catalogo_grupo_maquinado_id_grupo`) REFERENCES `catalogo_grupo_maquinado` (`id_grupo`),
  ADD CONSTRAINT `work_center_maquina_has_catalogo_grupo_ibfk_2` FOREIGN KEY (`work_center_maquina_id_work_center_maquina`) REFERENCES `work_center_maquina` (`id_work_center_maquina`);
COMMIT;
